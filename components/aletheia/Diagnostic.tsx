...
function Lettre({ n }: { n: number | null | undefined }) {
  ...
  if (!l) return <span className="text-stone-300">—</span>
...
export function NiveauAxe({ v1, vf, malDefV1 = false, malDefVf = false }: {
  ...
  const cell = (n: number | null, malDef: boolean) =>
    malDef ? <span className="text-[10px] text-stone-400 italic">n.d.</span> : <Lettre n={n} />
  ...
  return (
    <span className="inline-flex items-center gap-0.5">
      {cell(v1, malDefV1 === true)}<span className="text-stone-300 text-[10px]">→</span>{cell(vf, malDefVf === true)}
    </span>
  )
}
...
export function TrajectoireDiag({ semaines, diag }: { semaines: number[]; diag: Map<number, DiagnosticTravail> }) {
  ...
  if (cells.length === 0) return <span className="text-[11px] text-stone-400">pas encore diagnostiqué</span>
  return (
    <div className="flex flex-wrap items-center gap-x-2 gap-y-1">
      {cells.map(({ s, d }) => (
        <span key={s} className="inline-flex items-center gap-1 text-[11px]" title={titreDiag(s, d)}>
          <span className="text-stone-400">S{s}</span>
...
export function DetailDiagChapitre({ d }: { d: DiagnosticTravail | undefined }) {
  ...
  if (rien && d?.erreur_at) return <p className="text-xs text-red-500">Le calcul du diagnostic a échoué — relance le diagnostic.</p>
  if (rien || !d) return <p className="text-xs text-stone-400">Pas encore diagnostiqué.</p>
  return (
    <div className="space-y-1.5 text-xs">
      <div className="flex flex-wrap gap-x-4 gap-y-1">
        <span className="text-stone-500">Saisie de la thèse : <NiveauAxe v1={d.niveau_these_v1} vf={d.niveau_these_vf} malDefV1={d.these_mal_definie_v1} malDefVf={d.these_mal_definie_vf} /></span>
        <span className="text-stone-500">Restitution des arguments : <NiveauAxe v1={d.niveau_arguments_v1} vf={d.niveau_arguments_vf} /></span>
        <span className="text-stone-300">(V1 → VF · E faible → A fort)</span>
      </div>
      {d.inventaire_v1?.note && <p className="text-stone-500 italic">{d.inventaire_v1.note}</p>}
      {d.erreur_at && <p className="text-red-500">Le dernier calcul du diagnostic a échoué — relance le diagnostic.</p>}
    </div>
  )
}
