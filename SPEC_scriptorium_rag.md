...
2. **Parcours adopte le patron modèle → instance par classe** (celui des plans d'évaluation). Assigner = **matérialiser une instance indépendante** (créneaux + éléments copiés). L'instance diverge librement ; **modifier le modèle ensuite ne redescend PAS** dans les instances déjà créées (ré-initialiser l'instance depuis le modèle = geste explicite et destructif). Confirmé PO après explication de l'exemple « correction de novembre ».
   **Amendement PO du 02/09/2026 — le modèle SUIT ses classes.** « Je sais combien de temps vont durer mes cours, mais le contenu exact, je ne sais pas » (Louis, 01/09) : un parcours se remplit au fil de l'année, après son assignation, et la photo prise à l'assignation laissait chaque cours ajouté ensuite hors des classes (mesuré en prod : un cours ajouté au modèle, absent de T5). Désormais **ce que le modèle ajoute, retire ou déplace redescend dans chaque instance active, sauf sur ce qu'une classe a déjà vu** (au moins un élément coché) — le détail est au §4.1. La divergence reste libre pour ce qui est PROPRE à la classe ; `reinitialiserInstance` reste la remise à zéro destructive.
3. **Granularité par sections.** À l'import d'un cours, le prof déclare chapitres/sous-chapitres via un éditeur sur le texte extrait (**marquage manuel par le prof — même esprit que la découpe livre ; pas de proposition IA**, abandonnée sur les livres). Modèle **plages** (précision PO post-L2) : chaque section = titre + niveau (§/§§ — grain fin pour le « vu » partiel d'un chapitre entamé) + plage de lignes **début–fin** (saisie numérique ou pose au clic), **chevauchement PARTIEL interdit** — mais un **§§ entièrement contenu dans un § est la structure attendue** (amendement PO 31/08/2026 : un chapitre et ses parties ; deux niveaux au maximum), la matière d'une section étant alors ses lignes **propres**, celles qu'aucun de ses sous-chapitres ne prend (la découpe reste une partition : aucune ligne servie deux fois, et cocher « vu » sur un chapitre ne dévoile pas une partie non encore vue ; un chapitre entièrement couvert par ses parties devient un intitulé sans matière) —, lignes hors de toute section **tolérées et signalées** (écartées de la matière — bruit d'extraction PDF) ; page du cours à gauche en **une seule feuille à défilement continu** (une section traverse librement), éditeur à droite. À l'assignation, chaque section devient un **élément** de l'instance, placé par défaut dans la semaine de son créneau, déplaçable **par classe**.
...
- ~~**Modifier le modèle** ne touche aucune instance existante ; il ne sert qu'aux assignations futures.~~ **Amendé le 02/09/2026 — le modèle suit ses classes** (règles pures dans `utils/parcours-propagation.ts`, exécution dans `actions.ts`, bloc « LE MODÈLE SUIT SES CLASSES ») :
  - **ajout** d'un créneau au modèle → copié en fin de la même semaine de chaque instance active (`modele_creneau_id` renseigné), éléments matérialisés ; un créneau PROPRE de même cible déjà présent dans la classe est **rattaché**, jamais doublé ;
  - **retrait** → les copies **intactes** (aucun élément « vu ») partent avec lui ; celles qu'une classe a déjà vues **restent** (l'écran du modèle le dit : « retiré de 2 classes ; conservé dans 1 ») ;
  - **déplacement** de semaine → les copies intactes suivent (éléments refaits pour la nouvelle semaine) ; les vues gardent leur semaine ;
  - **réordonnancement** dans une semaine → les copies permutent parmi les positions qu'elles occupent (les créneaux propres à la classe ne bougent pas ; l'instance ne sait pas réordonner ses créneaux, il n'y a aucun choix de classe à écraser) ;
  - ce que la classe a **ajouté elle-même, retiré, ou déplacé élément par élément** n'est jamais touché ; les décalages, la date et le « vu » non plus.
  - **Reprise** : la grille d'instance liste « le modèle a N contenus que cette classe n'a pas » (créneaux du modèle sans copie ni équivalent propre — les ajouts d'avant le 02/09, ou ce que la classe a retiré) et les copie d'un clic (`recupererCreneauxModele`). Le lien est `modele_creneau_id` (déjà en base).
  - **Trois compléments du 02/09 (soir), migration `parcours_suivi_du_modele.sql`** : (a) **l'ordre propre d'une classe** — `reordonnerCreneauxInstance` permute les créneaux qui vivent dans une semaine de l'instance (⇈ ⇊ sur le premier élément de chaque créneau), et la propagation de l'ordre du modèle ne redescend que là où la classe **suivait encore l'ordre d'avant** (`suitLOrdreDuModele`, comparaison à l'ordre du modèle avant le geste — aucun marqueur) ; (b) **la provenance** — `scriptorium_parcours_classe_creneaux.modele_retire_at` est posé sur les copies conservées juste avant le DELETE du modèle (la FK `set null` effaçait le lien), l'instance montre « plus au modèle », et le rattachement d'un jumeau l'efface ; (c) **l'interrupteur** — `scriptorium_parcours_classes.suit_modele` (défaut `true`) : à `false`, rien n'arrive dans l'instance ni n'en part de lui-même (`instancesDetachees` écarte la classe de tous les gestes du modèle, qui le disent : « 1 classe ne suit plus le modèle »), la reprise reste le chemin manuel. Les deux colonnes sont lues à part et tolérées absentes.
