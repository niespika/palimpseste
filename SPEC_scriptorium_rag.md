...
- ~~**Modifier le modèle** ne touche aucune instance existante ; il ne sert qu'aux assignations futures.~~ **Amendé le 02/09/2026 — le modèle suit ses classes** (règles pures dans `utils/parcours-propagation.ts`, exécution dans `actions.ts`, bloc « LE MODÈLE SUIT SES CLASSES ») :
  ...
  - **Reprise** : la grille d'instance liste « le modèle a N contenus que cette classe n'a pas » (créneaux du modèle sans copie ni équivalent propre — les ajouts d'avant le 02/09, ou ce que la classe a retiré) et les copie d'un clic (`recupererCreneauxModele`). Aucune migration : `modele_creneau_id` (déjà en base) porte le lien.
