...
> # ✅ LA PASSE EST CLOSE — 5 août 2026, séance 19.
> **Six chapitres, dix-neuf séances, du 2 au 5 août** (Louis × Mètis). La SPEC C3 est passée de
> **v4.3 à v5.3** avec **63 amendements, A20 à A82** ; les cinq sources ont été passées une à une
> puis les unes contre les autres ; **trois annexes** ont recueilli les récits de séance ; et les
> ouverts survivants sont au **registre `INVENTAIRE_Non_Tranches.md`** — 42 items, chacun avec son
> statut et sa condition de fermeture.
> **Ce prompt ne se relance plus.** Il reste comme trace : sa table de progression ci-dessous est le
> récit séance par séance de la passe, et c'est à ce titre qu'on la lit.
> **La suite, arrêtée par Louis le 05/08** : sa **relecture paragraphe par paragraphe** des quatre
> fichiers — *« un bon quart du texte qui peut sauter »* — **puis** la revue adversariale, qui n'est
> donc plus la prochaine étape et dont **le prompt reste à écrire**.
...
| 5 | Les non-tranchés — inventaire, tri, décisions mûres | ✅ **TERMINÉ le 05/08/2026 — en trois séances (17, 18, 19).** Les onze mûrs traités, les 31 autres lignes validées par Louis pile par pile, et le registre des ouverts institué. *(historique ci-dessous)* — **items déposés au 03/08** : *(a)* **ce que R0 écarte réellement à l'allumage** — la prémisse de l'item 6 du `01-` §13 est dissoute : il n'y a plus de « compétences de lecture » à écarter en bloc, et **la clé de recette n'est pas étendue à la famille** (§12), donc une compétence rendue `evaluee` en composition est ciblable en réception sans y avoir été éprouvée ; *(b)* l'inventaire des **19 marques de non-tranché de `02-exercices.md`**, relevées section par section ; *(c)* les trois trous du `00-` et les items des séances 1 à 4 déjà inscrits ; *(d)* **déposé le 03/08, séance 8 — la table de correspondance de la PAIRE pour N2** : sur un cran de diagnostic il n'y a pas de version finale, donc pas de `delta_v1_vf` ; N2 lit alors la paire (« correction, puis nouveau cas de la même famille de défauts »), **mais quel résultat de paire vaut quelle branche n'est pas tranché** *(marqué `[à valider]` au `01-routeur.md` §6)*. **Daté par C4-L2, le 25/08** : tant que ce n'est pas écrit, N2 n'est pas exécutable sur un cran de diagnostic ; *(e)* **déposé le 03/08, séance 8 — le mot « geste », jamais jugé au chapitre 1** *(objection de Louis : « type et geste n'existent plus »)*. L'axe `gestes[]` est bien mort — absorbé dans `crans[]` le 31/07 —, mais **le mot n'a aucune entrée à la table des termes** ; la ligne vient d'y être posée. **139 occurrences sur les quatre documents** ; **les 20 du `01-` ouvertes → zéro usage vivant** *(notes de renommage, descriptions datées, français ordinaire)*. ~~**Restent à ouvrir : les 69 du `02-` et les 34 de C3.**~~ **✅ ITEM CLOS le 04/08/2026, séance 11.** **Les 141 occurrences ont TOUTES été ouvertes** — y compris les **16 du `00-`, que l'item avait oubliées** *(20 + 69 + 34 = 123, et le relevé disait 139 : seize n'étaient assignées à personne)*, et **les 20 du `01-` rouvertes plutôt que crues sur parole**. **Zéro usage vivant de l'axe, partout.** **Et la vraie trouvaille est ailleurs : le mot « geste » N'EST PAS proscrit — seul `gestes[]`, l'axe avec ses crochets, l'est.** Trois termes techniques **vivants** le portent — **`geste_revision`** (champ des retours), **`{{GESTE_TENTE}}`** (emplacement du prompt de retour) et **« le geste sur les conditions »** (le champ `temps_mis`, présent dans C3, le `01-` ET le `00-`) — sans compter le français ordinaire. *Un contrôle du chapitre 6 qui grepe « geste » en attendant zéro rapporterait **141 faux positifs**. La précision est écrite à l'entrée `gestes[]` du `LEXIQUE.md`.* *À rapprocher de l'autre résidu du même ordre : le nom de la table **`exercices_types`** est un reste d'avant la refonte — ses lignes sont les treize **objets** ; c'est ce qui a fait trébucher Louis sur `depot_id`* — **DEUX ITEMS DÉPOSÉS LE 04/08, SÉANCE 12** : *(f)* **l'énumération des compétences sans lettre mêle deux causes** — C3 §6 annonce fixer le périmètre de la cause « pont E→A non validé », puis nomme des compétences pour une raison qui n'en est pas une (« aucune passation ne les mesure ») ; **cette troisième cause n'est nommée nulle part** *(relevé en posant A69)* · *(g)* **la liste du cold start omet le Questionnement** — le `01-` §4 déclare `differee` la Connaissance et la Synthèse, alors que C3 écrit lui-même que le Questionnement « n'aura pas le statut `evaluee` à la rentrée, sa fiche n'étant pas écrite » ; **il devrait y être**, et le point rejoint le régime du statut `differee`, déjà ouvert ici. — **SÉANCE 17 (05/08, 7 h 11 – 10 h 45, CLOSE) : L'INVENTAIRE EST FAIT ET LE TRI EST RENDU — 37 ITEMS EN QUATRE PILES (11 mûrs · 15 qui attendent une donnée · 7 sessions dédiées · 4 hors passe). CINQ MÛRS SOLDÉS, DEUX AMENDEMENTS (A76, A77), C3 EN v5.2.** Livrable : **`INVENTAIRE_Non_Tranches.md`**, à la **racine de `palimpseste-conception`** *(domicile choisi par Louis à la clôture)*. **CONTRÔLE D'OUVERTURE PROPRE**, première fois en six jours. *(1)* **UNE COLLISION À L'OUVERTURE, ET MÈTIS S'EST TROMPÉ EN LA TRAITANT** : `conversations/proportion.md` §4 *(session parallèle du 04/08)* conclut « A51 est faux » et « A69 tombe » ; Mètis a repris l'inférence sans la vérifier. **Louis l'a réfutée — *« un exercice peut mesurer les modes que le prof lui a assignés »*** : A51 déclare ce que mesure UNE passation, ce qui ne dit rien de ce qu'une compétence PEUT instancier. **A51 et A69 tiennent** ; c'est `proportion.md` qui a tort, et il énonce lui-même la distinction à son §1. *Cause du faux pas : le §8 de `02-exercices.md` lu à partir de la ligne 880, sans la précision du 4 août trente lignes plus haut — « ces modes sont ceux de l'EXERCICE, pas ceux d'une compétence ».* **Item mûr n° 7, NON TRAITÉ — c'est par là qu'on peut reprendre.** *(2)* **A76 — LE RÉGIME DE `differee`, ET LE STATUT REND AU PROFESSEUR CE QUI LUI APPARTIENT.** `differee` = **on ne mesure pas** · `mesuree_silencieusement` = **on mesure et on stocke, sans verdict**. **Pas de fiche → `differee` forcé ; fiche déposée → le professeur choisit**, sans automatisme. **Ouverture B1-25** : l'écran de dépôt et de choix n'appartient à aucun lot du §9. *(3)* **LA RÈGLE DE MÉTHODE QUI GOUVERNE LA SUITE, ET ELLE EST DE LOUIS** : *« si ce qu'on veut c'est que le prof se débrouille pour faire les fiches, alors tout ce qui dit pourquoi les compétences ne sont pas là n'a aucun intérêt. Ne compliquons pas un projet déjà trop compliqué. »* **Aucun document ne tient plus la liste des compétences prêtes** — Mètis proposait de RÉÉCRIRE l'énumération, elle est **RETIRÉE**. **Cinq endroits vivants, pas un**, dont une **seconde copie complète** au `01-` §8 ; seul le lot **C5-L3** est gardé, sa formulation étant déjà la règle. **Le Questionnement n'entre plus par le segment 2 mais par sa fiche.** *(4)* **A77 — R0 N'A PAS DE TROU, ET LA RÉPONSE ÉTAIT DÉJÀ ÉCRITE.** Le garde-fou annoncé par C5-L3 — « le protocole de banc impose d'éprouver chaque mode » — **n'existe pas** *(le `PROTOCOLE-CALIBRATION.md` ne le porte pas)*, mais **Louis avait tranché la même objection le 30/07** *(F3)* : **« un argument est un argument. On extrait des squelettes […] ils valent indépendamment des exercices »** — la calibration transfère d'un type, d'un genre **et d'une famille** à l'autre. **A23 ne se généralise pas.** **Cinq règles** alignées sur `evaluee`. *(5)* **TROIS ERREURS DE MÈTIS, TOUTES RATTRAPÉES PAR SES PROPRES CONTRÔLES** : la ligne « Statut : v5.1 » oubliée de la liste d'A76 · le **§1 point 7**, qu'A76 venait d'écrire, glosant encore `evaluee` par « banc passé » **dans le paragraphe qui l'interdit** · et le compte de « quatre règles » qui en valait **cinq**. *(6)* **LE CACHE DU MONTAGE A MENTI** après écriture — fichiers d'avant rendus avec des métadonnées justes ; livrer par copie à nom neuf, empreintes vérifiées des deux côtés. *(7)* **Contrôles** : 62 tables, aucune irrégulière ; propagation **item 19**. **AUCUN COMMIT — décision de Louis.** — **RESTENT SIX MÛRS** : les deux « grain » · le **domicile de la table des modes admis** · l'**inférence fausse de `proportion.md` §4** · le quatrième terme du composite `Découvertes` *(`00-` §6)* · les **ancres non neutres** *(`00-` §5)* · la **fenêtre du taux de lucidité** *(A37)*. **L'item 1 du tri est à corriger** : il annonçait « les trois causes », il y en avait quatre, et elles ont été retirées plutôt que nommées. **APRÈS LA PASSE, ANNONCÉ PAR LOUIS le 05/08** : *« je vais reprendre les 4 fichiers un à un et passer paragraphe par paragraphe pour enlever ce qui n'a pas à y être »* — son constat étant que **les documents sont encore « a big mess »**, le chapitre 4 n'ayant pas suffi. — **SÉANCE 18 (05/08, 10 h 45 – 13 h 15, CLOSE) : LES ONZE MÛRS SONT TRAITÉS — 9 SOLDÉS, 1 SORTI DE LA PASSE, 1 TRANCHÉ EN PRINCIPE. CINQ AMENDEMENTS (A78 À A82), C3 EN v5.3.** **A78** la table des modes admis prend domicile au `02-` §4, C3 la borne par renvoi · **A79** les deux « grain », dette de la Décision 8 soldée pour deux incises *(127 occurrences, huit au sens de finesse, deux ambiguës dont une au lot C4-L4)* · **A80** la fenêtre du taux de lucidité à **15 exercices**, le registre du §11 reçoit sa table Monitoring · **A81** deux renvois morts tués la veille par A77 · **A82** le **K de R5** cesse de s'instancier en un nombre *(« ≈ 6 des deux côtés » est une énumération déguisée en chiffre — sixième endroit du défaut d'A76)*. Sans amendement : le composite `Découvertes` à **trois** termes ; le **principe des ancres neutres acté, l'écriture revenant à Louis** ; la **synthèse en classe CONSERVÉE**, son rythme rendu au professeur *(« le système me permet déjà de décider quand et combien, donc pas la peine de nommer ça comme un trou »)* — **cinq « sort non tranché » retirés du routeur**. **LE BALAYAGE DE SORTIE A ÉTÉ LE PLUS RENTABLE DE LA SÉANCE** : **cinq renvois morts** *(trois contre le travail de la veille)*, **trois mal adressés**, **cinq ouverts absents du tri** — le registre passe de **37 à 42 items**. **RÉFUTATION DE LOUIS** : l'ouverture n° 5 n'est PAS réglée — statuer *quel lot* elle bloque ne l'a pas traitée ; l'item 27 reçoit sa date, **avant C4-L2 le 25/08**. **CE QUI RESTE DU CHAPITRE 5** : la validation par Louis des **31 lignes** des piles 2, 3 et hors passe — toutes portent déjà statut et condition de fermeture. **Puis le chapitre 6.** — **SÉANCE 19 (05/08, 13 h 00 – 16 h 10, CLOSE) : LE CHAPITRE 5 EST TERMINÉ ET LE CHAPITRE 6 EST PASSÉ — LA PASSE EST CLOSE.** **Les 31 lignes sont validées par Louis, pile par pile** *(méthode qu'il a choisie : trois passes, pas 31 échanges)*. **Deux items changent de pile** : le **23** *(cycle sur une page unique)* et le **42** *(découpage en blocs)* passent **hors passe** — leur condition de fermeture est une **recette de lot** (C4-L3), pas une donnée qu'on attend. Comptes : **11 · 18 · 7 · 6 = 42**. **LE REGISTRE EST INSTITUÉ** : `INVENTAIRE_Non_Tranches.md` devient **le registre des ouverts du chantier** *(décision de Louis)*, sur le partage du `01-` §11 — les sources marquent qu'un point est ouvert, le registre dit quelle donnée on attend et quand ; **la source fait foi sur le fond, le registre sur l'échéance**. **Nom de fichier conservé** : ses deux seules citations sont dans des blocs datés. **TROIS COLLISIONS DE NUMÉROS** : `27a`-`27e` → **38-42** *(le 27 était pris)* · deux adresses **B3 → B2** · et **C3 avait DEUX items numérotés 8** au tableau de bord B, celui du 03/08 prenant le **23** *(jamais servi ; « B1-8 » n'était cité qu'une fois dans tout le chantier)*. **A75 AVAIT LAISSÉ TROIS RÉSIDUS VIVANTS DU CRAN 6** — la table du `02-` §8, la dérivation `regime_v1vf` du **§6 de C3**, et la **« Lecture 2 » du `02-` §5**. *ERREUR DE MÈTIS : il avait rangé cette dernière avec la « Lecture 1 » écartée ; c'est la lecture RETENUE, et elle énonçait une règle vivante. Dite et réparée dans la minute.* **CHAPITRE 6** : **quinze motifs proscrits, 143 occurrences, toutes ouvertes → deux seuls résidus**, `Support` avec majuscule *(A49)*, au `02-` §5 et à **C3 §0** · **81 renvois inter-documents, tous résolvent**, treize lots tous définis · **un défaut de forme : le §4bis de C3 n'avait pas de titre** — trente lignes référencées cinq fois, en gras dans le corps · **`04-competences-lecture.md` reçoit son avertissement d'en-tête** *(il parlait la langue du 29/07 sans le dire)* · les **quatre sources reçoivent leur bloc de sortie de passe**, qui renvoie au tableau des statuts de C3 au lieu de le recopier · **aucun numéro de version créé** *(décision de Louis)* · le `LEXIQUE.md` cesse d'annoncer comme dû un contrôle fait le 04/08 · **bilan à l'en-tête de C3** : v4.3 → **v5.3**, **63 amendements A20-A82**, plage **A1-A82 complète, aucun manquant**. **Contrôle de forme final : 83 tables sur treize fichiers, zéro irrégulière, zéro ligne orpheline.** **IMPACT C3 : v5.3 INCHANGÉE — cinq réparations sans amendement**, A75 annoté. **CE QUE LOUIS A ARRÊTÉ POUR LA SUITE** : *« il reste encore beaucoup de big mess dans les 4 fichiers […] un bon quart du texte qui peut sauter, mais ce doit être fait par moi, paragraphe par paragraphe. Une fois ça fait, là on pourra faire une dernière passe adversariale et vraiment avancer. »* **LA REVUE ADVERSARIALE ATTEND SA RELECTURE** — elle n'est plus la prochaine étape. *Son prompt reste à écrire : les deux du repo couvrent l'un `00-`+`01-`, l'autre C3 seule.* |
| 6 | Sortie de passe — contrôles mécaniques, livrables, en-têtes | ✅ **TERMINÉ le 05/08/2026, séance 19** — le détail est à la ligne du chapitre 5 ci-dessus, la séance ayant porté les deux. **Vocabulaire, renvois, livrables, en-têtes, état C3 : les cinq contrôles sont passés.** |
