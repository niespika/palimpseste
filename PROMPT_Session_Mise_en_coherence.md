...
| 3 | Cohérence croisée (02 ↔ 01 d'abord ; 00 face aux deux ; sources ↔ C3 ; relevés ; rapport) | 🔄 **EN COURS — le point 1 est CLOS (séance 8, 03/08, 20 h 08 – 22 h 20) ; les points 2 à 5 restent.** Les sept points de contact du croisement `02-` ↔ `01-` sont passés : les six attributs écartés *(`produit_mesure` ne se consomme plus au §5 ; « complexité » définie comme le couple (`grain`, `cran`) au principe 4 et inscrite au `LEXIQUE.md` ; les quatre autres à zéro)* · `couverture_observables` *(le §6 la disait « par type », elle est **par cran × compétence × observable**)* · **la règle N1 réécrite** *(aucun type n'isole depuis la Décision 1 : N1 **garde l'objet et change le cran**)* · **`distance_contexte`** *(**A61**, C3 en **v4.8** — la règle de calcul n'existait nulle part et C4-L1 doit écrire le champ ; **N2 ne lit désormais que les mesures `formatif`**)*. **Vérifiés PROPRES au relevé de fin de séance** — la **règle de montée** *(M-a à M-e du §6 correspondent exactement à A50)* · les **deux colonnes dérivées de F7** *(lettre plafonnée / valeur de ciblage non plafonnée)* · le **`regime_v1vf`** · le **`support` et le non-emboîtement** *(la règle vit à la banque d'instances, `02-` §7 — le routeur n'a pas à la porter ; mais la **SÉRIE** et son décompte au budget du cycle restent `[à valider]` à `02-` §5, item du chapitre 5)*. **Corrigé aussi** : la ligne « Identité » du `02-` §1 faisait déclarer au type les codes `t1…t14` / `l1…l12`, une **`famille`** et une **plage d'étayage** — trois choses que le document réfute ailleurs. **Deux items de propagation écrits (11 et 12)**, tous deux sur `PROMPT_Code_C4_L1.md`. 🔴 **LE TROU DE L'ÉLECTION DES MODES A ÉTÉ OUVERT (séance 9, 03-04/08, 22 h 29 – 00 h 11) — LA CHAÎNE EST ARRÊTÉE, UNE SEULE CASE RESTE À TRANCHER, ET RIEN N'EST ÉCRIT.** *Le relevé de la séance 8 disait « `modes` a zéro occurrence au routeur » : **c'est faux au sens littéral, il y en a cinq**, mais toutes disent la même chose (« Restitution… sont devenues des modes ») et aucune n'est une règle. `modes` comme **attribut** est bien à zéro.* **Le trou était plus gros que « personne n'élit » : deux décisions actées se contredisent.** La `famille` se dérive des modes (Décision 4) et `composer` se dérive du **cran** (table de Louis, 02/08) ; or le cran est indexé sur le niveau — donc les seuls crans sans `composer` sont le **1** (E-D) et le **7** (A), et **un élève à C ou à B ne peut recevoir AUCUN exercice de `reception`**, quand **F24 lui en promet un tiers**. **LA CHAÎNE RETENUE PAR LOUIS, à écrire à la séance suivante** : distinguer **`materiau_source`** et **`materiau_cible`**, chacun avec sa **provenance** et son **support** ; **seul le `materiau_source` détermine le mode** — présent → un mode réceptif est élu, donc `famille` = `reception` ; absent → `composer`, donc `composition` — et **quel** mode réceptif se décide par le **niveau, en distribution** *(ce qui retrouve exactement la décision du 02/08 au `02-` §8 : `restituer` → `expliquer` → `évaluer`, proportions `[à valider]`, séance des proportions)*. *Contrôle fait : cette chaîne ne retraverse jamais `depot_id` — le routeur élit les modes à la création de l'exercice, et `modes` est une colonne de la mesure depuis A52.* **LA SEULE QUESTION QUI RESTE, ET C'EST PAR ELLE QU'ON REPREND** : un `materiau_source` présent **retire-t-il** `composer` (lecture *exclusive* — alors la décision du 02/08 sur `condenser`, « composer ET restituer à la fois », et la table « Où `composer` est élu » tombent) ou **ajoute-t-il** un mode réceptif à côté (lecture *additive* — alors la Décision 4 renvoie `composition` et **le trou à C et à B revient intact**) ? **Troisième voie proposée, non tranchée** : additive **plus** une clause de la Décision 4 qui bascule — « un mode réceptif est élu → `reception` » au lieu de « `composer` élu → `composition` » — la forme « la famille se dérive des modes » étant préservée et rien d'autre ne bougeant. **DEUX ITEMS NOTÉS, NON DEMANDÉS** : `materiau_source.provenance` n'aura peut-être jamais qu'une valeur *(la source vient-elle jamais d'ailleurs que de la banque de textes ?)* · la **règle de non-emboîtement** lit `support` et devra être **repointée** sur `materiau_source.support`. *Renommage chiffré : `provenance_materiau` fait **7 occurrences en tout** — 4 au `02-`, 3 dans C3.* **ET DEUX DÉFAUTS TROUVÉS EN CHEMIN, NON TOUCHÉS** : *(1)* la sortie de la couche 3 (§4) énumère encore « (type, **étayage**, registre, compétences retournées) » — l'étayage est absorbé dans les crans depuis le 31/07, et **le cran élu n'y figure pas** ; *(2)* le routeur porte la formule **proscrite** « Restitution, Reconstruction et Évaluation sont devenues des modes » à **deux endroits vivants et non routés**, son **§1** (le compte) et son **§2** (la puce Tronc commun), tous deux écrits par nous au réalignement A51 *(le `LEXIQUE.md` les proscrit **sans remplaçant**, et la formule est fausse en fait : `02-` §4 écrit « quatre modes plus **deux crans** »)*. **`support` EST AMBIGU, ET C'EST UN DÉFAUT À PART** : le document le définit comme l'étendue du **texte d'auteur** (la source), Louis le lisait comme l'étendue de la **cible** ; les deux lectures coïncident sur L6 et **divergent dès qu'un paragraphe généré explique un extrait d'auteur**. — **Les deux items durs inscrits d'avance, toujours à faire** : le **contrôle des chiffres du routeur contre C3, en entier** *(le §8 et le registre du §11 portaient encore 4 620 / 7 700 × X après le réalignement en dix endroits de la séance 4 ; marqué `[À VÉRIFIER AU CHAPITRE 3]` dans le registre)*. **Un second item inscrit le 03/08** : ce qui **bloque encore C5** est à recompter — l'ouverture n° 5 de C3 a perdu sa moitié « R1-R6 lecture » *(**A57**)*, mais ce qu'elle bloque n'a pas été restatué ; **c'est une décision de Louis** |
| 4 | L'appareil — justifications gardées, redites retirées | à faire |
| 5 | Les non-tranchés — inventaire, tri, décisions mûres | à faire — **items déposés au 03/08** : *(a)* **ce que R0 écarte réellement à l'allumage** — la prémisse de l'item 6 du `01-` §13 est dissoute : il n'y a plus de « compétences de lecture » à écarter en bloc, et **la clé de recette n'est pas étendue à la famille** (§12), donc une compétence rendue `evaluee` en composition est ciblable en réception sans y avoir été éprouvée ; *(b)* l'inventaire des **19 marques de non-tranché de `02-exercices.md`**, relevées section par section ; *(c)* les trois trous du `00-` et les items des séances 1 à 4 déjà inscrits ; *(d)* **déposé le 03/08, séance 8 — la table de correspondance de la PAIRE pour N2** : sur un cran de diagnostic il n'y a pas de version finale, donc pas de `delta_v1_vf` ; N2 lit alors la paire (« correction, puis nouveau cas de la même famille de défauts »), **mais quel résultat de paire vaut quelle branche n'est pas tranché** *(marqué `[à valider]` au `01-routeur.md` §6)*. **Daté par C4-L2, le 25/08** : tant que ce n'est pas écrit, N2 n'est pas exécutable sur un cran de diagnostic ; *(e)* **déposé le 03/08, séance 8 — le mot « geste », jamais jugé au chapitre 1** *(objection de Louis : « type et geste n'existent plus »)*. L'axe `gestes[]` est bien mort — absorbé dans `crans[]` le 31/07 —, mais **le mot n'a aucune entrée à la table des termes** ; la ligne vient d'y être posée. **139 occurrences sur les quatre documents** ; **les 20 du `01-` ouvertes → zéro usage vivant** *(notes de renommage, descriptions datées, français ordinaire)*. **Restent à ouvrir : les 69 du `02-` et les 34 de C3.** *À rapprocher de l'autre résidu du même ordre : le nom de la table **`exercices_types`** est un reste d'avant la refonte — ses lignes sont les treize **objets** ; c'est ce qui a fait trébucher Louis sur `depot_id`* |
