...
const statutDEntree = (await admin.from('competences_statut_recette')
  .select('statut_recette').eq('competence', 'argumentation').maybeSingle()).data?.statut_recette ?? null
let ko = 0
...
try {
  ...
  dire(ouvertes.includes('argumentation'),
    `l'Argumentation est OUVERTE à la chaîne — ${ouvertes.length} au total : ${
  ...
  note(`statut de recette lu pour l'Argumentation : \`${statuts.argumentation}\` — ce script n'en `
  ...
  dire(statutDeSortie === statutDEntree,
    `LA RECETTE N'A PAS TOUCHÉ AU STATUT DE l'Argumentation — ni « pour tester » : `
