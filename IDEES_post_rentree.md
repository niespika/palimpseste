...
- **Le prompt hebdo en base ne réclame pas le signal d'intégrité — ET RIEN NE LE CONTRÔLE.**
  ⛔⛔ **Remesuré le 30/08, et cette entrée était fausse pour la production.** Elle concluait « la détection
  anti-triche par l'IA est donc muette en production ». La vérité est plus retorse, et pire :
  **bac à sable** — `fragments_config.prompt_evaluation` porte **10 064 signes** de prompt personnalisé, qui **ne
  contient pas** `signal_integrite` : la détection IA y est bel et bien **muette**.
  **production** — le même champ porte **une chaîne VIDE (0 signe)**, pas `NULL`. Or le repli est écrit pour ça :
  `config?.prompt_evaluation?.trim() ? … : PROMPT_FRAGMENT_DEFAUT` *(`utils/prompt-fragment.ts:113`, dont le
  commentaire dit « un simple `??` laisserait passer '' et enverrait un prompt vide au modèle »)*. Le **défaut
  versionné s'applique donc**, et lui **réclame** le champ *(`prompt-fragment.ts:88`)*. **La production est sauve —
  par accident**, parce que `fragments_config` n'était pas dans la liste blanche du montage de prod du 24/08.
  ⭐⭐ **LE VRAI DÉFAUT EST AILLEURS, ET IL N'EST PAS ÉCRIT : rien ne contrôle qu'un prompt personnalisé réclame
  encore les champs que le code PARSE.** `lancerAnalyse` lit `parsed.signal_integrite` ; un prof qui enregistre sa
  propre version **éteint la détection anti-triche sans le savoir, et sans qu'un écran ni un journal le dise**.
  ⚠️ L'asymétrie est parlante : `utils/prompt-fragment.test.ts:85` **affirme que le DÉFAUT contient le champ** — le
  dépôt sait donc que le champ est requis, et ne garde que la version que personne n'édite.
  ⭐ **Piste, et elle est petite** : au moment d'assembler *(ou d'enregistrer)*, comparer le prompt retenu aux clés
  que le parseur lit, et **le dire** — un avertissement à l'écran Paramètres, ou une alerte au bilan du job. La même
  question vaut pour `prompt_evaluation_essai` et `prompt_evaluation_orale`, qui ont le même repli et le même piège.
  *(constaté pendant C8·L2 ; corrigé et élargi le 30/08, en mesurant C-RLS-5.)*
...
- **2026-09-02 — `04-` §14, un reliquat qui contredit son amendement.** La l. 1542 (« AU CRAN 6, LE GUIDE EST DANS LA CONSIGNE … Les cinq patrons du cran 6 finissent tous par `<les appuis nommés>` ») contredit la l. 1522 (« LE CRAN 6 NE RÉPÈTE PLUS SES APPUIS DANS LA CONSIGNE — 29/08 ») et la table des patrons où la case a disparu. Trouvé en repérant les domiciles de la phrase du plan ; hors périmètre des fiches d'objet.

- **2026-09-02 — un lot à ouvrir : les trois dérivations du `09-Objets.md`.** *(1)* `derive-04.py` lit la ligne « ce que la fiche dit à l'élève » du `09-` et écrit la colonne cran 6 du `04-` §14.2 — la direction s'inverse, une phrase n'a plus qu'un domicile. *(2)* Les prompts de la fabrique (`guide.txt`, `appui.txt`, le retour) reçoivent la fiche de l'objet : constituants, joint, contre-exemple, « ce que la fabrique casse » — c'est ce qui manque pour tirer la position et la nature du défaut (diagnostic du 01/09). *(3)* Un module TS généré depuis le `09-` (`utils/deroule/fiches-objets.ts`) pour l'écran de l'élève et le prompt du juge, derrière un flag OFF. Conditions : le `09-` arrêté par Louis (il est `[à valider]`, exemplaires brouillons). Détail : `palimpseste-conception/RELEVE_Fiches_Objets_2026-09-02.md` §4.

- **Le statut `non_fait` d'un dépôt n'est pas au `07-` §1.1.** `c6_designation_non_fait.sql` *(28/08, les deux bases)* a ajouté une neuvième valeur au `CHECK` de `exercices_depots.statut` — *« un dépôt qui a EU LIEU et ne compte pas »* *(item 77)* —, et le §1.1 énumère toujours `assigne → ouvert → v1_remis → retour_publie → vf_remis → clos`, plus `abandonne` et `retire`. Constaté en fabriquant `C6-L4` *(02/09 ; 1 ligne `non_fait` en production)*. **Une phrase à écrire au §1.1 — section ouverte — par qui a posé la migration**, avec ce que ce statut fait à l'assiduité et au routeur.

- **2026-09-02 — C8 · Fragments transcrit ses essais DEUX FOIS, et la seconde coûte le prix des photos.** Depuis C6-L4, un essai déposé dans Fragments est transcrit par la chaîne (`transcription_v1`, deux passes, le prompt qui fait foi) **et** par `analyserEssai` (`utils/analyse-essai.ts`, un appel qui lit les photos, transcrit et juge). Deux transcriptions d'une même copie sont deux copies qui divergent, et deux fois les photos envoyées au modèle. **Mesuré le 02/09 sur une copie de 3 pages : 0,105 $ pour la transcription de la chaîne** (`api_couts`, 2 passes, `claude-sonnet-4-6`) ; le coût de l'analyse de Fragments n'est pas dans `api_couts` (il va dans `fragments_essai_depot_analyses.cout_api`, idée déjà parquée). Si le retour de Fragments doit relire `exercices_depots.transcription_v1` au lieu de refaire la sienne, c'est une décision sur le module Fragments (C8), pas un geste de C6-L4.
- **2026-09-02 — C8 · l'éligibilité à l'essai (`fragments_themes.essai_actif`) contredit la décision de Louis.** Le 02/09, Louis a tranché pour C6-L4 : *« je ne vois pas comment un élève pourrait ne pas être éligible si une classe a des fragments à faire »* — les dépôts de la chaîne naissent donc pour **toute la classe**. Mais la page de l'essai (`essais/[essaiId]/page.tsx`) et la page élève (`?vue=essai`) filtrent encore sur `essai_actif` : un élève non « activé » ne voit pas l'essai dans Fragments alors que son dépôt de chaîne existe et compte à son assiduité. **En production, 0 thème → 0 élève verrait l'essai.** À retirer ou à aligner côté Fragments.
- **2026-09-02 — C8 · deux retours pour une copie d'essai, et l'écran de l'élève le dit — mais le bloc partagé « Relis et corrige » ment pour l'essai.** La page élève de passation de Fragments rend `EcranEleve` (le même que Codex et Aletheia). Une fois la copie validée sans geste de l'élève (trois étapes, `06-` §1), le bloc « RELIS ET CORRIGE — Corrige ce qu'elle a mal lu, c'est ce texte-là qui sera lu ensuite » s'affiche en lecture seule avec ses 28 doutes : il invite à corriger une transcription déjà mesurée. Trouvé par le smoke du 02/09. ✅ **Réglé le soir même, sur décision de Louis** (« pour l'essai, l'élève ne se relit jamais ») : `EcranEleve` reçoit `sansRelecture`, posé par la page de Fragments, et rend « Ta copie, telle que la machine l'a lue » sans bouton. Reste vrai pour C8 : la page de Fragments dit « Ton professeur analysera ton essai bientôt » pendant que la chaîne, elle, a déjà transcrit.
- **2026-09-02 — C8 · la colonne « classe » du tableau de l'essai lit `profiles.classe`.** Sur la page de l'essai de la classe Test, Elo s'affiche « Elo THLP » : `TableauEssai` montre `eleve.classe` (un champ texte du profil), pas la classe de l'inscription. Sans rapport avec C6-L4, vu au smoke.
- **2026-09-02 — C4-L12 · la pose du 31/08 à 18:00 a été COUPÉE EN VOL à `maxDuration = 300`, et rien ne l'a dit.** ✅ **Réglé le 02/09 (`9db3a18`, poussé)** : file bornée par l'horloge (`utils/moteur/cadence.ts`), écritures groupées, garde `dejaServis`, minutes avant la pose, second passage du cron à 18:20. Reste à VÉRIFIER le 07/09 après 18:25 UTC. 59 élèves servis sur 62, décisions de 18:01:58 à 18:06:00 ; les trois non servies (Ana Fernandez Girard, Chloé Bonne-Longobardi, Nina Panaitescot — THLP) sont les trois dernières de l'ordre `(classe_id, eleve_id)`. Aucune trace en base : le bilan ne s'écrit nulle part, et une coupure Vercel ne passe par aucun `catch`. Deux gestes que la route annonçait déjà : **borner la boucle sur une horloge** (`budgetMs`, comme `/api/chaine`) et écrire le bilan (ou au moins la liste des non-servis) en base avant la coupure ; et **grouper les `insert`** de décisions/dépôts. En attendant, un contrôle du lundi : compter les inscrits actifs sans `routeur_decisions` au cycle. Rattrapage possible par `elevesDemandes` (écriture prod, sur autorisation).

- **2026-09-03 — Copie annotée · l'ÉLÈVE ne la voit pas encore.** La page professeur `/prof/<module>/copie/<depotId>` (flag `copie_annotee_actif`, OFF en prod) rend la copie surlignée par couches avec les observables de P1, les verdicts de P2 et les points du retour. Le même composant (`components/copie/EcranCopieAnnotee.tsx`) peut servir l'élève filtré sur le retour publié (et, plus tard, sur ce que Louis décidera de montrer de P1/P2) — décision de Louis : « flag à OFF pour le moment, je veux limiter l'accès ». À ouvrir derrière un second interrupteur, jamais derrière le même.
- **2026-09-03 — Copie annotée · les citations du `texte_support` ne se surlignent pas** : le texte d'auteur n'est pas à l'écran. Elles s'affichent dans la carte (« Le texte dit »). Si l'écran doit un jour porter les deux textes, relire [[reference_deux_textes_a_lecran]] avant.
- **2026-09-03 — Copie annotée · l'édition du retour reste sur la page de passation**, à la ligne du nom on ne l'a plus quand le flag est ON (la ligne devient un nom + l'état). Si Louis veut éditer les points depuis la copie annotée, `EditionDuRetour` (`EcranProf.tsx`) se déplace, avec `actionEditerLeRetour`.
- **2026-09-03 — Copie annotée · Expression cite deux fois le même mot** (« En effet » à deux phrases) : le placement prend l'occurrence SUIVANTE de la même citation dans la même famille ; mais deux familles (Expression et Argumentation) qui citent le même « En effet » se posent toutes deux sur la PREMIÈRE. Le numéro de phrase de P1 (`phrase`) permettrait de viser la bonne occurrence — à faire si ça gêne à l'usage.
- **2026-09-03 — Fragments élève · deux définitions du « dernier retour à lire »** : la page (`derniereAnalyseEcrite` = analyse de la semaine ouverte, sinon la plus récente par dépôt) et `utils/retours-lus.ts` (N'IMPORTE quelle analyse publiée non lue) peuvent diverger si un retour ANCIEN reste non lu : la carte « à faire » dirait « à jour » pendant que le bloc du dépôt dirait « bloqué — Fragments, retour écrit » en pointant vers la même page. Vu en préparant le décor du handoff (aucun cas réel : 0 analyse en prod). À aligner sur une seule règle (la plus ancienne non lue, ou toutes).
- **2026-09-03 — Fragments élève · sur téléphone (< 640 px), rien ne porte l'identité du monde** (pas de sceau, pas de nom : le bandeau mobile dit PALIMPSESTE + la classe, puis les onglets). Le handoff Fragments élève le voulait ; Louis a tranché « la convention des autres modules ». Si la convention change un jour, c'est dans `app/eleve/layout.tsx` (bandeau `sm:hidden`), pour tous les modules à la fois.
- **Instruments dérivés qui DIVERGENT de leurs sources (03/09, repéré pendant Aletheia E1, hors périmètre).** `npm test` rougit sur « les dérivés sont IDENTIQUES à leurs sources » : `python3 scripts/derive-instruments.py --verifie` dit `DIVERGE : MANIFESTE.ts` et `DIVERGE : calame-retour.ts` — les sources du dépôt de conception ont bougé sans que `--ecris` soit rejoué (ou l'inverse). Aucun fichier d'Aletheia E1 n'y touche ; 2 205 autres tests verts. À rejouer par la séance qui porte la chaîne, pas par celle-ci. *(repéré le 03/09)*
- **Aletheia → Quazian : les cartes de vocabulaire n'ont JAMAIS été créées (03/09, constat E5, hors périmètre du chantier étayage).** `creerCartesVocabulaire` insérait `type = 'vocabulaire'`, que la contrainte `quazian_flashcards_type_check` (philosophe | concept | mouvement | these) refuse : erreur 23514 journalisée à chaque retour V1, `0` carte `source = 'aletheia'` en prod comme en bac à sable. Corrigé sur la branche `feat/aletheia-etayage` en insérant `type = 'concept'` (la provenance reste `source = 'aletheia'`). Piste propre : un type `vocabulaire` dans la contrainte ET dans les libellés de Quazian (`CarteFlashcard.tsx`, page cible), avec un rendu distinct. *(repéré le 03/09)*
- **2026-09-04 — Surlignage · 23 consignes de prod aux crans 4·7·9 disent « deux » (deux idées, deux passages, les deux phrases…) alors que l'écran ne garde QU'UNE zone** : un élève a signalé « je n'arrive pas à souligner deux fois d'affilée » (cran 4 `paragraphe.restituer.apports`, 10 poses en 4 min). L'écran dit désormais « pour deux endroits, surligne d'un seul trait du premier au second », mais la doctrine (`02-` §5, une zone par cas) et ces consignes divergent : soit récrire ces consignes (trois domiciles), soit admettre plusieurs zones (verdict par union d'intervalles). Liste : `ex-paragraphe-restituer-04-apports-1`, `-operation-1-03`, `ex-argument-composer-04-densite-generique-1`, `ex-argument-expliquer-04-implicite-explicite-1`, `ex-objection-composer-04-jointure-presente-1`, `ex-phrase-restituer-04-notions-en-tension-1`, et 17 où « deux » n'est qu'une tournure.
- **2026-09-04 — Paire · la chaîne ne mesure que la réponse au PREMIER cas** (`texte_v1`) ; la seconde est désormais ÉCRITE (`texte_vf`, jamais avant ce jour : 0 sur 28 paires en prod) et montrée au retour, mais ni la chaîne ni Calame ne la lisent — le juge du cran (C7-L1) lit « la dernière version ». À trancher avec C7-L5 : que mesure-t-on d'une paire ?
- **2026-09-04 — Déroulé · « Passer au second cas » est un état d'écran, pas une donnée** : recharger la page ramène à la correction du premier cas tant que le second n'a rien écrit. Acceptable (la correction est courte) ; à persister si des élèves s'en plaignent.
- **2026-09-05 — Gabarit · le `10-` (décision 18, §8) doit porter les trois règles que la relecture de la vague 2 a fixées** : deux cas jamais à la même phrase, second cas jamais tourné comme le premier, aucune version du 3 qui recopie le passage. Elles vivent au prompt et au contrôle de `noyau/gabarit.py` ; le 10 n'a pas été édité parce que son empreinte est dans la doctrine dérivée (`derive-doctrine.py`, `SOURCES_GABARIT`) : l'éditer oblige à rejouer `--sql` + `--fixture` des deux côtés — à faire après la bascule, pas dimanche.
- **2026-09-05 — Gabarit · les contrôles d'import (py + TS) ne signalent pas une version du 3 identique au passage du matériau** ; la règle n'est que dans la fabrique. Si un fichier 1.5 vient d'ailleurs que la fabrique, ajouter le signalement des DEUX côtés (le miroir copie le tokeniseur).
- **2026-09-04 (soir) — Déroulé « un écran, une tâche » · le texte est GELÉ une fois la crédence donnée** : la crédence porte sur « celle que tu viens d'écrire » ; avant la refonte, le champ restait modifiable jusqu'à la remise. Si un élève se plaint de ne plus pouvoir corriger une coquille après sa crédence, c'est ce choix à rouvrir — pas un défaut.
- **2026-09-04 (soir) — Déroulé · la version finale et le retour final (régime plein, crans 2·6·8) ont leurs pages** (écrire, rendre ; « ce que tu as à reprendre » sur la dernière page du retour, après la validation de lecture) **mais n'ont été joués que sur un ancien dépôt de test au cran 8** : les crans 2·6·8 ne sont pas au gabarit et le décor n'en sert pas. À éprouver avant que le régime plein serve à nouveau.
- **2026-09-04 (soir) — Recette · deux `parcours-deroule.mjs` ne se jouent JAMAIS en parallèle sur le même compte** : un lien magique en annule un autre (4 parcours sur 8 ont atterri sur `/login`). Et tuer Chrome juste après la remise a laissé un job `mesure_v1` en `en_attente` à 0 tentative — le `after()` du serveur de dev n'a pas traité la file : garder le client vivant jusqu'au retour.
- **2026-09-04 (soir) — Chaîne · « retour non écrit » sur trois remises de cran 5 en bac à sable (deux `argument.garant.absent`, un `argument.garant.connecteur`, dépôts assignés par décor sans décision de routeur), là où la même remise le matin avait produit un retour de trois points.** Le motif est PERDU : `motifDuRetourManquant` ne garde que les alertes qui contiennent « retour » et tronque à 400 caractères — il ne reste que « cible INDÉTERMINÉE » (un bruit) et le début du garde-fou de Structure. Les entrées (texte, crédence, gestes, télémétrie) sont identiques d'un relevé à l'autre. À rejouer avec le bilan complet (`rejeu-retour.mjs`) pour lire la vraie raison.
- **2026-09-04 (soir) — Lint · `Attente` (`EcranDeroule.tsx`) appelle `setState` dans un `useEffect`** (réparation du 04/09 midi, `react-hooks/set-state-in-effect`) : c'est l'erreur eslint qui reste sur ce fichier, antérieure à la refonte du soir. Un `key` sur l'attente ferait le même travail sans l'effet.
- **2026-09-05 — Déroulé · le 4(b) SANS ÉCRITURE (Louis, sur la galerie : « le second cas est toujours un 4b, donc il n'y a pas d'écriture ; l'espace d'écriture peut être enlevé »).** La doctrine l'acte (`10-` §2 : 4(b) « sélectionne le passage », jugement « algorithmique (la zone) »), la console C7-L3 sert encore « Ce que tu en dis ». Retirer le champ demande : l'étape de la paire lue sur la DÉSIGNATION pour un cas 4(b) (`etapeDeLaPaire`, `vue.ts`), une remise sans texte quand tous les cas sont 4(b) (`remettre` refuse « Ta copie est vide »), la clôture algorithmique comme aux crans 1/3 (pas de chaîne, correction servie, registre C7-L5 lu sur la zone), et la chaîne qui ne mesure que le 4(a) d'une paire 4(a)/4(b). Un lot, pas un correctif d'écran — à ouvrir avec C7-L3/C7-L5.
- **2026-09-05 — Déroulé · la confiance de remise est calibrée sur UNE compétence** (cible retenue → cible primaire → compétence de l'observable isolé) : les **cibles secondaires** du `01-` §1 (« elles reçoivent un retour, comme la primaire ») n'ont aucun domicile en base — `routeur_decisions` ne porte que `cible_retenue` et `sondes_retenues`. Le jour où le routeur les écrit, `competencesVisees` (`vue.ts`) doit les lire.
- **2026-09-05 — Déroulé · la restitution à chaud n'est plus demandée qu'au geste `produire`** (crans 2·6·8), sur décision de Louis (« l'élève ne produit aucune thèse ») : le `06-` §3 dit encore « trois gestes courts » à toute remise — à amender dans la source. La garde de `remettre` suit (`restitutionDemandee`).
- **2026-09-05 — Déroulé · « montrer la comparaison et l'explication » à la correction du premier cas d'une RÉDACTION** (crans 4(a), 9) : l'écran met désormais la réponse de l'élève et ce qu'on tient pour vrai côte à côte, avec le pourquoi ; une explication de l'ÉCART demanderait un appel au juge du cran (C7-L1) AVANT la remise — aujourd'hui il ne tourne qu'à la remise, sur la v1. À trancher : un appel de plus au premier cas, ou la comparaison seule.

- ~~RAYÉE le 07/09/2026~~ *(séance d'arbitrage, Louis : dès `C7-L9`, P2 ne tourne plus aux crans 1·2·3·4·5·7·9 du routeur — restreindre P2 n'a plus d'objet ; aux 6·8 la forme (a) reste)* — **2026-09-07 — Chaîne · C7-L8, « juste l'observable », forme (b) : restreindre P2 à l'observable de la clé** *(Louis, 06/09 au soir : (a) pour ce lot — un seul P1 et un seul P2 sur la compétence de la clé, la mesure ne garde que l'observable de la clé, les autres codes à `n/a` ; (b) « à terme »)*. (b) est un prompt neuf, donc un instrument neuf, donc les fiches (`derive-instruments.py`, C4-L10) : hors du lot. Ce que (a) coûte encore par rapport à (b) : un P2 qui juge des observables qu'on jette — des jetons de sortie, pas un appel ; et une `lettre_equivalente` rendue par `code2` sur TOUS les observables jugés, mesurée le 07/09 sur un décor au cran 5 (voir `RELEVE_C7_L8_2026-09-07.md`, piège 11). ⚠️ Le prompt de C7-L8 disait cette ligne écrite le 06/09 : elle ne l'était sur aucune des deux branches — écrite le 07/09 par la séance.
- **2026-09-07 — Déroulé élève · la phrase qui explique POURQUOI valider sa lecture est fausse pour la MAISON** *(trouvé au smoke de `C10-L1`)* : `components/deroule/RetourSegmente.tsx`, bloc `ValidationDeLecture`, affiche *« Il te reste à valider ta lecture. **Tant que tu ne l'as pas fait, tu ne peux rien rendre d'autre — c'est la seule chose qu'elle bloque.** »*. ⛔ **Mesuré : c'est faux à la maison.** La source de `messageSiRetoursNonLus` qui lit `exercices_retours` filtre `exercices.lieu = 'classe'` (`utils/retours-lus.ts:128`) — c'est la garde de `C6-L4`, pour les essais de Fragments : **un retour de maison non lu ne bloque aucun rendu aujourd'hui.** ⭐ `C10-L1` a corrigé la phrase **pour le seul écran fermé** (prop `fermee` : « c'est le dernier geste de cet exercice : une fois ton retour lu, le bilan de la semaine s'ouvre »), parce que sur une semaine comptée elle est fausse deux fois. **Le cas non fermé reste à trancher** : soit la phrase dit la vraie raison (le bilan, `06-` §2 temps 6), soit le blocage est étendu à la maison — et c'est une décision, pas une correction de texte.
- **2026-09-07 — Calame · une personnalité unifiée à travers les modules** *(Louis, séance d'arbitrage `C7-L9`)* : Palimpseste, Vestigia, Aletheia, Codex, Scriptorium et Quazian parlent tous de la voix de Calame *(`07-` §4 : « un seul fichier de personnalité »)*, mais chaque prompt porte aujourd'hui son propre ton. À terme : **un paragraphe de personnalité propre à Calame, ajouté à chaque prompt** depuis le fichier unique, jamais recopié. D'autant plus utile que le juge et Calame restent deux appels (`07-` §4 bis) — la personnalité ne va qu'à celui qui parle à l'élève.
- **2026-09-07 — Chaîne · C7-L9, la table du `03-` §1 ne couvre pas un COMPTAGE dont le seuil vaut 1 ou plus** *(mesuré sur les instruments dérivés : `synthese|contresens_partiel`, comptage rapporté `au_plus` au seuil 2 par paramètre, isolé par 4 clés du `09-`)* : le « 1 » que la table donne au raté reste SOUS le seuil et se relirait « réussi ». Le code n'écrit jamais cette valeur : l'auto-vérification de `valeurDuVerdict` rend `n/a` et l'alerte le nomme — la mesure de cet observable ne s'écrit donc pas quand le verdict est raté. ✅ *Tranché le 08/09 (Louis) : le raté d'un comptage vaut le seuil doublé, 1 au minimum — appliqué dans le code.* Le `03-` §1 est amendé en v2.3 (08/09, accord de Louis). ~~Rien ne reste.~~
- **2026-09-07 — Routeur · C7-L9, aucun exercice 6/8 n'est SERVABLE en bac à sable** : le gabarit 1.5 n'a pas de cran 6 ni 8, et les 18 exercices 6/8 de la banque 1.4 qui portent l'Argumentation sont écartés du vivier `cours_par_notions_non_lu` (rattachement par notions, « la couche 4 ne le lit pas encore » — C4-L12). Le signal de trajectoire se lève et se journalise `sans_substrat` ; la sonde ne peut se poser que le jour où un 6/8 est servable — la fabrique du gabarit (C7-L4, « les crans 2, 6, 8 ») ou la troisième voie de C4-L12.
- **2026-09-07 — Monitoring · sur un cran qui isole, la calibration est `indetermine` par construction** *(C7-L9, piège 13 : l'instrument ne rend aucun niveau)* : le verdict du cran (réussi / raté) pourrait calibrer la confiance de remise là où la lettre ne le peut plus — une conversion verdict → « niveau » à écrire dans la fiche du Monitoring, jamais dans le code en passant.
- **2026-09-06 — Gabarit · le cran 2 du PLAN demande un écran à concevoir** : son trou est un ORDRE, pas un morceau de texte — « il faudra indiquer où vont les trous à combler, et permettre de faire glisser les phrases d'un endroit à l'autre » (Louis, relecture des pièces du 06/09). D'ici là le plan n'a pas de cran 2 servable (les 2 exercices dérivés sont retirés du fichier `gabarit-c2.json`) ; le geste du `09-` §7 le dit. À dessiner avec la règle du handoff (mesurer les thèses réelles : 70 à 148 caractères, trois par plan).
- **2026-09-06 — Gabarit · les problèmes GLOBAUX du `09-` (le défaut est le texte entier : plan dont I et II sont permutables, introduction sans promesse — paragraphe 3, plan 5, argument 1, partie 2 ; « pas dans cette vague ») restent hors fabrique.** Ils passent par le 1(b), le 3, le 5, le 7 et le 9 où « tout le texte » est la réponse (`10-` §1). Louis, 06/09 : « il y a des choses plus pressantes ». À rouvrir quand la fabrique par devoirs tient ; la liste est dans `RAPPORT_Refonte_routeur_cle_2026-09-05.md` §2 (dépôt conception).
- **2026-09-05 — Pilotage classe · revoir le calcul de l'élève « à risque ».** Demande de Louis. Aujourd'hui `motifsSante` (`utils/sante.ts`) ne lit que Fragments (manquants, moyenne sous seuil) et le retard de révision Quazian (≥ 15 cartes dues) ; ni la semaine Scriptorium (exercices maison faits/imposés, désormais comptée par la matrice), ni Aletheia, ni Codex n'y pèsent. Le drapeau et le tri « à risque » de `/prof/classes/<id>` en dépendent.
- ~~**2026-09-05 — Pilotage classe · la colonne « Élève » figée (`sticky left-0 z-10`) passe PAR-DESSUS l'en-tête du site quand la page défile** (vu au smoke prod dans Chrome à 1451 px : les noms « Maëlla Pierrette, Maïlys… » se dessinent sur la barre Tableau de bord / Pilotage). Antérieur à ce lot ; l'en-tête doit porter un `z-index` supérieur à celui de la cellule figée, ou la colonne perdre son `z-10` hors du cadre du tableau.~~ ➜ ✅ **FAIT le 05/09 (`bd6a9a7`)** : l'en-tête du site passe à `z-20` (`components/nav/EnTeteSite.tsx`), vérifié en prod sur 1HLP défilée.

## C10-L2 — cinq trouvailles hors périmètre (07/09/2026)

*Déposées par la session Code `C10-L2` (« le professeur clôt les dépôts d'une passation »). Aucune n'a de destinataire vivant au plan : les lots visés sont joués, ou la question est de doctrine.*

**1. ⛔ UN PORTIER POUR LA PASSATION EN CLASSE — c'est une REFONTE, pas un correctif.**
`app/deroule/actions.ts` garde **18 de ses 20 actions** en UN point : `portier(depotId, ecriture)`. Côté classe, `app/passation/actions.ts` porte **11 actions élève, chacune avec sa propre `garderEleve(false)`**, et `utils/passation/garde.ts` *(62 lignes)* n'ouvre **jamais** `exercices_depots` — il ne lit que le rôle et les deux portes. `C10-L2` a donc posé son prédicat `depotClos` **à onze endroits** plutôt que d'écrire le portier : la refonte n'était pas son lot, et une garde recopiée onze fois est une garde qu'on peut oublier au douzième écran.

**2. ⛔ LA TROISIÈME SURFACE ÉLÈVE — Fragments-érudition INTERPOLE le libellé dans une phrase.**
`app/eleve/modules/fragments-erudition/page.tsx:749-761` rend les retours de chaîne **à la main**, sans passer par le composant partagé `MesExamensPasses`, et écrit :
> `{r.etat.libelle} · ton essai a deux retours : celui de Fragments ci-dessous, et celui-ci (trois compétences).`
Après une clôture, l'élève lirait donc *« **abandonné** · ton essai a deux retours… »* — **une promesse de deux retours sur une copie qu'on vient de déclarer jamais rendue.** ⛔ **Écarté explicitement par `C10-L2`** : population **0 instance Fragments de classe en production**, et corriger la phrase est une décision de contenu, pas une réparation. *Condition d'ouverture : la première passation en classe de Fragments-érudition.*

**3. ⚠️ TROIS IMMUNITÉS DU JOURNAL TIENNENT À UNE VALEUR NULLE, PAS À UNE INTENTION.**
`C10-L2` a inventorié les **18 sites de lecture** de `routeur_decisions` pour savoir lesquels une ligne d'`override_prof` perturberait. Deux étaient atteints et sont réparés. Mais **trois des seize « immunisés » ne le sont que par accident** :
- `historiqueDesCibles` *(`utils/routeur/profil.ts`)* survit parce qu'il filtre sur la **cible** *(`cible_retenue` est NULL sur un override)*, pas parce que quelqu'un a pensé aux overrides ;
- `porte-serveur.ts:70` lit **toutes** les décisions d'un élève sans le moindre filtre, et n'est sauvé que par `if (!s.some((x) => x?.sonde_montee === true)) continue` ;
- `compterLesCiblages` *(`utils/deroule/vue.ts`)* n'est sauvé que par `.eq('cible_retenue', cible)`.
**Elles cassent le jour où quelqu'un remplit `cible_retenue` ou `sondes_retenues` sur une ligne d'override.** Un test le dit désormais *(`utils/routeur/profil.test.ts`)*, mais la vraie réparation serait un discriminant explicite au lecteur, pas une valeur nulle qui se trouve là.

**4. ⚠️ LA LISTE DES STATUTS RENDUS EXISTE EN QUATRE EXEMPLAIRES, dont trois copiés à la main.**
`STATUTS_RENDUS` vit à `utils/routeur/assiduite.ts` — et la même liste est recopiée à `utils/matrice-pilotage.ts` *(un `new Set([...])`)*, à `utils/registre/reussites-serveur.ts` *(`STATUTS_JUGES`)* et **en clair** à `app/prof/signalements/PanneauExercice.tsx` *(`const horsComptage = statutDepot === 'retire'`)* — dont le commentaire, deux lignes plus haut, dit pourtant *« `entreAuDenominateur(statut)` est la règle, et on ne la recopie pas »*. ⭐ **Aucune ne connaît `abandonne`**, donc l'indiscernabilité `ouvert` / `abandonne` sur laquelle repose `C10-L2` est vraie **partout** aujourd'hui — mais elle tient par quatre accords, pas par une règle.
...
- **08/09 — Aperçu professeur du gabarit :** sur les deux exercices réels `ex-gab-transition-bilan-theme-c5` et `ex-gab-objection-accord-non-retourne-c5`, l’aperçu omet la consigne composée du gabarit (captures avant/après du chantier ②), car il ne lit que `consigne_instanciee`. Alignement du gras réparé ; composition de cette consigne hors périmètre, à traiter séparément.
- **08/09 — Paramètres Scriptorium sur téléphone :** un ancien élément `code`, hors du bloc « Retours à relire », déborde jusqu’à 377,23 px pour une fenêtre de 375 px ; le nouveau bloc reste entre 16 et 359 px. Débordement hors périmètre, non corrigé.
