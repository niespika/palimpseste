...
- **Le diagnostic Quazian (écran « fragilités ») est doublement muet — à reprendre en C6.** `app/prof/quazian/diagnostic/actions.ts:98` demande lui aussi `type='unite'` (zéro ligne), et il agrège les réponses **par `quazian_quizzes.scope_unites`** — colonne que les quiz créés depuis C7·L1 laissent vide au profit de `scope_contenus`. L'écran ne montrera donc rien, même une fois des quiz passés. Le prompt de session range explicitement cet écran en C6 (« le diagnostic fondu dans la matrice ») : la note est ici pour que C6 sache qu'il y a deux fils à reprendre, pas un. *(constaté pendant C7·L1, renvoyé à C6)*
