...
## Codex — l'épreuve à sujet libre, projetée, minutée (24-25/09/2026 ; tout POUSSÉ jusqu'à `760683d` et déployé ; SQL joué en PROD le 25/09 ; porte OUVERTE en prod par Louis ; smoke prod fait dans la Classe Test)

Demande de Louis du 24/09 pour l'examen 1HLP du **mardi 29/09** (ligne de plan `1c00880c…`, à concevoir). Quatre décisions prises une à une : deux textes (sujet + consignes de travail → élève et juge ; consignes pratiques → projetées et tablette, jamais au juge) ; **dépôt automatique à la moitié de la rédaction**, relecture de Y minutes, confirmation avant validation ; tablette qui montre le sujet dès le lancement et passe seule au dépôt ; `exercices_actif` (OFF en prod exprès) ne ferme plus la passation en classe. Détail et arbitrages : `REPRISE_Codex_epreuve_minutee.md`.

- **Mesuré avant code (prod, lecture PostgREST)** : 127 sujets au corpus, **0** ne parle de Socrate ou de Chihiro ; énoncés 25 / 54 / 122 car. (min, médiane, max) ; consignes d'examen 88-96 car. ; `duree_estimee_min` nulle sur 28 lignes sur 28 ; 1HLP : 25 élèves, 0 exempté ; `exercices_actif` **false**, `passation_classe_actif` true ; 0 dépôt de classe `assigne`/`ouvert`, 0 transcription en file (le découplage ne réveille rien) ; fuseau `America/Toronto`. Le handoff du 29/08 était faux contre la base (2 doutes montrés vs médiane 7,5 / max 24 ; points 67-101 car. vs médiane 403 ; pastilles de compétence interdites RR4 ; « Pour la prochaine fois » : 0 retour sur 268).
- **Fait** : sujet libre à la conception (Codex, porte ouverte) ; consignes pratiques ; durées ; page projetée `/prof/codex/passation/[id]/projection` (facture du tirage de Vestigia, ajustement mesuré de la taille du sujet pour tenir en 720 px) ; lancement, « +5 min », annuler, ouvrir maintenant ; **ouverture paresseuse** par `ouvrirLesDepots` inchangé, heure posée au lancement (`epreuve_ouverture_at`) ; tablette « épreuve en cours » + sondage ; dépôt, attente de la transcription, relecture avec la photo (URL signées bornées au préfixe du dépôt), confirmation — **derrière la porte, Codex seulement** (Aletheia et Fragments gardent l'écran d'hier à l'octet) ; interrupteur dans Codex → Paramètres ; découplage d'`exercices_actif`.
- **Passe adversariale** (5 regards, une phase, 31 constats triés à la main) : corrigés — **photos d'un autre élève signables** par un chemin forgé (refus à l'écriture et au signement) ; envoi en échec figé sur l'attente ; heure d'ouverture recalculée par « +5 min » (désormais posée au lancement, arrondie à la minute) ; « dépôt ouvert » lu dans le FAIT ; relecture vide ⇒ « Temps écoulé » sans relecture (durée exigée, texte de fin propre) ; « +5 » après la fin ; onglet élève qui se relisait des jours avant (désormais le jour prévu seulement) ; porte de la passation ignorée par l'épreuve ; durées remises à zéro après un refus (champs contrôlés) ; sujet libre réduit au titre ; renvoi de photos jamais relu (`remettreEnFile`) ; consignes pratiques invisibles sans durée ; CRLF des consignes pratiques (mesuré : 124 car. en base pour 122 saisis) ; refonte sans porte (remise derrière la porte) ; textes prof contradictoires.
- **Contrôles** : `tsc` propre, eslint des fichiers du lot propre, **2 860 tests verts** (23 nouveaux, `utils/examens/epreuve.test.ts`).
- **Vu à l'écran au BAC À SABLE** (décor marqué : ligne de plan `note = 'recette-epreuve-minutee-2026-09-24'`, classe Test, instance `f2e1f587…`, 7 dépôts) : conception en sujet libre ; assignation ; projection avant lancement, en rédaction, dépôt ouvert, relecture, fin, « +5 » après la fin — à **1280×720/777, 820 et 390** sans débordement ; tablette 390 : avant le lancement, épreuve en cours, **passage seul au dépôt à 11 h 11** (sondage), dépôt, attente, relecture (bascule photo), confirmation (bouton grisé jusqu'aux trois cases), validation, « se juger » ; relecture à 1280 en deux colonnes. **Ouverture automatique mesurée en base** : les 7 dépôts ouverts 12 s après l'heure (1re épreuve) ; 2 vraies transcriptions (bac à sable).
- **Commentaire de Louis sur la planche des maquettes (24/09)** : « si l'épreuve est en cours, le sujet doit toujours être affiché. Pas la peine d'avoir un écran qui dit "appuie pour lire le sujet" ». Fait : l'onglet Examens montre le sujet entier et les consignes pratiques dès le lancement et jusqu'à la validation de la copie ; le dépôt ouvert, un bouton « Photographier ma copie → » ; le signal de lancement de cette épreuve se tait. Vu au bac à sable à 390 px (classe Test, dépôt ouvert non validé). Texte neuf : `TEXTES_EPREUVE.eleveDepotOuvert`. ⚠️ **Dépassé par l'entrée suivante** (le bloc et ce texte sont retirés).
- **Quatre commentaires suivants sur la planche (24/09, 17 h 10 à 17 h 13) — faits, vus au bac à sable, planche version 4** : (1) « si le dépôt est ouvert, […] on doit directement voir le bouton » → **l'onglet Examens EST l'écran de l'épreuve active** (`EcranEleve` chargé par `chargerVueEleve`, le même que la page du dépôt), **quelle que soit la classe choisie dans l'en-tête** (vu : un élève resté sur T5 ne voyait que le signal) ; une copie validée y reste jusqu'à fin de relecture + 30 min ; `EpreuvesEnCours.tsx` supprimé, `VeilleDesEpreuves.tsx` ne fait plus que relire l'onglet le jour prévu, avant le lancement. (2) « le bouton prendre la photo […] la première chose qu'on voit ; […] un toggle pour passer du sujet au dépôt » → bascule « Déposer ma copie | Le sujet » (« Mon dépôt » pendant la lecture), « Prendre une photo » en tête ; le sujet, les consignes et le message de la dernière fois vivent derrière « Le sujet » (et, avant l'ouverture, sur l'écran de l'épreuve en cours, où le message de la dernière fois manquait). (3) « naviguer facilement entre sa copie et le texte retranscrit. Un toggle » → « Texte retranscrit | Ma copie | Le sujet » en étroit, « Ma copie et le texte | Le sujet » en large (`@3xl`), **collée sous l'en-tête** (mesuré : 65 px sous `sm`, 194,5 px au-dessus, à 375, 820 et 1280). ⚠️ La réponse automatique annonçait deux onglets et une position de défilement conservée : ce sont trois onglets, et chaque volet garde sa propre position. (4) « Chacune de mes idées est justifiée » retiré de la confirmation (deux cases). En passant : « Après validation, ton texte ne bouge plus » s'écrasait à un mot par ligne à 375 px (`min-w-48`). `tsc`, eslint, **2 860 tests verts**.
- **Cinquième commentaire (« Après la validation » : « l'élève devrait voir clairement les étapes de la vérification ; s'inspirer des exercices ») — proposition dessinée (planche v5), ACCEPTÉE par Louis en séance (« c'est pas mal du tout » ; il fera une passe de design ensuite), CODÉE le 24/09** : fil d'étapes Déposer · Relire · Te juger · Ta confiance (`utils/examens/etapes-epreuve.ts`, pur, 10 tests), puis une tâche par écran après la validation (« Te juger » 3/4, « Ta confiance » 4/4, « Ta copie est remise »), le texte validé à un appui (« Mon texte »). La page tourne sur des FAITS serveur : `juger_fin_at`, `confiance_declaree`, la crédence en base (jamais servie pour l'essai, sans cran). Mesuré en prod : les 4 examens en classe ont `optin_se_juger` ET `optin_confiance_remise` vrais. Écarts avec la planche : la question de confiance est celle que Louis a récrite pour les exercices (05/09) ; pastilles comme dans les exercices ; « Mon texte » après la validation. Planche v7.
- **Passe adversariale de ce tour (4 regards, une phase, 24 constats, triés à la main)** — corrigés : la sonde de l'épreuve **rechargeait la page toutes les 15 s** pendant toute la rédaction (`…+00:00` contre `….000Z` comparés comme chaînes) ; la grâce ne courait que depuis la fin de la relecture (une validation tardive faisait disparaître « se juger ») → aussi depuis la validation ; épreuve affichée même hors de la classe choisie ET au-dessus de l'écran-seuil (« Toutes ») ; seulement les classes où l'élève est **inscrit** (un élève sorti de la classe voyait le sujet) ; « à venir » compté pour un dépôt déjà ouvert à la main (onglet relu toute la journée) ; veille coupée dès qu'une épreuve était affichée ; ordre stable ; id de confirmation unique ; URL signées 1 h → 8 h ; renvoi après échec sans écran d'attente, et échec jamais relu ; volet « Le sujet » qui survivait à l'arrivée de la transcription ; bascule collée à 65 px codés en dur (49 px pour un élève d'une classe) → **mesurée** ; collante seulement si l'écran fait ≥ 640 px de haut ; changer de volet ou de page **ramène sous l'en-tête** ; focus sur le titre de la confirmation, sur « Valider ma copie » au retour, sur le titre de chaque nouvelle page ; `role=tablist` sans panneau → `aria-pressed` ; photo tournée d'un quart de tour **coupée** → mise en page sur ses dimensions tournées ; `juger_fin_at` écrit sans contrôle ; examen en double dans « Mes examens passés » ; texte du signal de lancement changé hors porte (« photographier » → « déposer », Aletheia et l'élève exempté). Écartés : 2 élèves dans deux classes en prod, aucun en 1HLP (« Mes examens passés » reste filtré par classe — ligne `IDEES`) ; l'élève exempté pendant l'épreuve (0 en prod, déjà en `IDEES`).
- **Vu au BAC À SABLE (24/09 soir), Chrome sans fenêtre par CDP (volet caché)** : élève 2 (deux classes, resté sur T5) dans l'onglet Examens : relecture avec le fil, bandeau mesuré 65 → 49 px quand le sélecteur disparaît et la bascule suit, descendu en bas puis « Ma copie » → ramené sous l'en-tête (défilement 778 → 152), photo tournée de 90° entière (341 × 256 dans 343), confirmation (focus sur son titre, retour sur « Valider ma copie »), validation → « Te juger » 3/4 (focus sur le titre), → « Ta confiance » 4/4, → « Ta copie est remise », « Revoir mon texte » ; aucun débordement à 375, 820, 1280. En base : `juger_fin_at`, `confiance_declaree` et la ligne de métacognition écrits. `tsc`, eslint, **2 870 tests verts**. ~~⚠️ Décor : l'épreuve du bac à sable a été **recalée** (lancement 24/09 ~18 h UTC) et le dépôt de l'élève 2 remis à « relu, non validé » puis revalidé ; sa page 1 porte une rotation de 90°.~~ *(décor retiré le 25/09)*
- ☐ **Louis relit les textes élève** (`TEXTES_EPREUVE` et `CONFIRMATION_AVANT_VALIDATION` dans `utils/examens/epreuve.ts`, les trois consignes de relecture dans `EcranEleve.tsx` — dont « Tu peux ajouter du texte… C'est permis », repris du handoff) — sur la planche « Épreuve minutée — les écrans et leurs textes » (artefact privé de Louis).
- **3ᵉ passe adversariale, APRÈS le push (24/09 soir, 3 regards : ce qui vit en prod porte absente, les correctifs client, les correctifs serveur) — `7effd82`** : ⛔ **actif en prod dès le push** : `actionCredence` acceptait un dépôt de MAISON depuis le découplage d'`exercices_actif` (requête forgée : crédence d'un exercice de maison réécrite en bloc) → le lieu est vérifié ; mesuré en prod, porte absente : aucune requête ne lit les colonnes neuves (`42703` évités), conception sur corpus inchangée, écran d'hier identique pour Aletheia et Fragments. Pour mardi : une panne de réseau pendant « Enregistrer », la validation, « se juger » ou la confiance faisait TOMBER la page (aucune limite d'erreur dans `app/`) et perdre les corrections → l'échec devient un message ; formulaires soumis sans la réinitialisation de React 19 ; l'onglet ne veillait que si des durées étaient fixées AVANT le rendu (durées fixées au dernier moment → tablettes vides après « Lancer ») → sonde légère armée porte ouverte, rechargement seulement au changement ; signal masqué seulement pour un écran rendu ; photo tournée mesurée même chargée avant l'hydratation, URL ratée annoncée. **Vu au bac à sable** (Chrome sans fenêtre, port vérifié libre) : épreuve apparue 16,7 s après le lancement, durées fixées au dernier moment ; hors ligne pendant « Enregistrer », la validation et « se juger » : page debout, texte gardé, envoi réussi une fois en ligne (l'ajout hors ligne est en base) ; photo tournée après rechargement. `tsc`, eslint, 2 870 tests verts. ~~⚠️ Décor : l'épreuve du bac à sable est relancée (25/09 03 h 46 UTC) et le dépôt de l'élève 2 remis à « relu, non validé ».~~ *(décor retiré le 25/09)*
- **SMOKE PROD (25/09, déploiement de `760683d` vérifié « success » avant)** — ⓐ *sans SQL, porte absente* (session prof de Louis dans son Chrome ; Élo Dupuis, Classe Test, connectée par Louis dans le navigateur intégré) : Paramètres de Codex (« L'épreuve minutée est fermée »), conception du 29/09 inchangée (corpus seul), écran de passation d'un examen existant, page projetée (« fermée… Codex → Paramètres ») ; élève : Codex → Examens (« Rien en classe »), Codex → Exercices (« pas encore ouverts » : la maison reste fermée malgré le découplage), Aletheia → Examens — tout normal. ⓑ `codex_epreuve_minutee.sql` joué en prod sur le « go » de Louis (voir `SUIVI_SQL.md`), **porte ouverte par Louis**. ⓒ *épreuve réelle dans la Classe Test* : ligne « examen diagnostique — écriture » ajoutée au plan (semaine du 21/09) → conçue en **sujet libre** (5 min + 2 min, consignes pratiques : 65 car. **sans CR** en base) → assignée (1 dépôt, Élo) → page projetée : heure de Toronto, « Rédaction 5 min », lancée à 07 h 40 → « Le dépôt ouvrira à 07 h 44 » → **ouvert tout seul à 11:44:09,6 UTC** (9,6 s après l'heure) → Élo : sujet avant l'ouverture, puis fil + « Déposer ma copie | Le sujet » + « Prendre une photo » d'abord → une page d'essai déposée → **transcription réelle en ~20 s, fidèle, 0 doute** → relecture (photo signée affichée), paragraphes corrigés, « Enregistré. », confirmation à deux cases (grisé avant), validation → « Te juger » 3/4 (focus sur le titre) → « Ta confiance » 4/4 → « Ta copie est remise ». En base : `v1_remis`, texte corrigé (3 paragraphes), `juger_fin_at`, `confiance_declaree` (3 compétences), réponses de métacognition (fiche 4.3). Projection : « 1 copie validée sur 1 », relecture puis **« Temps écoulé »** à 07 h 47 ; écran de passation : « Lancée à 07 h 40 ; dépôt ouvert à 07 h 44 ; fin de la rédaction à 07 h 45 ». Non vu : l'apparition sans rechargement (volet caché : la veille se tait sur un onglet invisible — vue au bac à sable, 16,7 s). Petit défaut vu à 304 px seulement : la phrase « Après l'envoi, la machine lira ta copie… » s'écrase à côté de « Envoyer ma page » → **corrigé le 25/09** (`min-w-48`, comme à la relecture ; mesuré sur la ligne reproduite avec le CSS du serveur local, sans écrire en base : 304 px, 7 lignes dans 78 px → 2 lignes pleine largeur, bouton dessous ; 375 px, 2 lignes, bouton dessous ; 768 px, une ligne à côté du bouton, inchangé ; **revu sur le vrai écran élève au bac à sable** après relance de l'épreuve du décor, sur le « go » de Louis : mêmes mesures, aucun débordement horizontal) ; « 4. Ta confiance » passe sur deux lignes, sans coupure de mot : laissé tel quel.
- ☑ **Décor du BAC À SABLE RETIRÉ le 25/09** (demande de Louis) : ligne de plan marquée `recette-epreuve-minutee-2026-09-24`, son instance et ses 7 dépôts (cascade : 2 tâches de transcription ; 0 mesure, retour, décision du routeur), transaction répétée en `rollback` avant le `commit` ; 2 photos retirées du stockage `codex`. Au pied : 1 182 exercices, comme avant le décor. Les 4 lignes d'`api_couts` gardées (`depot_id` à NULL). ⚠️ La porte `epreuve_minutee_actif` du bac à sable est laissée **OUVERTE**, comme en prod (elle était fermée avant le décor).
- ☑ **Traces du smoke en PROD RETIRÉES le 25/09** (Classe Test), par Claude à la demande de Louis (l'application refuse, à juste titre, de retirer un examen qui porte une copie) : inventaire en lecture d'abord (ligne de plan `ba3eb7bb…`, instance `007c4f19…`, dépôt d'Élo `7554ad9e…` avec 1 tâche de transcription et 1 ligne de métacognition, 1 photo ; **0** mesure, retour, décision du routeur — la chaîne n'avait rien mesuré), puis une transaction en prod répétée en `rollback` avant le `commit` : règles des FK relues EN PROD (identiques au bac à sable), comptes attendus vérifiés dans un `do` qui lève, trois `delete` bornés chacun par deux identifiants (`DELETE 1` × 3), cascades comprises. La ligne de plan est EFFACÉE, pas « annulée » : elle n'existait pas avant le smoke. Photo retirée du stockage `codex`, dossier vide relu. Les 2 lignes d'`api_couts` sont GARDÉES (dépense réelle), `depot_id` passé à NULL. Au pied : 1 340 exercices, 1 172 dépôts. La session d'Élo reste ouverte dans le navigateur intégré.
- ☑ Commits `a21ee28` (le lot) et `2a77539` (le handoff du 29/08), 24/09 soir, **poussés par Louis** ; `7effd82` (3ᵉ revue) **poussé et déployé** le 25/09 ; ☑ `codex_epreuve_minutee.sql` en PROD (25/09) ; ☑ push ; ☑ porte ouverte en prod par Louis (25/09) ; ☑ smoke prod dans la Classe Test (25/09) ; ☐ **conception de l'examen du 29/09** (ligne `1c00880c…`, 1HLP) ; ☐ assignation.
- ☐ Retirer le décor du bac à sable après relecture (la ligne de plan marquée, son instance, ses 7 dépôts et leurs photos) et refermer la porte du bac à sable.
...
- **Trouvé à l'écran, corrigé (`3fb2f9c`)** : après l'enregistrement, la question corrigée passait de Q15 à Q1 et les numéros se rebattaient — les questions d'une génération partagent la même date de création (prod : 17 questions, 2 dates) et la page ne triait que sur elle. L'id départage : ordre stable (même requête relue deux fois en prod, même ordre). Élèves non concernés (ordre tiré et gardé par session). **Revu à l'écran en prod après le déploiement de `c2b1386`** : ordre des 17 questions relevé, « Modifier » sur Q7 (accord « l'avoir communiquée » corrigé dans un leurre), enregistré → même ordre, 0 différence, avant et après rechargement complet de la page ; en base, le leurre corrigé, la bonne réponse inchangée, la question validée.
...
- **Mesuré avant code (prod, lecture PostgREST)** : quiz de T5, 15 questions : bonne réponse strictement la plus longue **8/15** ; les **4** applications reprennent l'exemple même d'une carte ; la carte d'Inès porte bien « feuille blanche », c'est la reformulation du modèle qui l'a perdue. Cause : chaque carte devient une question, son verso la bonne réponse (recouvrement 0,63–1,00), les leurres sont inventés plus courts. Ancien générateur rejoué 3 fois : 8, 8 et 11 sur 15.
- **Décisions de Louis** : un écart de longueur d'un ou deux mots ne se voit pas, au-delà oui ; une application qui reprend la situation d'une carte avec d'autres noms lui va ; la bonne réponse ne doit pas se reconnaître à la formulation de la carte.
- **Fait** : prompt à quatre règles + champ `nature` (retiré avant la base) ; `utils/quazian-controle-questions.ts` (pur) : écart > 2 mots (mesuré en caractères ÷ mot moyen), réponse > 90 caractères, bonne réponse qui recopie 3 mots d'affilée d'une carte de plus que les leurres, exemple repris dans une application, lecture à l'aveugle des applications (autre réponse, réponse défendable, fait manquant) ; génération avec 20 % de marge et `max_tokens` proportionnel (6 000 fixes tronquaient au-delà de ~40 questions), UNE réécriture groupée, relecture, écart dit au professeur ; « Nouveaux distracteurs » garde la bonne réponse du professeur mot pour mot ; `maxDuration` 300 sur les deux pages.
- **Banc** (générateur réel sur les 71 cartes du périmètre de T5 lues en prod, aucune écriture en base — le bac à sable n'a pas ces cartes) : bonne réponse plus longue de plus de 2 mots **22/60 → 0/42** ; réponse de plus de 90 caractères **16/60 → 0/42** ; bonne réponse seule option la plus proche d'une carte **64–68 % → 38 %** (hasard 25 %) ; relecteur : Inès signalée, « blanche » rétabli plus rien ; questions obtenues **15, 15, 14 sur 15** ; création **56–69 s** (32–37 s avant) ; **~0,10 $** par quiz de 15 (0,045 $ avant).
...
- **Traces laissées en prod** : le quiz d'essai **en brouillon** dans la Classe Test (`7aa8f3f1…`, 17 questions, invisible des élèves) et son exercice au plan de la Classe Test (semaine du 21/09, `a_concevoir`) ; 9 lignes `api_couts` (0,18 $). **Nettoyé par Louis le 24/09** : quiz et questions supprimés (vérifié en prod : 0 ligne), exercice du plan de la Classe Test passé à `annule` ; les lignes `api_couts` restent (coûts réels).
- **Suite (état au 24/09)** : Louis corrige les indices restants à la main (« Modifier » fiabilisé, « Tout valider » confirmé — entrées ci-dessus) ; quiz d'essai supprimé. Restent ouverts, préexistants : ligne du 23/09 d'`IDEES_post_rentree.md`, points (2) à (7).
...
Demande de Louis du 22/09 après le quiz de T5 (20 élèves sur 23) ; conduite point par point avec lui (décisions ci-dessous). État de travail détaillé : `REPRISE_Quazian_retours_classe.md` (local).

- **Mesuré avant code (prod, agrégats)** : l'écran de lancement listait **64** élèves pour une classe de **23** ; 300 réponses, 69 % « 100 sur une seule », 88 % multiples de 25, 5 élèves sur 20 sans aucune nuance ; 20 réponses « 100 sur une fausse » = 41 % des points perdus ; **les 15 questions avaient `index_correct = 0`** et le mélange de session partait à la page.
- **Décisions de Louis** : curseurs − / + au point près avec plafond (comme les exercices) ; cases de navigation à trois états ; récapitulatif avant l'envoi ; antichambre + question d'essai « capitale de l'Australie » ; le tableau « Pourquoi partager tes points » remplace le texte de l'antichambre et figure sur l'écran de note ; par réponse « +X / −X / 0 » et le calcul de la moyenne ; la fuite de la bonne réponse intégrée au chantier ; tuteur à partir de **70 points sur une mauvaise réponse** ; tuteur en pause pendant un quiz.
- **Fait** : (1) liste = la classe du quiz, lecteur unique page + route ; (2) avancée par élève « 9/15 · reste 6 » ; (3) retour dans l'ordre de la session de l'élève, cases à trois états, récapitulatif ; (4) curseurs ; (5-6) antichambre, lancement en deux temps, retardataire, porte `quazian_antichambre_actif` ; (7) écran de note expliqué ; (8) « En parler avec le tuteur », porte `quazian_tuteur_actif`, contexte du quiz dans le suffixe par élève, coût « quazian-tuteur » ; tuteur en pause jusqu'à l'échéance d'un quiz lancé dans une classe de l'élève (sans porte, bornée par `ferme_at`) ; fuite : réponses remêlées au hasard avant la base, plus d'ordre de mélange dans la page, mélange de session sans biais.
- **Recette au bac à sable** (décors marqués `DECOR-RETOURS-CLASSE-2209`, captures 375 / 768 / 1280 dans `backups/quazian-retours-classe-20260922/`, ignoré) : tableau prof à 7 lignes et avancées 0 à 15/15 ; curseur B qui bute à 30 quand A tient 70 ; cases et récapitulatif sur 15 questions ; antichambre à deux sessions (prof + compte d'essai élève) + 3 figurants : « 4 / 7 », lancement, bascule seule de l'élève sur la question 1, retardataire aux consignes ; écran de note d'un quiz fermé semé (48,75 ÷ 15 = +3,25 → 13,3 / 20) ; tuteur : 3 boutons, message d'ouverture, réponse du modèle sur la bonne question (1 appel, 0,0011 $ compté « quazian-tuteur »), pause affichée « Il revient à 07 h 28 » ; fuite : « optionMapping » absent du HTML, 70/30 enregistrés sur les bonnes réponses d'origine. **Portes refermées, décors et conversation d'essai retirés, vérifiés par requête** (1 quiz, 7 conversations, portes OFF).
- **Revues adversariales** : points 1-6, 4 regards, 0 bloquant, constats corrigés (questions non validées entrant par l'antichambre, antichambre oubliée → expiration 3 h, boutons perdus sur une lecture ratée, « temps écoulé » serveur avant l'horloge, sauvegarde incertaine, arrondis, tableau fermé à 375, etc.) ; lot tuteur / fuite / pause : 3 regards, 0 bloquant, corrigés — ⛔ **deuxième chemin de fuite (préexistant)** : un élève qui soumettait en avance lisait par l'API son score PAR question, qui désigne la bonne réponse → les scores par réponse ne s'écrivent plus qu'à la **fermeture** (`fermerQuizz`, avant de figer ; la note se calcule toujours à la soumission) ; graine du mélange de session **aléatoire** (l'ancienne se recalculait avec le code public) ; « Modifier » et « Nouveaux distracteurs » refusés hors brouillon / antichambre ouverte (ils pouvaient déplacer la bonne réponse d'un quiz lancé) ; route du tuteur : contexte seulement pour une question de la classe de la conversation et une erreur assurée, historique qui repart toujours d'un message d'élève ; ouverture refusée pendant la pause, conversation vide retirée si le message d'ouverture échoue ; texte tapé rendu à la saisie après un refus ; synthèse hebdomadaire sans les fils sans message d'élève ; message clair si la porte est basculée avant le SQL. Recette de la fermeture au bac à sable : copie soumise par l'écran → **15 réponses sans score tant que le quiz tourne**, fermeture par l'écran → **90 réponses sur 90 scorées**, note 11,1 / 20 cohérente avec l'écran de note. Prod mesurée avant : **0 quiz brouillon, 0 lancé** (aucune question d'avant le correctif ne peut être lancée si le prochain quiz est créé après le push).
- **Revue adversariale FINALE du commit déployé** (23/09, 4 regards, à la demande de Louis) : 0 bloquant. Mesuré en prod : les 20 copies réelles de T5 — note affichée = note en base (0 écart), lettres = celles vues par les élèves ; policies Quazian lues (lecture seule) : `prof_all_answers` couvre l'écriture des scores à la fermeture. **Corrigé en local (à pousser)** : ⛔ troisième chemin de fuite — la note d'une copie envoyée tôt se lisait par l'API pendant le quiz (copie « sonde ») → migration `quazian_scores_apres_fermeture.sql` (policy élève bornée aux quiz fermés), **jouée au bac à sable seulement** ; « Nouveaux distracteurs » relit le statut APRÈS l'appel IA (un lancement pendant la génération faussait une question) ; l'écran d'attente ne cesse plus de sonder après une lecture ratée ; retour en haut de page à chaque question ; « J'ai vu ma note » en haut de l'écran de note (11 élèves de T5 ne l'ont pas encore cochée) ; pas de copie neuve après l'échéance (12,5/20 d'office) ; double toucher sans « Erreur création session » ; note non écrite → verrou relâché ; `z_quiz` non écrasé par un envoi tardif ; auto-soumission à la fermeture sans « duplicate key » ni score par réponse ; seconde passe des scores après avoir figé ; calcul de la note au millième ; refus de « Modifier » dits ; le tuteur garde la question même porte fermée ; cases de 40 px. Recette au bac à sable : défilement 0 px après « Suivant », note illisible par l'API pendant le quiz (0 ligne) puis lisible (1), 3 auto-soumissions, 90/90 scorées, 0 z nul, « J'ai vu ma note » à 548 px.
- **Suite du 23/09 (soir)** : `quazian_scores_apres_fermeture.sql` **jouée en production** sur le « go » de Louis (0 quiz lancé ; répétition annulée puis passage réel ; 20 notes intactes). Demande de Louis : une note de quiz non « vue » crée la tâche **« Ta note de quiz t'attend »** en tête de l'accueil (badge « bloquant », « tant que ce n'est pas fait, tu ne peux rien rendre dans les autres modules ») — vérifié au bac à sable aux trois largeurs, et la tâche disparaît une fois la case cochée. Diagnostic : le « Rapport de fragilités » n'est **jamais enregistré** (affiché une fois, perdu au rechargement) ; un appel IA Quazian du prof est au journal des coûts le 22/09 à 15:10 UTC, 40 min après la fermeture du quiz — vraisemblablement ce rapport.
- **Rapport de fragilités conservé + diagnostic paginé (23/09, soir ; décision de Louis : « fais les deux, par classe et daté »)** : ⛔ mesuré — le rapport n'était écrit nulle part (affiché une fois, perdu au rechargement) et mêlait toutes les classes ; les quatre lecteurs du diagnostic (`chargerDiagnosticClasse`, `chargerDiagnosticEleve`, `chargerLeDiagnosticParCible`, la rétention du pilotage) lisaient `quazian_answers` sans page — **300 réponses notées pour 1 quiz en prod**, PostgREST coupe à 1000 sans le dire, donc tronqué dès le 4ᵉ quiz de cette taille. **Fait** : `utils/quazian-pages.ts` (`toutesLesPages`, tri stable session + question — **0 doublon** de cette clé en prod) sur les quatre lecteurs ; table `quazian_rapports_fragilites` + porte `quazian_rapport_actif` (Paramètres → Passation) ; porte ouverte, Diagnostic → Par classe → une classe montre sa carte « Rapport de fragilités — classe » : le dernier rapport en entier, daté au fuseau du prof, les précédents repliés à leur date ; porte fermée, l'ancien rapport toutes classes revient tel quel. **Deux défauts vus à l'écran et corrigés** : le modèle rend du Markdown et l'écran l'affichait brut (`##`, `**` — l'ancien rapport aussi) → consigne « texte simple » + `texteSansMarkdown` à l'écriture et à l'affichage ; sans idée fausse ni lacune, l'IA était payée pour écrire « rien à signaler » → plus d'appel, message « Rien à signaler : sur N concepts évalués… ». **Recette au bac à sable** (classe « Test », seule à avoir un quiz fermé : 5 réponses, 5 concepts) : porte ouverte par l'écran, 2 rapports générés (873 car., 16 lignes ; 0,0046 $ chacun, coût enregistré), relus en base et après rechargement, « Rien à signaler » sans appel, carte aux trois largeurs 375 / 768 / 1440 sans débordement (document = largeur d'écran), porte refermée par l'écran → ancien rapport revenu ; les 2 rapports d'essai **supprimés** (0 en base). ⚠️ « Hydration failed … `open` » vu une fois sur le repli : c'est Chrome qui restaure un `<details>` ouvert au rechargement (mémoire `reference_details_open_hydratation`), pas le composant. **Migration** `quazian_rapports_fragilites.sql` jouée au bac à sable puis **en production** (répétition annulée vérifiée, porte **OFF**, 0 rapport). **Revue adversariale (1 regard)** : 0 bloquant ; corrigé — une page de lecture en échec rendait les premières pages comme si c'était tout (un rapport daté se serait bâti dessus) → elle rend l'erreur et **aucune ligne**, le rapport refuse, l'écran dit « Diagnostic incomplet » ; la classe lisait les réponses de TOUTES les classes puis filtrait → filtre par quiz de la classe **en base** (`!inner`, vérifié au bac à sable : 0 ligne sur 5 hors de la classe, 5 sur 5 dedans) ; `***` et `(*)` épargnés par le nettoyage ; porte ouverte sans classe choisie, la consigne le dit. **Non vu** : un rapport sur de vraies fragilités (le bac à sable n'en a aucune) — le premier se lira en prod sur T5, porte ouverte par Louis.
- **Contrôles** : `tsc` propre, lint des fichiers du lot propre, **2 786 tests verts**, `next build` réussi.
- **Déployé (23/09, 23:14 UTC)** : `6e0d9a8`, `a04cf95`, `236abb7`, `8ad72e0` poussés par Louis ; `origin/main` = `8ad72e0` après fetch, statut Vercel `success` à 23:14:21 UTC ; la page de connexion servie est neuve (`age` 19 432 s avant → 0 après) ; `/prof/quazian/diagnostic` et Paramètres → Passation renvoient vers la connexion (307), la route d'antichambre répond 401. Prod relue en lecture seule : antichambre et tuteur **ouverts**, rapports conservés **fermés**, 0 rapport, 0 quiz lancé. ~~Non vu connecté en prod~~ ➜ **Vu par Louis le 23/09 (« ça fonctionne »)** : il a ouvert la porte `quazian_rapport_actif` et généré le premier rapport de T5 — relu en base (lecture seule) : 1 rapport, 1 476 caractères, 3 paragraphes, **aucun Markdown restant**, 0,0067 $.
- **Production (23/09, sur l'ordre de Louis, absent de son poste)** : `quazian_antichambre.sql` puis `quazian_tuteur.sql` joués AVANT le push (répétition annulée puis passage réel, constats au `SUIVI_SQL.md`), portes relues à OFF. Commit `4c955f6` fait par l'assistant (le push lui était impossible : aucun identifiant GitHub dans la séance), **poussé par Louis le 23/09 ; déploiement Vercel Production `success` à 17:45 UTC** (statut GitHub du commit et du déploiement). Vérifié en direct : la route neuve `/api/quazian/antichambre/…` répond 401 « Non authentifié » sur palimpseste.ink (`x-vercel-cache: MISS`, pdx1) — elle n'existait pas avant ; `/login` 200 ; portes relues OFF, aucun quiz lancé ni brouillon en prod.
- **Reste à Louis** : créer le prochain quiz (le code déployé remêle ses réponses) (les questions générées avant gardent la bonne réponse en tête) ; ouvrir les deux portes (Quazian → Paramètres → Passation). ⚠️ Ne pas pousser pendant une passation : les points 1-4, la fuite et la pause changent la passation sans porte.
- **Limites** : pas éprouvé en prod ; un élève qui recharge perd l'état « vue » des cases ; le retardataire qui n'appuie pas sur « Commencer » n'a pas de note ; aucune fermeture automatique à l'échéance (hors lot, noté aux idées).

## Quazian — main du professeur sur les questions, recette locale terminée (21/09/2026 soir, 22/09 UTC)

Spécification : `PROMPT_Code_Quazian_quiz_main_du_prof.md`, lue intégralement. Louis a choisi **la confirmation dans la carte** (« Confirmer le refus » / « Garder »). Livraison locale, **push à Louis**, aucun déploiement. **Aucune migration, aucune colonne, aucune ligne ajoutée à `SUIVI_SQL.md`. Aucun nouvel interrupteur**, conformément à la dérogation explicite du prompt. La porte du plan, relue ON au bac à sable, n'a pas été modifiée. Dépôt de conception non ouvert.

- **Mesuré avant code**, par PostgREST : production, 1 brouillon / 15 questions `suggere`, énoncés médiane 82 / max 123 caractères, options médiane 50,5 / max 108, 4 options chacune, 14 tags distincts ; bac à sable, 1 quiz fermé / 5 questions `valide`, énoncés 83 / 97, options 106 / 160. **63 cartes partagées validées**, réparties en 45 et 18. Compteurs exacts dans les deux bases, un seul `created_at` par lot initial et aucune source de flashcard renseignée.
- **Implémenté** : `refuserQuestion` relit le statut, refuse hors brouillon et DELETE par `quiz_id` **et** `id`, erreurs lues (dont FK 23503) ; `ajouterQuestions` garde 1–10, tronque la consigne à 300, partage la lecture bi-source avec la création, transmet tous les énoncés/tags existants et insère en `suggere`. Même modèle, schéma, règle JSON, garde et journal de coût. Statut relu après l'appel IA. Chaque mutation recalcule par COUNT, synchronise le plan et revalide les pages. **Concurrence éprouvée par test** : l'ancien UPDATE retardé pouvait laisser 18 au lieu de 21 ; relecture après UPDATE et nouveau COUNT si nécessaire, test désormais vert. Deux appels réussis produisent deux lots : pas de déduplication des requêtes.
- **Recette réelle, professeur au bac à sable** : création de 15 questions depuis l'écran, validation → plan `concu` ; ajout de 3 avec consigne → **18 lignes / compteur 18**, nouvelles questions à la fin en `suggere`, plan `a_concevoir`, message « 3 questions ajoutées sur 3 demandées. », N remis à 3 et consigne vidée. Coût à 00:51:43 UTC : **0,020664 $**, Sonnet 4.6, 4 403 / 497 tokens, classe correcte. Les 15 énoncés et les 3 nouveaux sont reproduits dans le rapport local ; aucune des trois questions ne répète une question initiale dans cet échantillon.
- **Refus et édition** : ouverture de Q2 et saisie, refus confirmé d'une autre carte → **17 lignes / compteur 17**, numéros Q1–Q17, texte conservé à l'écran. Ajout sans consigne → **20 lignes / compteur 20**, texte toujours conservé, coût **0,018693 $** avec la classe. Nouveaux distracteurs → Modifier : les quatre options ouvertes sont exactement les nouvelles (avant/après et capture conservés). Réinitialisation au clic seulement, aucune synchronisation par `useEffect` ; clés des cartes inchangées.
- **Gardes et fin du quiz** : appel direct de `refuserQuestion` sur le quiz fermé → « Seul un brouillon se modifie. », ses **5 lignes intactes**. Demande forgée à 11 depuis le navigateur → refus serveur et consigne conservée. Validation de tout → plan `concu`. Refus jusqu'à la dernière question → **0 ligne / compteur 0**, plan `a_concevoir`, message « Ce quiz n'a plus de question — génère-en ci-dessous. », lancement absent.
- **Rendu relu** à **375, 768 et 1280 px**, avec des options de **108 et 160 caractères déjà présentes au bac à sable**, temporairement reprises dans le décor de recette : `document.scrollWidth` = respectivement **375 / 768 / 1280** ; carte, trois boutons et formulaire sans débordement. Confirmation mobile et erreur conservant la consigne capturées. Ces options constituent un décor de longueur, pas une validation du contenu pédagogique.
- **Nettoyage vérifié** : nouveau brouillon et nouvel exercice de plan retirés ; quiz préexistant, 5 questions et lien au plan **identiques au relevé initial**. Les 4 lignes de coût restent : **0,087417 $** au total (création, deux ajouts, distracteurs ; ces derniers restent non attribués, patron existant). Aucune écriture métier en production. Navigateur de recette fermé.
- **Contrôles** : TypeScript propre, ESLint **sans erreur ni avertissement sur les six fichiers du lot**, **2 748 tests réussis**, dont 31 nouveaux. Le lint global n'est pas propre : **120 erreurs / 52 avertissements hors lot**, dont 3 erreurs dans les sources de l’application ; les autres portent sur des scripts de recettes antérieures, suivis ou ignorés. Les sept fichiers suivis concernés sont **identiques à HEAD** ; consigné dans `IDEES_post_rentree.md`, aucun correctif hors périmètre. Pas de build pendant le serveur de développement. `git fetch origin` réussi avant le bilan Git.
- **Limites** : lot partiel, échec de FK/COUNT/UPDATE et concurrence éprouvés en tests isolés, pas provoqués dans la base réelle. Une question refusée peut revenir (aucune mémoire des refus) ; le tri des questions d'un même lot reste sans départage. L'absence de doublon du premier ajout est une observation de recette, pas une garantie générale du modèle. Les gestes anciens de validation/modification/distracteurs ne sont pas réécrits.

Preuves détaillées et captures, **locales ignorées par Git** : `backups/quazian-main-prof-20260922/RAPPORT.md`. Aucun identifiant élève ni secret ajouté aux journaux suivis.

## Vestigia prof — « Supprimer » un dépôt depuis la fiche élève, éprouvé au bac à sable, COMMITÉ en local — push à Louis (21/09/2026, soir)

`supprimerDepot` existait sans écran. Nouveau : section **« Dépôts du semestre »** dans la fiche élève (`eleve/[eleveId]/page.tsx`) — TOUS les dépôts du semestre consulté, semaine, « en retard », état de l'analyse (sans analyse / en cours / erreur / à valider / publiée), lien vers l'atelier, et `BoutonSupprimerDepot.tsx` (confirmation qui nomme la semaine et prévient si le retour est PUBLIÉ). La fiche ne chargeait que les analyses publiées (courbe, moyennes) : elle charge désormais tous les statuts pour la liste, la courbe reste sur les publiées. **`supprimerDepot` durci** : la ligne D'ABORD (client admin, `.select` lu — refus explicite si `DELETE 0`), les fichiers ENSUITE ; l'ancien ordre retirait les photos avant d'effacer la ligne avec le client prof sans lire le retour (le piège du 29/08 côté élève). Revalidation de la fiche ajoutée.

- **Rejoué au bac à sable** (prof par lien magique, Elo, semaine 9) : dépôt par le formulaire prof → fiche : « Semaine 9 · analyse en cours… · Supprimer » → clic (confirmation forcée à oui par script, message lu : « Supprimer le dépôt de la semaine 9 ? Les photos, l'analyse et ses pistes seront effacées… ») → section disparue ; en base : 0 ligne, 0 analyse, 0 fichier dans le dossier. `tsc`, `eslint` propres.
- ⛔ **Dommage collatéral au bac à sable, par moi** : au premier essai, mon dépôt n'avait PAS été créé (clic à côté du bouton, aucune trace serveur) et la seule ligne de la section était le dépôt de DÉCOR d'Elo — « semaine 2 · publiée », créé le 30/08 comme celui de Sacha — que j'ai supprimé en croyant supprimer le mien. La confirmation disait bien « le retour est PUBLIÉ », je ne l'ai lue qu'après. Sacha garde le sien (« à valider »). Aucune donnée réelle : bac à sable seulement. **Reposé le 21/09 soir à la demande de Louis** (script jeté, bac à sable vérifié) : dépôt `9a7757f1` d'Elo, classe Test, même semaine que Sacha, daté du 30/08, une photo dessinée (PIL, 1500 × 2000, marquée « décor » dans l'image), analyse PUBLIÉE le 31/08 (3·2·3, textes sur la Méditation première, `notes_prof` = marque décor), 2 pistes proposées. Relu en base : ligne, photo (209 Ko) et analyse présentes.
- **Limites** : rendu téléphone non regardé (une ligne `flex-wrap`, bouton en `ml-auto`) ; pas de test automatisé (composant + action) ; le rendu de la section n'a été vu qu'avec un seul dépôt.

## Vestigia — le schéma rendait la corruption SILENCIEUSE : règle « jamais de guillemet droit » ajoutée par le code + appel rejoué sur signature, COMMITÉ en local — push à Louis (21/09/2026, soir)

Louis a poussé le schéma (`c21155e`, `851ea22`), relancé l'analyse de Nina P. : elle est passée en `generee`… **impubliable**. Lu en prod : `retour_langue` se termine par « Pour répondre à la question », `retour_style` COMMENCE par « , manque de virgules… », `retour_contenu` par « , il faut… », `commentaire_general` par « », voici la correction ». **Quatre champs = les morceaux d'une seule phrase.** Mécanisme : le modèle émet le guillemet droit de « la question "qui suis-je ?" » ; la grammaire de `output_config.format` l'ACCEPTE comme fin de chaîne, force la clé suivante, et le modèle continue sa phrase dans le champ d'après. JSON valide, rien ne lève — pire que l'erreur d'avant, qui au moins se voyait. ⭐ La grammaire contraint la FORME, pas l'ÉCHAPPEMENT.

- **Parade 1 — `REGLE_JSON_TEXTE`** (`utils/ia-commun.ts`) : « n'écris JAMAIS le guillemet droit ni la barre oblique inverse dans les chaînes ; cite avec « … » ; vaut aussi pour la transcription ». Ajoutée **par le code au moment de l'appel** (`prompt + REGLE_JSON_TEXTE`, ou `system + …` pour flashcards/QCM) dans les 7 modules : un prompt personnalisé en base ne peut pas l'omettre.
- **Parade 2 — `creerJsonSurveille`** (`ia-commun.ts`) : la signature de la corruption est un champ texte qui COMMENCE par `, ; . » ) ]` (`champJsonSuspect`, récursif, chemin du champ). Si présente : `console.warn` et l'appel est rejoué UNE fois ; l'usage rendu somme les deux appels (coût vrai). Branché sur les 5 analyses à objet (Vestigia, orale, essai, Codex V1/VF, synthèse de semestre) ; flashcards et QCM n'ont que la règle (sorties courtes).
- **Preuve** : 5 tests unitaires (`utils/ia-commun.test.ts`, dont la forme EXACTE de la réponse de Nina → `retour_style` détecté) ; **2 rejeux réels sur les photos de Nina** avec règle + garde : 0 guillemet droit dans toute la réponse, aucun champ suspect, 4 retours de 660 à 1 540 caractères, 3 pistes, « bonne réponse » transcrit. +100 jetons d'entrée (6 496 → 6 602). `tsc`, `eslint`, `npm test` 2 717 verts (5 ajoutés).
- **Reste à Louis** : pousser, puis **relancer l'analyse de Nina** (la ligne `generee` corrompue sera remplacée). ⚠️ Son `retour_progres` dit « l'historique fourni ne contient aucun retour antérieur » : vrai (premier dépôt de Nina en S3), mais formulé pour le prof — le prompt demande un progrès « à partir de la semaine 2 » ; à relire avant publication, non touché.
- **Limites** : le rejeu réel ne couvre que Vestigia ; les autres modules reçoivent le même mécanisme sans copie réelle rejouée. La détection ne voit que la coupure « en début de champ » ; une coupure qui tomberait pile en fin de phrase resterait invisible (cas improbable : le champ suivant commencerait alors une phrase neuve).

## Vestigia — l'analyse IA tombait en `erreur` sur un JSON mal échappé ; le JSON est désormais GARANTI par l'API (schéma), COMMITÉ en local — push à Louis (21/09/2026)

Cas : le dépôt fait par Louis pour Nina P. (S3, prod) — dépôt et photos sains (2 JPEG de 1505 × 2000, lus depuis le bucket), analyse `erreur` deux fois (17:41, relance 17:56), seule en erreur sur les 15 dernières. **Cause reproduite hors plateforme** (rejeu de l'appel en local, sans écriture : mêmes photos, même prompt assemblé avec le thème et la config de prod, même modèle) : 1er rejeu → JSON invalide à la position 3476 (guillemets droits de « bonne réponse », écrits par l'élève, recopiés non échappés dans une chaîne) ; 2e rejeu → valide. Intermittent, lié au contenu de la copie. Même famille que Lisa G. (14/09), où la réponse était tronquée.

- **Correctif** (`utils/analyse.ts`) : `output_config.format` avec un schéma JSON (`SCHEMA_ANALYSE`, exporté) — l'API contraint la réponse, l'échappement ne peut plus manquer. Le prompt décrit toujours le format ; `signal_integrite` devient exigé (`type: "aucun"` sinon — `signalDepuisIA` rend `null` pour « aucun », inchangé). ⚠️ L'API refuse `minimum`/`maximum` sur un entier (400 mesuré) → `enum: [0,1,2,3,4]` pour les trois notes.
- **Preuve** : 2 rejeux avec le schéma sur les photos de Nina → JSON valide les deux fois, notes 2·2·2, signal « aucun », « bonne réponse » présent dans la transcription (donc échappé). Entrée +1 000 jetons par analyse (5 503 → 6 496, le schéma compte en entrée) ≈ +0,003 $ ; 43-46 s.
- `tsc`, `eslint` propres. Aucune migration, aucune porte.
- **Reste à Louis** : après déploiement, **relancer l'analyse de Nina** depuis le panneau.
- **Étendu le même jour aux autres analyses** (commit suivant) : 7 schémas sur 6 fichiers — `analyse-orale` (SCHEMA_ORALE), `analyse-essai` (SCHEMA_ESSAI), `codex-analyse` (SCHEMA_V1 avec `ortho` en `anyOf` chaîne/null, SCHEMA_VF), `synthese-semestre` (SCHEMA_SYNTHESE), `extraire-flashcards` (SCHEMA_CARTES, tableau à la racine, deux appels), `generer-questions` (SCHEMA_QUESTIONS, deux appels). Mesuré auprès de l'API avant d'écrire : tableau racine, `anyOf` + `null`, `enum`, `number` acceptés ; `minItems`/`maxItems` > 1 REFUSÉS → la garde `estQuestionValide` continue de vérifier les quatre options. **Les 7 schémas réels validés** par un appel minimal chacun (Haiku, ~300 jetons) : tous acceptés, JSON lu. `tsc`, `eslint` (2 avertissements préexistants), `npm test` 2 712 verts. NON touché : la synthèse Scriptorium (`scriptorium-synthese-rag`) passe par l'abstraction multi-fournisseurs du C12 (`fournisseur.repondre`), qui n'expose pas `output_config` — à traiter dans C12. Aucune migration, aucune porte. Limite : aucun rejeu sur une vraie copie pour l'oral, l'essai, Codex et la synthèse (même mécanisme, mêmes types de champs).

## Vestigia prof — déposer un fragment À LA PLACE d'un élève, semaine fermée comprise ; codé, éprouvé au bac à sable, COMMITÉ en local — push à Louis (21/09/2026)

Demande de Louis : verser lui-même les photos d'un élève qui n'a pas réussi à déposer, même semaine fermée. Nouveau : action serveur `deposerPourEleve` (`app/prof/fragments-erudition/actions.ts`), formulaire `FormulaireDepotProf.tsx`, branché dans le panneau de l'élève **manquant** de la vue Semaine (bouton « Déposer à sa place… » sous « Pas de dépôt cette semaine »). Même chemin que le dépôt élève (ligne `fragments_depots` → `fragments_photos` → `fragments_analyses` → analyse IA), à trois différences voulues : aucune garde de semaine ouverte / blocage / retour non lu (le prof décide) ; envoi des fichiers par le client admin dans le DOSSIER DE L'ÉLÈVE (la policy d'insertion du bucket n'admet que l'élève, mais ses lectures signées restent valides) ; statut au choix (case « Compter comme rendu à temps », cochée par défaut, sinon la date limite tranche). Le commentaire du dépôt porte « Déposé par le professeur — <note> » (mesuré : `commentaire_eleve` n'entre pas dans `utils/analyse`). Un dépôt existant n'est pas écrasé : refus avec message, suppression d'abord depuis la fiche. Aucune migration, aucune porte (geste prof, réversible par `supprimerDepot`).

- `tsc` propre. Rejoué au bac à sable (compte prof de Louis par lien magique, classe « Test », **semaine 9 fermée**, image générée par canvas injectée dans l'`<input>`) : Elo → « déposé », analyse « en cours… » puis « à valider » (le tableau de bord affiche « 2 fragments à valider »), photo visible dans le panneau, commentaire « Déposé par le professeur » ; Sacha → note « photos reçues par courriel (essai bac à sable) » reprise dans le commentaire, case décochée → « depose » quand même (date limite du 26/10 non passée, attendu). Téléphone 375 px : la feuille montre le formulaire entier, `scrollWidth` = 375. Console sans erreur.
- **Passe adversariale (moi-même, sur le diff)** : 2 repris — un échec du rattachement des photos laissait une ligne de dépôt SANS photo (désormais ligne effacée puis fichiers retirés, message « rien n'a été enregistré ») ; changer d'élève par ← → ou Échap avec des photos choisies les perdait sans prévenir (le formulaire signale « modifié » au parent, qui demande confirmation comme pour un retour non enregistré — mécanisme existant, non rejoué à l'écran). Relu sans correctif : le filtre « Manquants » fait sortir l'élève de la liste après son dépôt (le panneau se ferme, attendu) ; le `classeId` vient du client mais l'inscription ACTIVE de l'élève dans cette classe est exigée en base.
- Les deux dépôts d'essai (Elo et Sacha, semaine 9, classe Test) **retirés du bac à sable** le 21/09 par script (lignes effacées en cascade, 2 fichiers retirés du bucket, 0 dépôt restant sur la semaine). ⚠️ `supprimerDepot` existe dans `actions.ts` mais n'est branché à AUCUN écran.
- **Limites** : pas de vrai fichier HEIC ni photo lourde passés (le traitement est celui, inchangé, du côté élève) ; pas regardé côté ÉLÈVE ce que voit Elo de ce dépôt (il verra un dépôt « déposé » avec ce commentaire) ; l'`inscription` retenue est l'inscription ACTIVE de l'élève dans la classe affichée — un élève sans inscription active reçoit un refus explicite. Pas commité : Louis décide (un push déploie).

## Vestigia — un thème validé ne se re-propose plus ; « activer l'essai pour la classe » n'efface plus les thèmes — POUSSÉ par Louis le 20/09/2026 (`a074631`, `51c12eb`)

Louis a constaté que des élèves re-proposaient un thème déjà validé (oubli du C8 : « Modifier » et le formulaire restaient offerts à tous les statuts, l'action serveur ne refusait rien).

- **Règle pure** `themeModifiableParEleve` (`utils/fragments-theme.ts`) : modifiable si vide, à valider ou commenté ; fermé si validé ou posé par le professeur (arbitrage : un thème saisi par le prof sans proposition est arrêté au même titre). Appliquée par l'écran (`ThemeEleve.tsx` : plus de « Modifier », plus de formulaire) ET par `proposerTheme` (refus avec message : « Ton thème a été validé par ton professeur : il ne se modifie plus. »). Un commentaire du prof posé APRÈS la validation rouvre la proposition, voulu : il demande un changement.
- **Passe adversariale** : toutes les écritures de `fragments_themes` relues (1 élève, 3 prof) ; carte « à faire » élève ne mène au thème que vide ou commenté ; formulaire ouvert avant validation → l'envoi arrive après et est refusé. Trouvé hors périmètre puis corrigé sur demande : `activerEssaiPourClasse` faisait un `upsert` avec `theme: ''` → un clic EFFAÇAIT tous les thèmes de la classe (validés compris). Désormais update de `essai_actif` seul sur les lignes existantes, insert au thème vide pour les autres, erreur remontée au bouton (`BoutonActiverClasse.tsx`).
- `tsc` propre, 14/14 tests de `fragments-theme` (1 ajouté). Aucune migration, aucune porte (correctif d'un comportement existant).
- **Limites** : pas de rendu en navigateur ni de clic sur le bouton de classe contre une base — la modification est un booléen qui masque deux blocs existants, et une séparation update/insert. Déploiement Vercel non vérifié par moi (lire la carte « Production Deployment »). Les re-propositions déjà faites par des élèves sur un thème validé sont en base au statut « à valider » : elles attendent Louis dans le Suivi.

## Scriptorium élève — le Plan de cours en vue Année + volet Parcours, éprouvé au bac à sable, commité en local, push à Louis (18/09/2026 soir)

Handoff `design_handoff_plan_cours_eleve` éprouvé avant d'être suivi : mesuré en prod — 32 semaines d'enseignement, semaine courante 4 ; 1 à 2 parcours par classe, tous titrés (aucun « non dévoilé », `titre` NOT NULL) ; « La Naissance de la Tragédie » alterne (semaines d'année 2 · 9 · 16) ; 3 à 25 éléments par semaine, 1 à 2 créneaux, libellés de 39 à 98 caractères. `tsc`, `eslint`, 2 707 tests verts (9 nouveaux, `utils/scriptorium-plan-eleve.test.ts` : états, segments, pistes, reprise de l'alternance, anti-spoiler — aucun texte ne franchit le DTO). Vu au bac à sable, serveur de dev de l'autre séance (`palimpseste-attache`), élève de test par lien magique, classe « Test » **avec un décor temporaire** (assignation « test » redatée au 07/09 ; « Introduction à la philosophie » au 24/08 → terminé ; « retest » au 31/08 décalé {2:6, 3:12, 4:18, 5:24} → alterne ; « Philosophie des sciences » au 19/10 → à venir ; **décor retiré, état d'origine relu**) : **1280 × 760** — rail 32 semaines, 2 pistes (le parcours qui alterne réserve son enjambée, « test » et « Philosophie » partagent la seconde), trait « aujourd'hui » sur S4, trois colonnes de cartes, page de 859 px ; clic carte → volet (`?parcours=`), colonne Année resserrée avec « ouvert › » et barre pleine ocre, frise avec « ✓ 1 semaine vue · 1 élément » + « déplier ⌄ » (déplié : les rangées existantes + « replier ⌃ »), « dernière semaine » sur S4, note de fin ; **Échap** → `?vue=plan` ; ✕ = même lien ; **768 × 1024** — cartes sur deux colonnes, volet plein écran avec « ‹ Année · test », aucun débordement ; **375 × 812** — un débordement horizontal de 37 px trouvé (une carte imposait sa largeur à la grille) et corrigé (`min-w-0`), libellés du rail cachés sous sm, rerelu à 375 sans débordement (hauteur 1200 px). Console sans erreur. Aucune écriture hors décor, aucun message envoyé. Classe T5 du bac à sable (2 parcours sans élément) : « Introduction » terminée, « Philosophie des sciences » S2 → S7 en cours, carte sans ligne d'éléments. **Limites** : pas regardé en prod (session élève réelle requise) ; la maquette d'un parcours « non dévoilé » n'a pas été vue à l'écran (donnée inexistante) ; le rendu de l'alternance dans la FRISE (semaine « courante » = dernière commencée) est l'existant, non retouché. **Passe adversariale puis reprises (18/09 soir)** : 0 bloquant ; les cinq « à corriger » repris (pause d'alternance sans « cette semaine », parcours terminé tout vu, « commence le », autre année scolaire → terminé, frise lue par la page seule) et trois mineurs (doublons, `Math.min`, axe sous « replier ») ; 2 711 tests verts (4 ajoutés). Rejoué au bac à sable à 1280 avec le décor reposé puis retiré : le parcours qui alterne dit « tu es en semaine 1 · reprend le 19 octobre », barre verte, aucune rangée « cette semaine » ; « test » déplié : l'axe court sous « replier ⌃ ». Console vide.

## L'agenda de classe — évènements libres et intitulés d'évaluation, éprouvé au bac à sable, DÉPLOYÉ 18/09 21:52 UTC, porte prod à OFF (18/09/2026)

Porte `agenda_classe_actif` (Scriptorium → Paramètres), migration `calendrier_agenda_classe.sql` jouée en bac à sable, **remise à OFF après la recette**. Joué dans le navigateur intégré, session prof par lien magique puis session élève (élève de test, classe T5) :

- **Prof, calendrier** : « ＋ Ajouter un évènement » (barre de commande, toutes vues) → titre, date, classes cochées (T5 + THLP), détail, « Visible des élèves » → **deux lignes créées**, redirection sur le jour ; « modifier » (titre et détail réécrits) ; « retirer » avec confirmation en page (la ligne THLP retirée, tombstone). Vue mois : la pastille du 25/09. Téléphone 375 px : formulaire lisible, une colonne.
- **Prof, plan d'évaluation (T5)** : quiz ajouté semaine du 14/09 → « Nommer » → « Quiz — Les Lumières — chapitres 1 à 3 » → jour calé (vendredi 18/09) → « Annoncer aux élèves ». Calendrier prof du 18/09 : « Quiz — Les Lumières — chapitres 1 à 3 · à concevoir · T5 ».
- **Élève (T5)** : mois de septembre : le quiz nommé le 18, « Avoir terminé la lecture de Candide » le 25 ; vue jour du 25 : le détail sous le titre ; agenda téléphone (375) et grille tablette (768) : les deux lignes, le détail dans l'agenda. Sous la classe « Test », rien des deux (scope par classe).
- **Porte remise à OFF, rejoué dans le navigateur** : calendrier prof du 25/09 « Aucune échéance ce jour », du 18/09 « Quiz · à concevoir » (l'intitulé disparaît), bouton d'ajout absent ; Scriptorium → Paramètres montre « L'agenda de classe est fermé ». ⚠️ `get_page_text` a rendu « chargement » sur une page pourtant rendue (déjà noté : la console et la lecture du volet mentent par omission) — la capture et `main.innerText` ont tranché.
- **Passe adversariale puis corrections (18/09 soir)** : le trou réel était le quiz servi par le réglage global D5 (intitulé sans annonce) — fermé par un test (`libelleExercicePlanifie` élève sans `annonce` ⇒ générique). Les corrections sont couvertes par tests purs (2698 verts), pas rejouées dans le navigateur : les écrans n'ont pas changé de structure (un `ring` de focus, un `line-clamp-3`, un `key`).
- **Production** : migration jouée le 18/09 soir, porte à OFF, table vide, relu par PostgREST. Code déployé à 21:52 UTC (`79d85ef`, après incident Vercel — voir `PLAN_DE_CHANTIER.md`). Aucune recette écran en prod : la porte y est fermée, Louis l'ouvre dans Scriptorium → Paramètres puis vérifie « Ajouter un évènement » au calendrier et « Nommer » dans le plan.
- **Limites** : aucun élève réel, bac à sable seulement pour la recette écran. Prod : migration à jouer par Louis (ordre libre, code tolérant), porte à ouvrir par lui.

## Scriptorium élève — la Discussion en bureau sans en-tête (ruban + tiroir), codée en LOCAL, NON commitée (18/09/2026)

Handoff `design_handoff_scriptorium_discussion_bureau` éprouvé avant d'être suivi : mesuré en prod (27 conversations vivantes, 14 élèves) — titres de conversation **60 caractères en médiane et au maximum** (la maquette en montrait 31), 1 conversation par élève en médiane (max 6), classes de 2 à 4 caractères (« Hors classe » et « Classe Test » à 11), tuteur 744 car. en médiane (max 2 384, 14 lignes). Rien ne change la structure ; deux arbitrages : titre tronqué avec points de suspension dans l'en-tête de lettre (entier en `title`), pastille de classe tronquée à 56 px. `tsc` et `eslint` propres. Vu au bac à sable avec l'élève de test par lien magique, classe « Test » (4 conversations, un fil de 2 messages) : **1280 × 760** — aucune barre d'en-tête (mesuré `display: none`, `<main>` sans cadre), ruban 64 px pleine hauteur, colonne de lecture 900 px centrée, la page ne défile pas, seul le fil défile (802 px dans 652), écritoire collé en bas ; tiroir ouvert : 360 px dans le flux à x = 64, la colonne passe à 856 px ; **1024 × 700** — colonne 900 px, tiroir ouvert la colonne se serre, saisie et bouton « Envoyer » actifs ; **375 × 812** et **768 × 900** — inchangés (bandeau, sous-onglets, liste ↔ fil, feuillet). `?vue=plan` à 1280 : en-tête à deux barres intact. Aucun message envoyé, aucune écriture en base, aucun test automatisé touché. Commité `e53ce09`. **Passe adversariale** (regard indépendant sur le diff) : 1 bloquant — la césure du `:has()` était en `px` quand le `lg:` de Tailwind est en `rem` : avec une police par défaut agrandie, une bande de largeurs sans AUCUNE navigation (en-tête masqué, ruban pas rendu) → `64rem` ; 4 à reprendre — l'en-tête de lettre défilait avec le fil (quota invisible dès qu'on lit) → épinglé `sticky` ; l'en-tête du tiroir et « Récentes » s'écrasaient sous une longue liste → `flex-none` ; le chevron du médaillon rogné par le clip du disque → frère hors du clip ; le ＋ du ruban ne remettait rien à zéro après un envoi échoué sur une conversation neuve (pas de remontage) → réinitialisation locale ; 2 mineurs — sceau nommé en `sr-only`, tiroir nommé `aria-label`, repli au choix limité au bureau. Consigné sans correctif : sans `:has()` (Firefox < 121) l'en-tête et le ruban se montrent ensemble, dégradé mais navigable. Re-vérifié à 1280 : en-tête épinglé après 95 px de défilement, chevron entier, bouton × à 40 px, brouillon effacé par ＋ ; `tsc`, `eslint` verts. Poussé par Louis, déployé par Vercel à 15:33 UTC (`e6950ae`, statut GitHub « Deployment has completed ») ; la feuille de style de prod porte la règle en `64rem` (vérifié par `curl`). **Reste à Louis** : regarder l'écran en prod avec un compte de décor, dans un onglet neuf.
...
## Vitesse côté prof — mesures en prod, diagnostic seul (18/09/2026)

Compte prof de Louis dans Chrome, `palimpseste.ink`, que des `GET` : 50 écrans mesurés par `fetch` (1 chauffe + 3 tirs, médiane), Navigation Timing sur l'accueil (document 2,4 s) et la classe 1HLP en vue Compétences (document 3,2 s, `load` 6,3 s, 1 086 Ko décodés / 45 Ko transférés). Vu à l'écran : tableau de bord complet, vue Compétences avec ses deux signaux de citation composée. Lectures serveur : observabilité Vercel 12 h, `pg_stat_statements` en session lecture seule, comptes de lignes PostgREST (`exercices_routes` 3 294, passations de 16/22/23 copies). Aucun code modifié, aucune action serveur jouée. Tableau complet dans `DIAGNOSTIC_lenteur_prof_2026-09-18.md`.

## Vitesse côté élève — cinq lots, éprouvés au bac à sable, mesurés en prod (17/09/2026)

Chaque lot : `tsc`, 2685 tests, passe adversariale (un regard indépendant sur le diff), puis équivalence PAR EXÉCUTION avant le commit — ancien code (worktree de HEAD, `next dev --webpack` sur 3100) contre nouveau (3000), même session par lien magique du décor d'Élo, texte rendu ET suite ordonnée des identifiants comparés dans les trois contextes de classe (T5, Test, Toutes) : 10 écrans au lot 3, 9 au lot 4, 39 au lot 5, tous identiques. Chargeur du déroulé : vue et contexte sérialisés sur 22 dépôts du bac à sable et 138 de prod (lecture seule), 0 différence. Actions : base simulée qui journalise chaque écriture, ancien contre nouveau — `soumettreQuizz` 34 cas, `soumettreNote` 11 cas, 0 écart. Joué pour de bon au bac à sable : un parcours complet du déroulé (cran 4 en paire jusqu'à la correction), 13 notes de cartes Quazian (état et journal écrits à chaque fois), le sondage Aletheia (séance mise en attente puis restaurée : tics de 300 ms, bascule seule, arrêt). Charge : 90 chargements simultanés de l'écran d'exercice, 0 erreur ; 30 accueils simultanés ont révélé que « Mes mondes » n'avait jamais eu d'ordre — trié sur le menu. Mesures en prod après chaque déploiement sur 11 écrans avec le compte d'Élo, sans y écrire. **NON éprouvé, à vérifier à l'œil par Louis** : une soumission Aletheia jusqu'au retour IA, un dépôt Vestigia réel, la proposition d'un thème (trois endroits où un `router.refresh()` jugé redondant a été retiré) ; une soumission de quizz réelle à l'écran ; la prod sous la charge d'une classe entière. À surveiller dans les journaux Vercel : `[acces] modules des classes ILLISIBLES`, `[deroule] … illisible`, `[quazian] score par réponse NON ÉCRIT`.

## État courant — Vestigia : le TIRAGE AU SORT sur une page à projeter, et l'enregistrement oral qui S'ARRÊTE SEUL (3 min en Première, 4 en Terminale) — CODÉ EN LOCAL, non commité, smoke bac à sable fait sur trois tailles (15/09/2026)

Louis, 15/09 : les passages à l'oral ont commencé ; le tirage au sort du pli « Présentation orale » de l'onglet Semaine est bien, mais il faut une **page séparée** que les élèves voient à l'écran (noms qui défilent, nom tiré, petit mot pince-sans-rire, **sujet du fragment** sous le nom), un tirage à la fois (« relancer » pour le second) ; et l'enregistrement doit **s'arrêter au terme** (3 min en 1HLP, 4 min en THLP) avec un son rigolo.

**Mesuré en prod avant de coder** : 2 classes portent Vestigia (`1HLP` niveau `1ere`, 25 inscrits ; `THLP` niveau `terminale`, 16) + « Classe Test ». 36 thèmes du semestre : **27 à 285 caractères, médiane 86** ; noms : médiane 13, max 25. 1 présentation en base (statut `presente`).

**Ce qui est fait** :
- `app/prof/fragments-erudition/tirage/` (`page.tsx` + `SceneTirage.tsx`) : `?semaine=&classe=` ; attente → défilement (70 ms) pendant l'appel serveur, ≥ 1 s de suspense, puis ralenti de 14 noms qui finit sur le tiré → révélation : nom, mot (12 messages, jamais deux fois le même de suite), carte « Ton sujet » (corps en 3 tailles selon la longueur du thème), « Tu as N minutes ». « Relancer » exclut les déjà tirés de la semaine. Bouton « Plein écran » (Fullscreen API sur la scène seule : l'en-tête du site disparaît). Pli « Réglages » fermé par défaut (exclure un élève, liste des tirés avec lien vers la fiche).
- `TirageAuSort.tsx` (pli de l'onglet Semaine) : le tirage en est retiré, remplacé par « Ouvrir le tirage ↗ » (nouvel onglet) ; le suivi des orateurs (A présenté / Reporter / Retirer) y reste.
- `utils/fragments-oral.ts` (pur, 5 tests) : `dureeOralSecondes(niveau)` → 180 pour `1ere`, 240 sinon ; messages ; cadence et suite du ralenti. `utils/carillon.ts` : ritournelle synthétisée en Web Audio (do-mi-sol-do + trille + sifflet descendant), aucun fichier audio.
- `EnregistreurAudio.tsx` : prop `dureeMaxSecondes` (lue sur `inscriptions → classes.niveau` par la page de présentation ; 4 min sans inscription) ; affichage `m:ss / m:ss`, alerte à 30 s, **arrêt automatique au terme + carillon**. Le plafond 10 min disparaît.
- L'action `tirerOrateur` est inchangée. Aucune migration, aucun interrupteur (l'écran remplace l'ancien, décision de Louis).

`tsc` OK, `npm test` 2682 verts, eslint propre.

**Smoke bac à sable** (session prof par lien magique, classe Test / semaine 2, décor posé : 5 dépôts + 6 thèmes dont un de **292 caractères**, marqués `décor smoke tirage 15/09` dans `commentaire_eleve`, laissés en place ; les 2 présentations du smoke retirées) : tirage complet vu à 1024 (Elo → « Ce n'est pas personnel. C'est mathématique. » → sujet → « Tu as 4 minutes (4:00) ») ; exclusion de 5 élèves puis « Relancer » → Sacha, thème de 292 car. rendu à **1280×720, 768, 375 sans débordement horizontal** (scrollWidth = largeur aux trois tailles). ⚠️ À 1280×720 AVEC l'en-tête du site, la ligne « Tu as 4 minutes » finit à 771 px (sous le pli de 720) : en plein écran l'en-tête n'est pas rendu (≈ 230 px de moins) — le plein écran n'a pas pu être exercé dans le volet (`requestFullscreen` refusé), **à voir sur le projecteur**. Enregistreur : micro bloqué dans le volet → stub de flux audio + minuterie ×100 : l'enregistrement s'arrête à **4:00** exactement et passe à « Écouter avant d'envoyer », le carillon est instancié. Le son lui-même et le vrai micro : **non entendus, à essayer en classe**.

**Revue adversariale (15/09, un relecteur, 17 constats) — corrigé** : (B1) l'arrêt automatique vivait dans l'updater de `setDuree` → StrictMode le jouait deux fois, second `stop()` sur un recorder inactif → `InvalidStateError` à la seconde 180 ; désormais un `useEffect` sur `duree`, et `arreterEnregistrement` teste `mr.state`. (B2) aucun `try/catch` autour de `tirerOrateur` : session expirée = noms qui défilent à jamais ; désormais l'erreur s'affiche et la scène revient à l'attente. (G1) le serveur ne se fiait qu'aux exclusions du client : deux onglets « tirage » tiraient deux fois le même élève ; `tirerOrateur` exclut lui-même les présentations de la semaine (`actions.ts`). (G2) toutes les minuteries (défilement, suspense, ralenti) sont suivies, purgées au tirage suivant et au démontage ; garde de réentrance. (G3) la page ne filtrait plus sur `role = 'eleve'` : même ensemble que le serveur. (G4) le pli « Réglages » reste rendu en plein écran (estompé) pour exclure un absent sans sortir. (M1) `aria-live`, `h1` caché, `aria-pressed`. (M4) `ctx.resume()` sur le carillon (iOS). Laissés ouverts : effet de démontage de l'enregistreur sur `[audioUrl]` (préexistant), homonymes dans le ralenti (cosmétique), l'onglet Semaine ne se rafraîchit pas seul après un tirage dans l'autre onglet, pas de contrainte unique `(inscription_id, semaine_id)` sur `fragments_presentations` (migration à journaliser si Louis la veut). Louis : le mot pince-sans-rire passe en **noir** (`text-encre`, 2xl/3xl) pour les projecteurs faibles. `tsc` OK, 2685 tests verts, eslint propre ; smoke bac à sable rejoué après correctifs.

## État courant — Déroulé, cran 4(b) : le passage qu'il fallait surligner est servi ENTRE les deux cas, et EN CONTEXTE — COMMITÉ sur `main` le 15/09, NON poussé, smoke bac à sable fait sur trois tailles (15/09/2026)

Louis, 15/09, sur un signalement élève (dépôt `525744f9…`, T5, exercice `ex-gab-paragraphe-exploitation-voisine-d2-c4b`) : « l'exercice ne donne pas la réponse : c'est une zone à surligner, mais l'élève n'est pas présenté la zone qu'il fallait surligner ». Deux élèves l'ont dit le 14/09 (« la réponse reprend la consigne »), un troisième le 13/09 (« la correction c'est "Les" mais c'est écrit "Les" à deux moments »).

**Mesuré en prod avant de coder** : 266 exercices de cran 4 au gabarit (133 (a), 133 (b)), **266/266 ont la MÊME `reponse_attendue` sur leurs deux cas** — une redite de l'énoncé, jamais la zone. 111 dépôts, 33 rendus, 47 zones jugées. Sur 114 matériaux distincts, la cible dérivée du diff mot à mot tient en **1 mot pour 28** (« Les », « bacs. », « cela »), 4 sont des insertions (diff vide).

**Deux causes, deux correctifs** (`utils/deroule/vue.ts`, `designation.ts`, `marquage.ts`, `components/deroule/EcranDeroule.tsx`) :
1. **Entre les deux cas d'une paire 4(b)/4(b), rien n'était servi** : `passageParCas`, le verdict de zone et sa précision étaient gardés par `v1_remis_at`, qui n'est posé qu'à la dernière crédence ; et l'écran « correction du premier cas » ne les transmettait pas. Désormais ils suivent `correctionDue` (après la crédence DE CE cas — la remise y suffit aussi), et l'écran intermédiaire les reçoit. Le second cas porte un autre matériau : rien ne fuit.
2. **La cible nue ne se montre plus** : `passageAttendu()` rend la PHRASE qui la porte, la cible marquée (même rendu `MateriauMarque` qu'aux crans 3·5) ; sur une insertion, la jointure (les deux mots autour), et le titre le dit. Sur les 114 matériaux de prod : 0 passage nul, 45 à 341 signes, médiane 99. ⚠️ Le JUGEMENT ne bouge pas : `verdictDeLaZone` se compare toujours à l'intervalle nu.

`tsc` OK, `npm test` 2677 verts (8 tests neufs sur `passageAttendu` / `bornesDeLaPhrase`). Aucune migration, aucun interrupteur.

**Smoke bac à sable** (élève de test, décor sur l'exercice `a29a27a4…` = le même qu'en prod, mêmes zones que l'élève, localhost:3000, captures CDP 1280/768/375 dans le scratchpad de la séance) : état « entre les deux cas » → « Ta réponse n'est pas la bonne » + « LE PASSAGE QU'IL FALLAIT SURLIGNER : Cela montre **surtout que les lycées doivent développer la surveillance numérique des élèves.** » ; état final → les deux corrections, la seconde en contexte. Décor retiré après capture.

**⚠️ Point ouvert pour Louis — la sévérité du verdict de zone, mesurée** : au cas 2 du même dépôt, l'élève a surligné la phrase entière (« Cette pratique donne … classe », sans le point final) et l'écran dit « pas la bonne · touche le passage sans le couvrir » parce que la cible du diff commence à « donne » (« Cette pratique » est commun aux deux versions) et finit sur « classe. ». Sur les 47 zones jugées en prod : 19 hors cible, 6 justes, **22 entre les deux (12 « mal bornée », 6 « chevauche », 4 « partielle ») dont 10 deviendraient justes** avec une règle « la zone, bornée aux mots, tient dans la phrase de la cible et la couvre ». C'est la mesure du registre (`issueDuDepot`) : non touchée ici, décision de Louis.

**Revue adversariale (15/09, un relecteur, 14 tests temporaires sur les bornes de phrase et la jointure : aucune exception, cible jamais tronquée)** — un défaut OUVERT PAR LE LOT, corrigé : sur une paire 4(b)/4(b), l'écran de la correction du premier cas montrait le passage attendu alors que le temps était encore « ecrire » : la zone du cas 1 restait modifiable et `enregistrerLaDesignation` ne refusait qu'après `v1_remis_at` — l'élève pouvait recopier la réponse dans sa zone. Fermé aux deux bouts : le serveur refuse une désignation dès que la crédence de ce cas est donnée (`gestes.ts`, garde lue sur l'entrée existante, une seule lecture), et l'écran gèle la désignation dès `credenceDonnee` (`EcranDeroule.tsx`). Vu en bac à sable : plus de boutons sur l'écran de correction. Deux constats laissés ouverts, préexistants ou doctrinaux : (a) sur une INSERTION (cible nulle, 4 matériaux/114), l'élève voit la jointure sans verdict — la règle « une zone qui contient la jointure est juste » (`10-` §5, décision 4) n'est jugée nulle part, ni à l'écran ni au registre ; (b) sur une paire 4(a)/4(b), après la crédence 2 et avant « rendre », le passage du cas 2 est dans la charge utile RSC sans être affiché — la `reponse_attendue` y fuyait déjà. `tsc` OK, 2682 tests verts.

**À faire par Louis** : commit + push (Vercel suit `main`) ; en prod, rouvrir le dépôt signalé depuis l'onglet Assignation (rejeu prof) et vérifier que la correction du premier cas montre le passage.

## État courant — Déroulé des exercices : le SAUVETAGE du texte tapé (brouillon local + minuteur réparé + porte du brouillon) — CODÉ EN LOCAL, non commité, smoke bac à sable fait (15/09/2026)

Louis, 15/09 : « Le problème de la perte de texte d'Aletheia existe aussi dans les exercices. Un de mes élèves a eu le même problème » — bloqué par un retour de fragment non lu, il est parti le lire, son texte est parti avec lui ; on part du principe que rien n'est récupérable, on corrige pour la prochaine fois. Quatre gestes, aucune migration, aucun interrupteur (comme le brouillon Aletheia du 13/09) :

1. **La porte « retour non lu » (et le strike) bloquaient aussi l'enregistrement AUTOMATIQUE du brouillon** — `actionEnregistrerBrouillon` passait par `portier(depotId)` en mode écriture. Et en silence : le `catch` avalait l'erreur, le pied continuait de dire « enregistré tout seul ». Désormais `portier(depotId, 'brouillon')` : les deux gardes transverses ne jouent pas sur le brouillon (un brouillon n'est pas un rendu), la fermeture de la semaine comptée reste. **Décision de doctrine prise par Louis le 15/09.**
2. **Le minuteur de 15 s repartait à CHAQUE FRAPPE** (`texte` en dépendance de l'effet de `ChampDeRedaction`) : un élève qui écrivait sans pause de quinze secondes n'était jamais enregistré. Il lit maintenant la `ref` `courant`, comme `PlanAOrdonner` le faisait déjà.
3. **Le pied dit la vérité** : « pas enregistré [depuis hh:mm] · ton texte est gardé sur cet appareil » en rouge, et le message du serveur sous le champ, dès qu'un enregistrement automatique échoue.
4. **Brouillon LOCAL** (`localStorage`) sur `ChampDeRedaction` et `PlanAOrdonner` : crochet générique `app/eleve/brouillon/useBrouillonLocal.ts` (`useBrouillonLocalParCle`, extrait du crochet Aletheia qui le délègue désormais), clé `deroule:brouillon:<élève>:<dépôt>:<v1|vf>[:<cas>]` (`utils/deroule/brouillon.ts`, 2 tests), élève passé par `PageDuDeroule` → `EcranDeroule` (prop `eleveId`, contexte `IdentiteEleve`, `null` en rejeu professeur). Écrit 300 ms après la frappe et tout de suite au démontage ; **retiré dès que le serveur porte le texte** (`synchroniser`) — ce qui reste sur l'appareil est toujours ce que la base n'a pas ; au montage, un brouillon présent remplace le texte de la base, l'écran dit « Ton texte a été repris du brouillon gardé sur cet appareil », et le prochain tic le porte au serveur.

`tsc` OK, `eslint` OK sur les fichiers touchés (deux erreurs PRÉEXISTANTES ailleurs : `EcranDeroule.tsx:2306` setState dans un effet, `PlanAOrdonner.tsx:93` `Date.now` au rendu), `npm test` **2677 / 2677**.

**Smoke bac à sable** (élève de test, dépôt `1a3dfcae…` ouvert, paire de cran 4, cas 1, localhost:3000 à 1024 px, `scripts/recette/lien-deroule-brouillon.mjs`) : (a) frappe → 1 s → rechargement : le brouillon est sur l'appareil (clé avec `:v1:1`), le champ revient avec le texte ajouté et l'avis « repris du brouillon » — vu trois fois ; (b) après le tic de 15 s la base porte le texte et le brouillon local disparaît ; (c) `fetch` POST coupé dans la page → frappe → 18 s : pied « pas enregistré · ton texte est gardé sur cet appareil », message « Failed to fetch » sous le champ, brouillon local à jour. ⚠️ **Limites vues** : sur plusieurs rechargements en dev, la page est restée NON HYDRATÉE 5 à 17 s (aucun effet ne tourne ; le champ SSR se montre avec le texte de la base) — recompilation Turbopack après mes éditions, ou l'autre séance qui compile sur le même serveur ; une fois hydratée la restauration joue. Pas mesuré en 375 / 768 (le pied est le même `flex-wrap` qu'avant, avec une phrase plus longue). **Non exercé** : la porte « retour non lu » réelle en bac à sable (le décor n'en portait pas), `PlanAOrdonner` (aucun dépôt de forme `ordre` chez l'élève de test), la version finale (`RetourDUnTexte`). Texte de test retiré de la base (237 car., comme avant) et brouillon local effacé. ⚠️ Le navigateur intégré a basculé DEUX FOIS sur une session prof pendant le smoke (`/prof`) — une autre séance se connecte sur le même localhost:3000 ; refaire le lien magique à chaque lot d'essais.

**Revue adversariale du 15/09 (deux relecteurs en lecture seule, un axe chacun ; 17 constats, 10 corrigés, `npm test` 2685 / 2685 avec 3 tests de plus dans `utils/deroule/brouillon.test.ts`)** : (1) ⛔ **un brouillon local plus ANCIEN que la base était restauré par-dessus puis renvoyé au serveur** (téléphone jamais synchronisé, puis deux paragraphes écrits à l'école : le soir, le téléphone remettait le texte court) — le brouillon porte désormais sous `__base` l'état serveur sur lequel il a été pris et n'est restaurable que si la base d'aujourd'hui est encore celle-là, sinon il est jeté (`avecBase` / `estRestaurableSur`, purs, testés) ; (2) `synchroniser` n'annulait pas le minuteur d'écriture de 300 ms → brouillon orphelin égal au texte envoyé, faux « repris du brouillon » au montage suivant — le minuteur est tenu dans une ref et annulé ; (3) enregistrement MANUEL : `sale` baissé après l'`await` effaçait une frappe faite pendant l'envoi — baissé avant, remonté sur échec ; (4) le pied promettait « gardé sur cet appareil » sur la seule présence de la clé, même sans `localStorage` (Safari restreint) — conditionné à `disponible` ; (5) React StrictMode (dev) purgeait le brouillon aussitôt lu — drapeau `restaurationEnCours` ; (6) `pagehide` écouté (onglet fermé, iOS qui suspend) ; (7) `actionOuvrir` restait derrière la porte pleine : un élève bloqué écrivait son brouillon sur un dépôt jamais ouvert et `reelMs` à la remise fabriquait un faux signal « durée très courte » — `actionOuvrir` et `actionMicroQuestion` passent en mode `'brouillon'` ; (8) `enregistrerLeTexte` : compare-and-swap sur `v1_remis_at` / `vf_remis_at` (un tic parti juste avant la remise ne réécrit plus), et le message Postgres va au journal, l'élève lit une phrase fixe ; (9) côté client, un `TypeError` réseau devient « Pas de connexion » (`motifLisible`). **Écartés, nommés** : garde de STATUT dans `enregistrerLeTexte` (préexistant, inventaire des statuts à faire avant) ; bandeau « tu es bloqué » dès l'écran de travail (la vue ne porte pas le blocage — l'élève l'apprend à la crédence, comme avant) ; purge du brouillon après la remise (sans effet aujourd'hui : la remise passe toujours par un enregistrement réussi qui purge) ; relevé de télémétrie dans le brouillon (sous-compte dans le sens de l'élève). ⚠️ **Smoke navigateur de la revue NON concluant** : l'autre séance reprend la session prof sur `localhost:3000` en plein chargement — le HTML arrive en élève, les requêtes suivantes en prof (« Accès refusé », 500) et la page ne s'hydrate jamais ; c'est aussi l'explication des « 5 à 17 s sans hydratation » du premier smoke. Vu tout de même avant la bascule : brouillon écrit AVEC sa base, tic à 15 s, base à jour, brouillon purgé. Le scénario « base différente ⇒ pas de restauration » n'est éprouvé que par les tests purs. Bac à sable remis à 237 car., brouillons locaux effacés.

**À faire par Louis** : commit + push (aucune migration, aucun interrupteur) ; en prod, sur son Chrome, taper dans un exercice, recharger avant 15 s, voir le texte revenir.

## État courant — Shell professeur : le cadre passe de 1152 à 1440 px, en-tête compris — CODÉ EN LOCAL, non poussé (15/09/2026)

Louis, après le smoke prod de l'onglet Assignation élargi : « il faudrait appliquer ça partout sur le site ; est-ce qu'il y a un réglage simple ? ». Oui : UN plafond, `app/prof/layout.tsx` (`max-w-6xl` → `max-w-[1440px]`), et l'en-tête `EnTeteSite` qui suit pour le rôle prof (l'élève reste à 1040, décision de Louis : « je verrai plus tard côté élève »). C'est un PLAFOND : sur téléphone (375) et tablette la page prend déjà toute la largeur, rien ne change — vérifié à 375 sur le tableau de bord. Le contournement local de l'onglet Assignation (`xl:mx-[calc(50%-50vw+24px)]`) est retiré : un seul mécanisme. ⚠️ **Trouvé par la passe de captures** : Vestigia (`app/prof/fragments-erudition/layout.tsx`) débordait déjà du gabarit de 96 px de chaque côté (13/09, pour la vue Semaine) — cumulé au nouveau cadre, la page sortait de la fenêtre (1517 px sur 1451, frise coupée à gauche) ; le débordement est retiré, le gabarit suffit (25 élèves sur deux colonnes à côté du retour). Captures à 1451 × 900 : tableau de bord, Routeur (budgets, assignation), Classes, Élèves, Scriptorium, Vestigia Semaine, Aletheia, Calendrier — aucune page ne déborde (`scrollWidth = innerWidth`), les proses restent bornées par leur `max-w-3xl`. `npm test` 2669 / 2669.

**Revue adversariale rapide du cadre 1440 (15/09, un relecteur)** — deux failles corrigées : (1) la liste des élèves collait à `top-[132px]` sous un en-tête collant de **194,5 px** (mesuré) : ses titres passaient dessous → `lg:top-[211px]`, `max-h-[calc(100vh-227px)]` ; (2) entre 1024 et 1279 px tout s'empilait (régression sur iPad paysage) → deux colonnes dès `lg` (360 px, 440 px à `xl`), vérifié à 1100 (360 + 672) et 1280. Réserves consignées : six autres `sticky` du côté prof passent déjà sous l'en-tête (→ `IDEES_post_rentree.md`) ; Vestigia ne rétrécit à aucune largeur (avant : 1296 utiles dès 1400 ; après : 1352) ; trois pages « copie » restent bornées à 1152 sous un en-tête à 1440 ; aucun smoke ne capture au-delà de 1280. ⚠️ Une autre séance travaille sur l'arbre (brouillon local du déroulé, `EcranDeroule.tsx` modifié) : commits par fichiers nommés seulement.

## État courant — Routeur : l'onglet « Assignation » REFAIT (classes → élèves → exercices → les écrans de l'élève) — CODÉ EN LOCAL, non commité, smoke bac à sable fait sur trois tailles (14/09/2026)

Louis, 14/09 : « voir les exercices que mes élèves font, ainsi que leur taux de succès, au fur et à mesure » — wireframe puis rendu charte validés, puis : « pas d'interrupteur, pas d'onglet neuf : ce travail remplace l'onglet Assignation, il fait la même chose et plus ; le retrait reste ». **Sans porte, par décision de Louis** (la colonne `suivi_exercices_actif` posée puis retirée du bac à sable le même soir ; aucune migration ne reste).

Ce qui est codé : `app/prof/routeur/suivi-serveur.ts` (une semaine, toutes les classes, six lectures paginées + le registre des réussites lu par lots — `lireLesDepotsPourLeRegistreDesDepots`, même règle `issueDuDepot`), `VueSuivi.tsx` (tuiles classes ; liste des élèves à gauche : exercices, réussis / rendus, prévu · passé, compétences ; à droite les exercices de l'élève cliqué avec compétence, clé = objet + observable, cran, réussite, prévu · réel, retrait), `PanneauEcrans.tsx` + `FriseDesEcrans.tsx` (les écrans de l'élève rejoués en lecture seule, une tuile par écran, `utils/deroule/rembobinage.ts` pur + prop `etapeForcee` sur `EcranDeroule`). `VueAssignation.tsx` supprimé ; `chargerAssignation` reste (testé). `tsc`, `eslint` (hors une erreur préexistante d'`EcranDeroule.tsx:2263`), `npm test` **2658 / 2658**.

Mesuré en prod avant de coder (PostgREST, lecture) : T5 = 23, 1HLP = 25, THLP = 16 élèves ; 3 à 9 exercices par élève et par semaine ; noms 13 car. en médiane ; temps ouvert → v1 médiane 10 min mais 15 / 140 dépôts laissés ouverts > 1 jour ; `propositions_iso_duree` sur 31 / 300 décisions seulement ; `id_import` 39 car. en médiane. **Trois arbitrages** : temps passé plafonné à 3 h (« laissé ouvert ») ; temps prévu = `exercices_types_crans.duree_exercice_min` (comme l'écran élève) ; clé courte = objet, `id_import` en survol. **Un quatrième, contre la maquette** : la fenêtre des écrans prend TOUTE la largeur sous les deux colonnes — à 640 px le déroulé se coupait à gauche et le champ d'écriture tenait sur 40 px.

**Revue adversariale du 14/09 (trois relecteurs, un axe chacun, constats vérifiés puis corrigés — `npm test` 2664 / 2664 avec 6 tests neufs `utils/deroule/rembobinage.test.ts`)** : (1) la semaine se bornait en UTC quand l'assiduité, « Ma semaine » et le journal du retrait la bornent dans le fuseau — corrigé par des instants (`debutDuJourDansFuseau`) ; en bac à sable 7 dépôts sur 25 changent de semaine, en prod 0 dépôt posé un dimanche soir depuis le 31/08 ; (2) « rendu, sans verdict » (crans 6·8, juge absent, examens : **18 rendus sur 140** en prod, semaine du 07/09) était compté et peint RATÉ — troisième compte `sansVerdict`, taux sur les rendus jugés, barre à trois segments ; (3) les dépôts retirés disparaissaient (l'ancien écran les montrait « retiré par vous ») et le retour du retrait ne s'affichait jamais — gardés hors comptes, `details` affichés ; (4) la frise gardait son index d'un exercice à l'autre (Next ne remonte pas sur un changement de `?depot=`) et débordait → `key={depotId}` + borne ; (5) changer de classe gardait l'élève de l'autre → `key={classe.id}` ; (6) `?semaine=2026-13-45` plantait la page → `Date.parse` ; (7) rembobinage : la correction du 1er cas n'était JAMAIS gardée (la suite la porte avec `cas: 1`, pas `null`), les crans guidés rejouaient une colonne vide, les pages crédence/gestes montraient un formulaire vierge (curseur à 50 %) au lieu de la déclaration — encarts de lecture en rejeu ; une semaine COMPTÉE (C10-L1) rejouait une suite inventée → `sansFermeture` côté professeur ; `SeJuger` appelait une action élève en rejeu → gardé ; les `.in()` de 300+ uuid (400 muet mesuré à 700) → tranches de 200 ; `.order('id')` en second critère de pagination ; les dépôts de CLASSE marqués « en classe », sans lien. Plus de présélection du premier élève (« lorsque je clique »). **15/09 — poussé et déployé par Louis, smoke PROD fait sur son Chrome (lecture seule)** : tuiles de la semaine du 14/09 (1HLP 181 assignés, T5 100 · 16 rendus · 46 %, THLP 110 · 10 · 60 %), T5 ouverte, un élève, ses six exercices, la frise sur une VRAIE paire de cran 4 (10 écrans : surlignage du cas 2 rejoué avec la zone de l'élève, « Crédence déclarée : 90 % », le retour de v1 avec sa copie) ; aucune erreur console. Retour de Louis : « la section “les écrans de…” devait être à droite, elle est en dessous ; des colonnes larges perdues de chaque côté ». Correctif : l'onglet sort du `max-w-6xl` du layout (`xl:mx-[calc(50%-50vw+24px)]`), liste à gauche 400 px (noms qui se REPLIENT au lieu de se tronquer — vu en prod sur un nom composé), exercices puis écrans dans la colonne de droite (983 px à 1451 px), cadre du rejeu réduit, tuiles de la frise sur deux rangées, motif des sondes sans `_`. Vérifié en local à 1451 × 840. Sous `xl` (1280), tout s'empile comme avant. Non traité : les tuiles « Classe Test » et « Hors classe » (1 élève chacune) restent à l'arrivée.

**Suites demandées par Louis le 14/09, faites le soir même** : (a) **la version finale est dans la frise** — après « Rendre », les écrans « Ton retour », « Ta version finale », « Rendre la version finale », « Ton retour final » (au régime plein, jamais sur une paire où `texteVf` est la réponse au second cas) ; `ecransRejoues` / `vueALEcran` (`rembobinage.ts`), `vfRemiseLe` ajouté à la vue, 3 tests de plus (2667 / 2667) ; vérifié sur Élo (8 écrans, « Ton retour » rejoue le retour de v1 avec sa copie) ; (b) **les informations de l'ancien écran** (règle déclenchée, dégradé, sondes silencieuses et de montée, « sans cible de routeur », origine, échéance) **sur la ligne de l'exercice, dans le panneau de droite** ; (c) **le plafond 1000** : PostgREST rend 1000 lignes MAX par requête quel que soit le `limit` — le registre lisait les 1140 matériaux à `id_import` de prod TRONQUÉS à 1000, donc ~140 matériaux de cran 2 sans leur devoir ; effet : « deux et deux » peut compter deux réussites sur le MÊME devoir comme deux devoirs distincts et débloquer un cran trop tôt — pas cet écran, le ROUTEUR ; **corrigé** par une lecture paginée (`range`, tri par `id`) dans `reussites-serveur.ts` ; (d) `SUIVI_SQL.md` : les trois lignes cochées Prod « constatée 14/09 » sur une LECTURE PostgREST de la prod (colonnes présentes, 206), la date d'exécution restant non journalisée. **Restent, nommés** : un élève bi-classe compte dans ses deux tuiles (comme l'assiduité) ; sur téléphone la bascule Lire/Écrire du déroulé rejoué est inerte (volet choisi par l'écran). ⚠️ Vu au smoke : après un HMR, l'ancien payload RSC en cache a fait lever `sondes.some` sur une ligne sans le champ — un onglet neuf n'a aucune erreur ; en prod, un onglet ouvert AVANT le déploiement appellera l'ANCIEN déploiement (mémoire), à recharger.

Smoke en bac à sable (`semaine=2026-08-24`, décor Élo) : tuiles, classe T5, élève, exercice, frise à 7 écrans, écran 1 rembobiné (consigne + sujet + passage + texte de l'élève, « 1/6 »), écran 7 « où il en est » ; téléphone 375 et tablette 768 vérifiés après deux corrections de repli. ⚠️ **Non exercé** : le retrait depuis la nouvelle ligne (même action `retirerLExercice`, mêmes champs `depot_id` / `motif`) ; les crans à désignation (4, 7, 9) et les paires dans la frise — le décor du bac à sable n'en portait pas cette semaine-là. ⚠️ Cliquer un autre élève vide la fenêtre des écrans (état d'écran) : voulu. **À faire par Louis** : regarder en local ou pousser ; le smoke prod sur une vraie classe (T5, semaine du 14/09) dira si la frise tient sur une paire de cran 4.
...
Éprouvé en bac à sable avec un décor temporaire (deux sous-chapitres « DÉCOR 2.1 / 2.2 » sous le chapitre 2 de « NAture humaine », classe Test, Louis connecté en prof sur localhost) : chapitre 2 déplacé en semaine 3 par le sélecteur « sem. » → base : sem. 3 ordre 1 chapitre, 2 et 3 les sous-chapitres ; écran conforme, aucune erreur console. Décor retiré (2 éléments, 2 sections), chapitre 2 remis en semaine 4 ordre 2. **Poussé et déployé par Louis le 14/09 (`b3ff72e`), vérifié en prod sur 1HLP** : « 5 Socrate devant ses juges » ramené en semaine 3 puis renvoyé en semaine 4 ; base : sem. 4 → 1 Conclusion, 2 le chapitre, 3-5 ses trois sous-chapitres ; écran « 0/3 sous-chap. » sous le chapitre, semaine 3 passée de 18 à 14 chapitres.
...
- **Fait** : colonne `annonce` + CHECK (`plan_evaluation_annonce.sql`, jouée en bac à sable, 0 annoncé après) ; action `annoncerExercice` ; bouton « Annoncer aux élèves » / badge « annoncé aux élèves » + « Taire » sur chaque examen (évaluatif, en classe) dans la grille du plan ; le calendrier élève émet le créneau annoncé sous son libellé générique. Déplacer, reporter et « à caler » remettent `annonce` à false.
- **Vérifié en bac à sable (dev local)** : prof T5 → clic « Annoncer » sur l'examen de lecture du 25/08 → badge, base à `true` ; élève de T5 (lien magique) → vue mois : « Examen diagnostique — lecture » le 25/08, l'examen d'écriture non annoncé du 28/08 absent ; vue « À venir » à 375 px : idem, lisible. Contrainte éprouvée au `psql` : annoncer un quiz sans jour ⇒ refus `exercices_annonce_chk`. `tsc` propre, 2658 tests verts. Bac à sable remis à 0 annoncé.
- **Non fait** : commit, push, SQL prod — gestes de Louis. La légende du calendrier élève ne nomme pas la pastille « prévu ». Le bouton reste cliquable sans jour calé (la garde serveur explique) — pas de désactivation visuelle au-delà de la couleur grise.
- **Revue adversariale (14/09, agent, 8 constats), corrigés** : (1) `retirerExercice` et (2) `recalerExercice` → `annuler` écrivaient `statut='annule'` sans toucher `annonce` ⇒ 23514 brut sur un examen annoncé — `annonce: false` ajouté ; (3) `retirerEssai` (Fragments) idem, erreur non testée ⇒ ligne de plan zombie — corrigé et testé ; (4) `regenererPlan` soft-supprimait un examen annoncé en silence — un annoncé sort du périmètre et bloque sa clé, comme un conçu ; (5) le journal SQL disait « prod après le code » : le code n'est PAS tolérant à l'absence de colonne — **migration d'abord, push ensuite** ; (6) un examen annoncé à une classe SANS le module (bac blanc → codex) posait un badge prof sans rien côté élève — `annoncerExercice` refuse et le dit ; (7) un quiz annoncé passe outre « surprise » (D5) — voulu, dit dans le `title` du bouton. (8) `app/prof/scriptorium/actions.ts` porte un diff d'une AUTRE séance (`deplacerElement`) — pas dans ce lot. Éprouvé au `psql` en transaction annulée : annulé + annonce=false accepté. `tsc` propre, 2658 tests verts.

## État courant — Vestigia : analyse d'un fragment en `erreur` à chaque relance — RÉPARÉ, DÉPLOYÉ, vérifié en prod (14/09/2026)

- **Symptôme** : un dépôt en retard de trois pages (S3, THLP) tombait en `erreur` à chaque relance, sans message. Les 29 autres analyses depuis le 12/09 étaient `publiee` : panne propre au dépôt.
- **Cause** : `max_tokens: 4096` dans `utils/analyse.ts`. Les analyses qui passent font déjà jusqu'à 10 500 caractères ; trois pages denses dépassent le plafond, le JSON tronqué ne se parse plus. Aucun des trois chemins d'échec n'écrivait de journal.
- **Correctif** (`318a7b7`, déployé par Louis) : plafond à 8192, `console.error` avec l'identifiant du dépôt sur chaque échec.
- ⚠️ **Piège rencontré** : la première relance après déploiement a encore échoué — les journaux Vercel montraient qu'elle tournait sur l'ANCIEN déploiement. Un onglet ouvert avant un déploiement envoie ses actions serveur à l'ancienne version. Recharger l'onglet (Cmd+Maj+R), puis relancer : l'analyse est passée.
- Non fait : dimensions en pixels des photos non mesurées (téléchargement refusé par le classificateur) ; aucun redimensionnement avant l'envoi à l'IA.

## État courant — Scriptorium : le DECK DE PRÉSENTATION d'un cours — POUSSÉ `18e0b56` par Louis, migration jouée en bac à sable ET en prod le 14/09 ; revue adversariale jouée et corrigée (14/09/2026)
...
- `scriptorium_presentation.sql` — UNE colonne `scriptorium_contenus.presentation_ref text null` (chemin `<prof>/<contenu>/presentation-<horodatage>.html` dans le bucket privé `scriptorium`). **Jouée en bac à sable le 14/09** (12 contenus intacts, 0 deck) **et en prod le 14/09** sur ordre de Louis, après son push (13 contenus intacts, 0 deck). Rollback fourni. Lecture tolérante (`presentationsDesContenus`) : le code peut partir avant le SQL — et si la colonne manque au dépôt, le fichier est retiré et le message le dit.
- `actions.ts` — `deposerPresentation` (cours vivant seulement, `.html`/`.htm`, 8 Mo max, **nom versionné** pour ne pas attendre le CDN de Supabase, ancien fichier retiré), `retirerPresentation`, `presentationsDesContenus` ; la **purge de la corbeille** retire aussi le deck du bucket.
- `LigneContenuBiblio.tsx` — en édition, une ligne « Présentation (HTML) » : Déposer / Remplacer / Retirer, l'édition se referme après dépôt ; en lecture, pour un cours qui en a un, « Présenter » et « Onglet ↗ ».
- `presentation/[contenuId]/page.tsx` + `Presentation.tsx` — cadre plein-page au-dessus de l'en-tête (`z-50`), barre discrète (retour, titre, Onglet, **Plein écran** masqué si le navigateur ne le sait pas, touche F) ; l'iframe reçoit le focus au chargement ; **`sandbox="allow-scripts allow-popups allow-modals allow-downloads"`**, sans `allow-same-origin`.
- `presentation/[contenuId]/fichier/route.ts` — **sert le deck PAR L'APPLICATION**, avec `Content-Security-Policy: sandbox …` (origine opaque même dans un onglet ou en fenêtre présentateur). ⛔ Mesuré le 14/09 : le stockage Supabase rend un `.html` en `text/plain` avec `Content-Security-Policy: default-src 'none'; sandbox`, URL signée comprise — le navigateur montre le code source.

**Le deck réel de Louis (`S1 - C2.html`, 1,1 Mo) mesuré avant de choisir le bac à sable** : aucun `localStorage`/`sessionStorage`/`cookie` ; polices Google en ligne ; touche F → `requestFullscreen` ; fenêtre présentateur par `window.open(location.href + '#presenter')` et `postMessage(…, '*')` dans les deux sens ; export de lui-même par téléchargement (`a.download`). D'où les quatre jetons du bac à sable, et aucune restriction réseau.

Vérifié en bac à sable, session prof par lien magique, cours « Cognitif », avec un deck synthétique qui imite ces quatre traits : dépôt → l'édition se referme, « Présenter » et « Onglet ↗ » apparaissent ; remplacement → **un seul fichier `presentation-<horodatage>.html` dans le dossier**, l'ancien retiré ; route 200 `text/html` + CSP sandbox. **Dans un Chrome sans fenêtre piloté par CDP** (le volet intégré bloque la requête d'une iframe en bac à sable — `ERR_BLOCKED_BY_CLIENT`, un blocage du volet, pas du serveur) : cadre chargé (200), flèches reçues sans clic, `document.cookie` et `window.parent` → `SecurityError` (isolation prouvée), police Google chargée, fenêtre présentateur ouverte, messages aller et retour reçus, **touche F du deck → plein écran effectif** (`fs:ok`, `fullscreenElement = IFRAME`, barre masquée). Deck de test retiré (0 fichier restant). `tsc`, `eslint`, `npm test` (2 658) verts. Aucune écriture en prod.

**Revue adversariale (agent, 14/09) : 0 bloquant, 3 gênants, 7 mineurs — tous corrigés** (ordre code/migration, origine du deck, purge orpheline ; nom versionné, multi-prof, édition refermée, corbeille, iPhone, focus, requête dans le `Promise.all`).

**Limites** : le bouton « Plein écran » de la barre n'a été éprouvé que par la touche F du deck lui-même ; sur iPhone Safari, il n'y a pas de plein écran, le bouton disparaît et « Onglet » reste. Un deck qui utiliserait `localStorage` se casserait dans ce bac à sable (mesuré : le tien n'en a pas).

## État courant — Aletheia : brouillon LOCAL des formulaires V1 et VF — COMMITÉ (`bbd9a67` + revue), non poussé (13/09/2026, nuit)
...
**Revue adversariale (agent, 13/09) : 4 gênants, 4 mineurs, 6 corrigés dans un second commit.** `window.localStorage` qui LÈVE à l'accès (Safari « bloquer tous les cookies ») faisait afficher « L'envoi a échoué » sur un rendu ACCEPTÉ → accès par un `stockage()` qui rend `null` ; les cases « je ne sais pas » étaient DÉDUITES des champs et se recochaient à tort (une thèse écrite après un « je ne sais pas » refermé partait en `texteJeNeSaisPas`) → sauvées telles quelles ; la position dans le fil est sauvée ; rien n'est écrit au montage (les valeurs de la base ne sont pas un brouillon) ; la frappe en attente s'écrit au démontage ; la fusion prend le brouillon là où il porte la clé, même vide (un champ effacé reste effacé — arbitrage) ; revenu à l'état de la base ⇒ brouillon retiré. Non repris : deux onglets sur la même séance (pas d'écoute `storage`).

Vérifié en bac à sable (élève de test, *Qu'est-ce que les Lumières ?* séance 1, formulaire VF fil), rejoué après la revue : aucune clé écrite à l'ouverture sans frappe ; frappe à l'écran 2 puis départ IMMÉDIAT vers Fragments → clé écrite avec `index: "1"` ; retour → texte et écran 2/3 restaurés. : texte tapé dans le premier champ → clé écrite dans `localStorage` avec les trois champs ; navigation vers Fragments puis retour sur la séance → le champ montre le texte tapé, pas la V1 de la base ; aucune erreur console. `tsc`, `eslint` propres ; brouillon de test purgé du volet. **Écran des réponses aux relances (`ReponsesRelancesFil`) couvert dans un troisième commit** : même hook, phase `relances`, réponses et position sauvées (les surlignages sont déjà en base à chaque vérification) ; vérifié en bac à sable (réponses de l'élève de test remises à `null`, état d'avant) : aucune clé au montage, réponse tapée à la relance 1 puis départ immédiat → clé écrite, retour → texte et écran 2/7 restaurés. **Revue adversariale de l'écran des relances (agent, 13/09) : 0 bloquant, 2 gênants corrigés (quatrième commit).** (1) Au téléphone, une relance avec fenêtre de texte rouvrait sur « Le texte », la réponse cachée et Suivant invisible → au restaure, toute relance déjà répondue rouvre sur « Ta réponse » ; (2) la clé ne portait pas l'identifiant du travail alors que les réponses répondent aux relances d'UN retour → `cleBrouillon(…, travailId)` pour la phase `relances`. Non repris : le brouillon s'écrit dès que l'élève avance dans le fil sans rien taper (c'est la position voulue) ; flash de l'écran 1 avant le saut restauré (`FilEcrans` défile en lissé, comme les autres fils) ; brouillon orphelin si l'envoi a réussi sur un autre appareil. Rejoué au bac à sable à 375 px : texte et écran 2/7 restaurés, clé suffixée de l'identifiant du travail, rien ne déborde ; la relance de test n'a pas de fenêtre, la bascule n'a pas été vue à l'écran.

**Limites** : le formulaire V1 n'a pas été rejoué dans le navigateur (même hook, mêmes tests) ; le brouillon ne survit pas à un changement d'appareil.
...
**Smoke prod (13/09, après le push de `66fd3be` par Louis, sur son Chrome)** : T5 · Philosophie des sciences — semaine 1 : le cours « Qu'est-ce que la Connaissance ? » (12 chapitres) tient sur 5 lignes, « 3/3 sous-chap. » et « 4/4 sous-chap. » repliés ; titres seuls, numéros 1 à 5 ; semaine 2 ouverte par défaut ; aucune erreur console. **Un défaut trouvé** : sous « 5 Conclusion » (semaine 2) se rangeaient « 4.4 » et « 4.5 », sous-chapitres du chapitre 4 resté en semaine 1 — `arbreChapitres` rattachait un niveau 2 au dernier chapitre de la semaine sans regarder son numéro. Corrigé (`6035efd`, poussé et déployé par Louis le 13/09). **Revérification en prod jugée bonne à tort** : l'arbre ne rattachait plus « 4.4 » et « 4.5 » à « 5 Conclusion », mais un sous-chapitre orphelin gardait le MÊME retrait qu'un enfant — à l'écran, rien ne changeait, et Louis l'a vu. Seconde correction : l'orphelin perd son retrait et porte un jeton « chap. 4 · sem. 1 » (le chapitre est cherché dans les autres semaines du même créneau). **Revérifié en prod après le push de `2c7420a`** (13/09, tard) : en semaine 3, « 5.1 » à « 5.3 » sont alignés sur les chapitres, chacun avec le jeton « chap. 5 · sem. 2 » ; « 6 Synthèse (2) » suit. Même rendu pour « 4.4 » / « 4.5 » en semaine 2. Le chantier est clos ; l'ordre « 5, 4.5, 4.4 » d'une semaine reste l'ordre enregistré (flèches ↑ ↓), volontairement non retrié.

## État courant — Vestigia « Semaine » (prof) : un seul écran, frise + classes + retour validé sur place — DÉPLOYÉ, smoke prod fait (13/09/2026, tard)

**Déployé par Louis en deux temps le 13/09 au soir (le lot + les 13 constats, puis la courbe et les rangs à 30 px).** Dernier smoke en PRODUCTION après le second déploiement, lecture seule (aucun Publier / Relancer / Fermer), aucune erreur console :
- **1HLP à 1440 × 900** : 25 élèves sur deux colonnes, rangs mesurés à 30 px, la liste tient dans la fenêtre (seuls le tiroir du tirage et la ligne de bilan passent dessous) ; Audrey Lauriston (3 photos) : vignettes, notes C·C·B, courbe « Semaine 3 · C » chacune à sa place, `scrollWidth = clientWidth` sur le panneau, liste et panneau d'accord.
- **THLP à 1440** : 16 élèves, 7 déposés / 9 manquants, lettres de la liste = retours publiés ; signaux sur les segments (point rouge THLP, pastille « 1 à valider » sur 1HLP).
- **Téléphone 375 × 812** : aucun débordement horizontal (`scrollWidth 375`) ; frise défilante, trois segments sur une ligne, un rang par élève ; feuille de Matthias Merk (2 photos, A·B·A, courbe avec numéros, volets, pied Dépublier / Enregistrer / Relancer visible). Seul rognage détecté : le titre italique du sceau dans l'en-tête, troncature voulue, hors lot.
- Tous les commits sont sur `origin/main`. Rien en attente pour ce lot.

Les paragraphes ci-dessous sont l'historique de la construction.
...
- `page.tsx` — la frise du semestre (`FriseSemaines.tsx`, liens `?semaine=&classe=`) ; la semaine choisie dessous (`utils/fragments-semaine-courante.ts`, pur, 8 tests : ouverte > contient aujourd'hui > dernière commencée > première comptée ; une semaine de vacances ou inconnue par l'URL n'est jamais substituée en silence — autre semestre suivi, id inconnu → 404) ; les trois classes lues par lots (inscriptions, profils, dépôts, analyses, présentations bornées au semestre ; 1 requête `head` par semaine pour les comptes de la frise, à cause du plafond PostgREST). `semaine/[id]` redirige (liens de l'écran d'analyse et des présentations conservés).
- `VueSemaine.tsx` — classes en segments (déposés/inscrits, pastille ocre « à valider », point rouge « manquants », lisibles sur toutes les classes) ; filtres Tous / À valider / Manquants / Publiés ; liste sur deux colonnes (25 élèves sans défiler ; le module déborde de `max-w-6xl` de 96 px par côté à ≥ 1400 px, `layout.tsx`) ; validation par lot inchangée (`utils/validation-lot.ts`) ; tirage au sort en tiroir ; Échap / ← / → hors champ de saisie ; garde `confirm` si une saisie n'est pas enregistrée.
- `PanneauRetour.tsx` + action `chargerRetourDepot` — le retour complet chargé en UN aller-retour à l'ouverture (dépôt, photos signées, analyse, pistes, signaux, fil du semestre) ; notes E→A, six textes en volets, Publier / Enregistrer / Dépublier / Relancer sur place ; feuille depuis le bas au téléphone, colonne collée en haut sur grand écran, hauteur mesurée pour que Publier soit visible sans défiler ; se réaligne après une publication par lot ; « Ouvrir en grand » garde l'écran d'analyse. Le type `RetourDepotComplet` vit dans `types/fragments.ts` (piège `export type` dans `'use server'`).

Vérifié en bac à sable (classe Test, S2 : 2 dépôts, 1 à valider, 1 publié) : **1440 / 768 / 390 px**, aucune erreur console ; publier depuis le panneau → « publiée ✓ », lettres dans la liste, compteurs à jour ; dépublier → retour à « à valider » (bac à sable remis dans son état) ; ← → Échap ; l'ancienne route `/semaine/[id]?classe=` redirige. `tsc`, `eslint`, 2 650 tests verts.

**Passe adversariale (agent, 13/09) : 17 constats, 4 graves, 12 corrigés dans la foulée** — Échap depuis un champ ne ferme plus, garde sur saisie non enregistrée ; `.catch` + `try/finally` sur le chargement et les quatre gestes (plus de squelette ni de bouton grisé à jamais) ; `?semaine=` d'un autre semestre suivi, inconnu → 404, vacances ignorées ; la classe suit le changement de semaine ; flèches vivantes quand l'élève ouvert sort du filtre ; panneau réaligné après un lot ; « Lancer l'analyse » sur un dépôt sans analyse guette quand même ; présentations bornées au semestre ; même dénominateur (inscriptions) frise et segments ; signaux d'attention sur tous les segments ; `revalidatePath` de l'ancienne route rebasculés ; `role="dialog"`, focus au panneau, nom accessible des cases. Volontairement non repris : piège du focus (Tab) derrière la feuille au téléphone ; 17 requêtes `head` par rendu (42 inscrits, coût négligeable) ; `chargerRetourDepot` vérifie le rôle, pas la propriété du dépôt (mono-prof, comme le reste du module). **Limite** : le bac à sable n'a que 7 élèves et 2 dépôts ; la liste à 25 et le panneau sur un vrai commentaire de 737 car. sont à regarder en prod sur 1HLP après déploiement.

**Seconde passe adversariale (agent, à froid sur le commit, 13/09 tard) : 13 constats, TOUS corrigés** (commit suivant) — garde de saisie étendue à tout lien (frise, onglets, « ouvrir en grand », capture du clic) et à la fermeture de l'onglet (`beforeunload`) ; « Relancer » demande confirmation si une saisie est en cours ; la pile de lot suit le FILTRE (changer de filtre vide la sélection, la case d'en-tête ne coche que les lignes visibles — sinon « Manquants » laissait publier douze retours invisibles) ; la frise lit `?classe=` dans l'URL courante (`useSearchParams`, composant client) ; `?semaine=` sur des vacances ou un semestre hors liste → 404, plus de substitution ; bandeau + bouton pour réaligner le sélecteur de semestre quand un lien vise un autre semestre ; attente d'analyse bornée à deux minutes ; pas de pastille « E » sans analyse lisible ; liserés sans astuce `nth-child` ; hauteur du panneau remesurée au défilement ; zone sûre iPhone sous « Publier » ; `lg:static` retiré ; dénominateur de la frise = élèves distincts. Recontrôlé en bac à sable à 1440 px (ouverture, ← →, aucune erreur console). **Smoke complet à faire avec Louis.**

**Déployé par Louis le 13/09 au soir ; smoke en PRODUCTION (lecture seule, aucun Publier/Relancer/Fermer) le 13/09 tard.** Vu à 1440 px : la frise des 17 semaines, 1HLP (25) sur deux colonnes — 13 rangs resserrés à 30 px sur demande de Louis (34 px faisaient défiler la page d'environ 160 px) —, THLP et Classe Test en segments avec leurs signaux, les filtres, l'ouverture d'un élève, ‹ › ; à 390 px : la feuille, les trois photos, les notes, les volets. Aucune erreur console. **Défaut repéré par Louis, reproduit et mesuré** : à TROIS photos (2 en médiane, 4 max), la bande photos + notes + courbe dépassait les 600 px du panneau et la courbe « Semestre » recouvrait les lettres de la ligne Réflexions. Corrigé (commit suivant, non poussé) : vignettes 58 × 78, notes à côté, la courbe sur SA ligne (une barre par retour publié, sa lettre au-dessus, les numéros de semaine dessous, la semaine courante en pigment plein). Vérifié en bac à sable dans un Chrome sans fenêtre à profil isolé (le volet de la séance portait la session élève d'une autre séance) à 1440 et 390 px : aucun débordement (`scrollWidth = clientWidth`). ⚠️ Constaté en prod pendant le smoke, sans explication dans le code : la note Réflexions d'un dépôt 1HLP est passée de B à E (`updated_at` 20:52 heure de Montréal, statut resté « publiée ») entre deux rendus de la page — panneau et liste sont d'accord avec la base après rechargement. Louis confirme : c'est lui qui l'a changée.

## État courant — Scriptorium « par classe » : plis semaine › cours › chapitre — CODÉ EN LOCAL, non commité (13/09/2026)

Demande de Louis : la vue `?vue=classes&classe=…&instance=…` défilait sans fin dès qu'une classe avait beaucoup de chapitres, et deviendrait illisible avec plusieurs parcours par classe. Mesure en prod par PostgREST AVANT le dessin : un cours pose jusqu'à **25 chapitres dans une seule semaine** (T5, S1 ; 42 chapitres sur 3 cours), 16 en S2 pour 1HLP, 13 en S2 pour THLP ; titres de chapitres médiane 22 car., max 62 ; titres de cours 20 à 41 car. Le scroll venait de là : une ligne par chapitre, toujours dépliée, six commandes par ligne.

Wireframe (canevas Claude Design, option A retenue par Louis) puis implémentation, **2 fichiers, aucune migration, aucun interrupteur** (présentation seule) :
- `app/prof/scriptorium/GrilleInstance.tsx` — en-tête compact (jetons durée / début / horaire / suivi) ; rail des semaines à gauche sur grand écran (pastilles défilantes au téléphone) ; **une semaine repliée = une ligne** (bilan « k/n vus » ou « vide », résumé des cours) ; **un cours replié = une ligne** (titre, N chapitres, barre d'avancement, « ✓ tout vu », ⇈ ⇊, ✕ — les gestes du créneau quittent les lignes de chapitres) ; le chapitre ne garde que sa case « vu », ses jetons, ↑ ↓ et « sem. ▾ » ; Calendrier et Modèle deviennent deux tiroirs repliés sur une ligne d'état (le tiroir Calendrier s'ouvre seul quand il n'y a pas de date). Ouvert par défaut : la semaine courante et, dedans, le cours qui porte le prochain non-vu ; état des plis mémorisé par navigateur (`localStorage`, `useSyncExternalStore`, pas de `<details>` natif — cf. piège d'hydratation du 30/08) ; un autre jour, la semaine courante se rouvre.
- `app/prof/scriptorium/page.tsx` — classes en **onglets** sur une rangée au lieu de la grille de tuiles ; parcours assignés en **lignes** (une par parcours) ; livres et synthèses élève en `<details>` repliés, plan d'évaluation en une ligne.

Vérifié en bac à sable sur l'instance « test » (5 créneaux, 9 éléments, seule instance sandbox avec des chapitres) : **1440 / 768 / 390 px**, aucun débordement horizontal (scrollWidth = 390), aucune erreur console ; correction en cours de route d'un chevauchement à 390 px (compteur du cours sous « ✓ tout vu ») et de l'accord « 1 vus ». `tsc` et `eslint` propres. **Limite** : le bac à sable n'a aucun cours à 25 chapitres, le bloc à fort volume n'a été vu qu'à 2 chapitres ; à regarder en prod sur T5 après déploiement. Non fait, volontairement : la frise de l'année de la classe (planche 1 du wireframe) — ajout, pas réorganisation, à décider à part.

**Passe adversariale (agent, 13/09) : 21 constats, 0 bloquant, 6 corrigés dans la foulée** — magasin de plis mémoire d'abord (un `setItem` refusé rendait les plis inertes) ; « ✓ tout vu » rafraîchit même si un appel échoue à mi-chemin ; les tiroirs Livres/Synthèses de la page classe passent par un composant client `Tiroir.tsx` au lieu d'un `<details>` (piège de restauration Chrome) ; jour local `fr-CA` au lieu d'UTC pour la réouverture de la semaine courante ; cibles de tap à 44 px au téléphone (lignes, cases, flèches, ✕) ; `aria-controls` seulement quand la semaine est ouverte ; `coursDefaut` vise un cours à plusieurs chapitres ; `title` de « plus au modèle » rétabli sur les éléments seuls. Volontairement non repris : décaler / ajouter un contenu ne se montrent que sur une semaine ouverte. Recontrôlé à 390 px après corrections.

## État courant — analyses de fragments en échec visibles au tableau de bord (13/09/2026)

Demande de Louis : deux dépôts THLP ont échoué à l'analyse sans qu'aucun message n'apparaisse dans le « À faire ». Mesure en prod par PostgREST avant de coder : 1 analyse en `erreur` (dépôt du 12/09 23:32 UTC), 0 `en_cours`. Cause : `app/prof/page.tsx` ne lisait que le statut `generee`.

**Poussé par Louis le 13/09 (`origin/main` = `39b97aa`), déployé et constaté fonctionnel par lui.** Correctif : le tableau de bord charge aussi `erreur` et `en_cours` de plus de 30 min ; héros « N analyse(s) de fragments en échec » placé entre l'examen humain et les fragments à valider, ligne « à relancer » dans « À préparer » sinon, compte « N analyse(s) en échec » sur la tuile de classe. Le CTA ouvre la page d'analyse du premier dépôt, qui porte déjà le bouton « Relancer l'analyse ». Vérifié en bac à sable avec un décor (une analyse passée en `erreur` puis remise en `generee`), sur écran large et téléphone. Aucune migration, aucun interrupteur.

## État courant — proposition de routage continu, vérification documentaire (13/09/2026)

Livrable local `refonte_routeur_principes_2026-09-12/PROPOSITION_01_ROUTEUR.md`. Relecture des transitions contre le `02-` §2.5, des budgets contre le `01-`, des liens locaux et de la distinction décisions/hypothèses. Consultation des références citées sur écriture, alternance, espacement et mesure ; aucun seuil de maîtrise ni liste de transitions réputés validés par ces études. Aucun test applicatif, essai élève, calibration de juge ou nouveau résultat de recette. Les autres états de livraison ci-dessus restent inchangés.

## Historique — boucle pédagogique actée, vérification documentaire (13/09/2026)

Louis accepte le schéma amendé. Inscription au `02-exercices.md` v6.9 §2.5 du dépôt de conception ; renvois au `01-` v5.17 et `10-` v0.15, décision dans `CONTEXTE.md`. Contrôle des 9 crans et 22 transitions, des branches V1/VF, des cas discordants et des renvois ; budgets préservés ; `git diff --check` sans erreur sur les quatre fichiers de conception. Cette vérification porte sur la transcription de la décision. Aucun essai sur élèves, calibration de juge ou test applicatif ; aucun changement de code ou déploiement.

## Historique — proposition pédagogique de refonte (12/09/2026)

Socle accepté par Louis ; proposition d'application aux exercices dans le dossier local ignoré `refonte_routeur_principes_2026-09-12/APPLICATION_AUX_EXERCICES.md`. Relecture ciblée des sources de conception et vérification des textes officiels philo/HLP. Exemples et réponses fictifs, durées proposées sans mesure ; aucun essai sur élèves, calibration de juge ou test applicatif dans cette étape. Cette proposition documentaire ne modifie aucun résultat de recette antérieur.

## État courant — audit du routeur terminé, sans correctif (12/09/2026)

Dossier local ignoré : `audit_routeur_2026-09-12/ETAT_DES_LIEUX.md`, quatre parcours anonymisés et annexe de preuves/reprise. 558 tests ciblés existants réussis ; vérification des prompts du 05, concordance des dérivés, contrôles Python/TypeScript de la banque THLP sans import (0 blocage, 314 avertissements chacun).

Contre-exemples reproduits dans le code local : cran 2 prévu par la méthode mais fermé par le filtre antérieur ; second devoir de méthode pouvant changer de clé. Calendrier recalculé sur les dates de production : segment 2 de deux semaines, segment 3 dès le 14 septembre, alors que le 01 décrit encore trois semaines au segment 2. Les 6/8 du pilote n'alimentent pas le registre ordinaire : limite du raccordement, aucun seuil nouveau décidé.

Aucune recette visuelle, modification applicative, mutation de base ou validation d'efficacité pédagogique. Lectures actuelles et comportements du code local distingués des états historiques. Les anciens chiffres de banque ou de matériaux ci-dessous sont datés ; le dossier porte les nouveaux comptages.
...
- [x] **Le second retour bloque les rendus de Fragments** (Louis, 02/09 soir) — `retoursNonLus` porte `fragments_essai_chaine` tant que le retour de la chaîne n'est pas lu, plus rien après (couture, 65 ✓ sans appel) ; la bannière transversale le montre à l'élève.
- [x] **L'élève ne se relit pas pour l'essai** — `EcranEleve` avec `sansRelecture` : « Ta copie, telle que la machine l'a lue », sans bouton ni doutes à trancher (vu au navigateur).
...
## C7-L7 — Le routeur par objet (séance du 06/09/2026, branche `feat/ecrans-passation`, 0 migration, derrière `gabarit_actif`)

**Le lot** : l'objet a un état par élève — en méthode, ouvert, tenu —, DÉRIVÉ du registre et des objets déjà servis (`utils/registre/objet.ts`) ; la phase B pose la méthode entamée, puis un exercice par objet ouvert dans l'ordre de la règle 4 (observable non acquis, puis le moins mesuré, puis le tirage), puis l'objet neuf si sa séquence tient (`utils/moteur/objets.ts`, par `candidatsPour` — une seule phase B) ; journal `alternatives_ecartees.objet`. Les deux « pas fait » de C7-L6 sont fermés (deux et deux sur deux devoirs à un cycle d'écart ; sondes dès deux échecs). Relevé : `RELEVE_C7_L7_2026-09-06.md`. ⚠️ Ni C7-L5 ni C7-L6 n'ont de section ici : ce que j'ai éprouvé de leurs coutures est dit ci-dessous (③, ④).

### Prouvé en séance (bac à sable, élève de test `test@test.com` en décor — D en Argumentation et en Structure, les 328 instances 1.5 passées `concu` ; registre AVANT, tout retiré et vérifié par requête ; `scripts/recette/couture-c7l7.mjs --essai`, joué deux fois)
- [x] **C7L7-1 — ① la clé de l'instance → l'observable** : `lireLesInstances` lit `exercices_cas.probleme` sur la même jointure ; sur les 328 exercices 1.5 : 328 clés (41 distinctes), 328 observables, 0 sans, 0 hors instrument ; structure 159 · argumentation 97 · expression 56 · questionnement 16 ; `ciblables` = {compétence de l'observable} 328/328.
- [x] **C7L7-2 — ② le registre → `etatDeLObjet`** (registres de décor) : deux réussites même devoir même jour ⇒ ouvert, cran à servir 1, « le 1 compte 1 réussite(s) sur 2 » ; deux devoirs à un cycle ⇒ le 4 débloqué, cran à servir 2 ; bande 1..5 tenue ⇒ `tenu` ; jamais servi ⇒ `methode` ; banque sans cran 2 ⇒ bande 1·3·4·5, absent [2].
- [x] **C7L7-3 — ③ la porte et la quarantaine → la phase B** (instances réelles de « argument », 53, 12 devoirs) : en méthode, 3 retenues sur UN devoir (1·3·4) ; l'objet ouvert reçoit UN exercice (cran 1) sur un AUTRE devoir (4 instances du devoir servi écartées `devoir_en_quarantaine`) ; second tour : `objet_un_par_cycle`.
- [x] **C7L7-4 — ④ semaine 1 en base** (`poserLesSemainesDuRouteur`, cycle 2026-09-14, segment 3, **aux vraies lettres : C en Argumentation et en Structure**) : **4 décisions, 4 dépôts** ; objets `argument` (crans 1·3) et `exemple` (1·3) — la séquence C-B 1·3·2, le 2 fermé par la porte (Q4) —, toutes `objet.etat = methode`, `semaine_1: true`, `methode.rang` et séquence sur chaque ligne, écart au plancher vide — **fait quand (1a)**. ⚠️ Au premier essai (avant « un objet par compétence », lettres D en décor) : 1 seule décision — Q1 au relevé ; au deuxième (décor D, avant les trois décisions) : 5.
- [x] **C7L7-5 — ④ semaine 2 en base** (cycle 2026-09-21, après les trois décisions de Louis) : **3 décisions** — `argument` OUVERT, cran 1 « sous la bande, exigé par le registre » (Q2), autre devoir, `lien_explicite` non acquis ; `exemple` OUVERT (Structure, entrée par le trio entier — Q3), cran 1, autre devoir, `jointure_presente` non acquis ; `objection` NEUF en méthode (un seul, TC), cran 1 — son 3 attend (même observable, PB2 sur l'observable — Q1, résidu). Écartés au journal : `objet_un_par_cycle` sur les deux ouverts. **Fait quand (1b)**. Constaté par requête sur `routeur_decisions`. ⚠️ Avant les décisions (décor D) : 1 décision, `exemple` `objet_sans_cran_ouvert`, aucun objet neuf.
- [x] **C7L7-6 — fait quand (2)** : plafond réduit à 20 min en mémoire sur la composition réelle : « objection n'entre pas en méthode ce cycle : sa séquence pèse 33 min, 0 min sont posées, le plafond est de 20 min » (`objet_entree_hors_budget`) ; et le test unitaire (53 min / 8 posées / plafond 60 ⇒ attend ; 90 ⇒ entre).
- [x] **C7L7-7 — fait quand (3)** : entre `argument` et `objection` ouverts (Argumentation, 24 instances au cran 1), celui dont l'observable est non acquis passe d'abord, dans les deux sens, motif journalisé (« NON ACQUIS sur la fenêtre d'évidence ; n mesure(s) »).
- [x] **C7L7-8 — fait quand (4)** : `argument` tenu par décor (crans 1..5, deux devoirs, un cycle d'écart) : état `tenu`, ses 41 instances 1..5 écartées `objet_tenu`, seuls le 7 et le 9 sortent, marqués sonde de montée.
- [x] **C7L7-9 — fait quand (5)** : les 16 instances 1.4 du cran 6 (matériaux tenus pour génériques en décor — leur rattachement par notions n'est pas lu par la couche 4) : 42 couples (instance × cible) avec cible secondaire (conclusion/macro → 3 cibles, objection/méso → 2), 13 sans (micro).
- [x] **C7L7-10 — fait quand (6)** : `banc-objet.mjs --prod --cycle 2026-08-31`, lecture seule : 480 décisions, 58 élèves, 4,38 objets par élève ; la règle aurait gardé 2 objets par élève, posé 306 exercices contre 479, et fait tomber 218 (transition 89, phrase 41…).
...
## C7-L8 — La chaîne à l'heure de la clé (séance du 07/09/2026, branche `feat/ecrans-passation`, migration `c7_l8_chaine_cle.sql` — bac à sable seulement, derrière `chaine_cle_actif` à OFF)

**Le lot** : quatre pièces (Louis, 06/09) derrière SON interrupteur `chaine_cle_actif` — (1) la chaîne SIGNALE une v1 qui reproduit le devoir (crans 3·5·7) ; (2) sur un exercice qui isole avec une clé (`exercices_cas.probleme` non nul), la chaîne ne mesure QUE la compétence de l'observable de la clé (aucune sonde), la mesure ne garde que cet observable (les autres à `n/a`), et le retour ne reçoit que son squelette plus la borne en clair ; (3) le « se juger » n'interroge que cette compétence, l'observable de la clé en tête ; (4) une citation « copie » présente dans le matériau ET dans la copie s'élague, sauf sur le passage à corriger. Part pure : `utils/chaine/cle.ts` ; porte : `utils/chaine/porte-cle.ts` ; la clé lue par `contexte.ts` (`ctx.cle`, `ctx.chaineCleActif`, `passageMarque`). Relevé : `RELEVE_C7_L8_2026-09-07.md`. ⚠️ Ni C7-L1 ni C7-L6 n'ont de section ici.

### Prouvé en séance (bac à sable ; élève de test `test@test.com` en décor, marque `assigne_at = 2026-09-07T07:07:07.707Z` ; registre AVANT sur disque — lettres, escalade, montée, niveaux de Monitoring — tout reposé par `couture-c7l8.mjs --retire` et vérifié par requête ; la porte OUVERTE le temps de l'essai et REMISE comme trouvée)
- [x] **C7L8-1 — la migration** : `c7_l8_chaine_cle.sql` répétée à blanc sur le corps (transaction annulée, drapeaux à `t`, 0 colonne après rollback), puis jouée : `porte_posee = t`, `porte_a_off = t`, `params_intacts = 1` ; vérifié par psql (`boolean NOT NULL` défaut `false`) et PostgREST (`false`). Ligne au `SUIVI_SQL.md` écrite AVANT. Prod : en attente du geste de Louis.
- [x] **C7L8-2 — ① la clé du cas → l'observable** (`couture-c7l8.mjs --constat`) : sur les 328 instances 1.5 du bac à sable, 328 clés, 328 observables routés, 0 sans, 0 absente de la doctrine ; par objet : transition 87, argument 53, objection 52, paragraphe 52, exemple 35, phrase 25, plan 24. 206 problèmes dérivés dont 19 sans observable routé (aucune instance n'en porte). `derive-doctrine.py --verifie` : 17/17 IDENTIQUE avant de lire.
- [x] **C7L8-3 — ② la copie du cran 5** : dépôt de décor au cran 5 (`ex-gab-argument-garant-absent-c5`), `texte_v1` = le passage seul (145 car. contre 503 au matériau) ; `casPourLeRetour[0].reassemble` porté (avant 0 car., après 405 car.) — le juge lit le devoir réassemblé ; `ctx.cle` = « argument.garant.absent » → `lien_explicite` (argumentation), lecture `ok`.
- [x] **C7L8-4 — porte FERMÉE, la chaîne d'hier** (dépôt B, même exercice) : 5 compétences entrées, 3 mesures écrites (argumentation, expression, structure — connaissance et questionnement refusées par `preP2` comme hier), retour écrit, 8 appels (p1 3 · p2 3 · retour 1 · juge 1), 11 816 jetons, 0,0727 $ ; aucune alerte du lot ; l'alerte « cible du retour INDÉTERMINÉE » d'hier est là (le motif 1 de C9-L3).
- [x] **C7L8-5 — porte OUVERTE, la mesure sur la seule compétence de la clé** (dépôt A, cran 5 de `argument.garant.connecteur` — la contrainte `uk_depots_eleve_exercice` interdit deux dépôts sur le même exercice ; observable `garant_present`) : `competencesMesurees = [argumentation]`, les 4 autres écartées AVEC LE MOTIF « hors de l'observable isolé par la clé … (C7-L8) » ; en base 1 mesure, 1 squelette ; `observables` : `garant_present` gardé, 8 autres à `n/a` ; **fait quand (2), la mesure**. Aucune sonde : dépôt sans décision (dit dans la recette, pas fabriqué à la main).
- [x] **C7L8-6 — piège 11, MESURÉ : ce que `code2` rend avec un seul observable** : `lettre_equivalente = E` sur A (argumentation, `garant_present = 0`, les huit autres `n/a`) — la lettre vient de `branchement.lettre(agrege)` sur TOUT le jugement de P2 (forme (a) : P2 juge encore les observables qu'on jette) ; ce que le moteur en fait sur quatre mesures à un observable : NON mesuré (une seule mesure de décor) — condition de reprise C7L8-x3.
- [x] **C7L8-7 — le retour ne parle que de l'observable de la clé** : sur A, retour écrit, **2 points, tous `argumentation`** (une réussite citée « ce qui prend le regard prend l'attention et qu'un élève qui n'écoute plus n'apprend plus », un point de travail cité « Donc, puisque ») ; le verdict du juge est là (RÉUSSI) ; 0 citation « copie ET matériau » hors du passage — **fait quand (2), le retour**. ⚠️ Au premier essai, ma phrase de décor réutilisait le garant des zoos sur le devoir des téléphones : juge RATÉ, aucune réussite, retour refusé par la règle 2 — un défaut du décor, pas du lot (et `lettre_equivalente = E` sur ce jugement-là, C sur le second : la lettre vient de P2 entier, non déterministe).
- [x] **C7L8-8 — ④ le REJEU d'un dépôt mesuré porte fermée** (B, N squelettes ; `rejouerLeRetour` sans écriture, porte ouverte) : « retour borné à argumentation (observable « lien_explicite », clé « argument.garant.absent ») : 2 squelette(s) écarté(s) — expression, structure » ; 2 points rejoués, tous `argumentation` — le filtre vit au même endroit pour la mesure et le rejeu (piège 13).
- [x] **C7L8-9 — fait quand (4) sur le décor : le coût par dépôt baisse** : B (porte fermée) 8 appels, 0,0727 $ → A (porte ouverte) 4 appels (p1 1 · p2 1 · retour 1 · juge 1), 0,0445 $ — N compétences → une ; le juge du cran ne bouge pas (verdict posé sur A et B).
- [x] **C7L8-10 — ⑤ le signal de recopie au cran 7** (dépôt D, `ex-gab-argument-garant-absent-c7`, v1 = le devoir recopié avec la correction glissée) : « recopie du devoir (C7-L8, cran 7) — cas 1 : v1 = 1.18 × le matériau ; 4 phrase(s) sur 5 du devoir retrouvée(s) telle(s) quelle(s) » au bilan ; un signal, ni refus ni strike ; retour écrit, 2 points, **1 citation élaguée comme recopiée** (dans la copie ET le matériau, hors du passage), 0 « copie ET matériau » hors passage sur le retour servi — **fait quand (1) sur le décor**.
- [x] **C7L8-11 — fait quand (3) : le « se juger » au cran 2 sur un objet méso** (dépôt C, `ex-gab-argument-garant-circulaire-c2`, argument, `chargerLeDeroule` hors Next) : porte OUVERTE, servie, 1 question — `argumentation|lien_explicite`, aucune autre compétence, aucun « sans question » ; porte FERMÉE, l'élection d'hier de la branche : **0 question** (à ce cran, `ctx.servable` est vide sur la branche — la couche type des crans de production ne porte rien ; l'offre était VIDE, `seJugerAServir` la saute ; sur `main` la mesure du 06/09 servait 3 questions dont 2 de Structure).
- [x] **C7L8-12 — porte fermée, à l'octet, par les tests** : `cle.test.ts` (les compétences froides `v1 ? mesurees : [cible]`, la cible d'hier, l'ordre des candidates rendu tel quel, la mesure entière) ; `retour-cle.test.ts` (le message de Calame identique sans `observableIsole` — borne du cran 2 comprise —, l'élagage identique sans `materiaux`, `controlerRetour` identique). Suite entière : **2393 → 2422 tests, 1 rouge hors lot dans les deux cas** (`verifie-import-gabarit.test.ts`, la fabrique — piège 29) ; `tsc` propre ; eslint : 1 avertissement antérieur (`messageAvecMateriau` importé et non utilisé dans `chaine.ts`).
- [x] **C7L8-13 — le banc `--prod`, lecture seule, AVANT l'ouverture, avec la colonne neuve** (`banc-citations-origine.mjs --prod`, 07/09) : 62 retours chauds depuis le 31/08, 141 points, 18 sans citation ; 86 « copie seule », **33 « copie ET matériau » dont 21 HORS du passage à corriger et 12 DANS**, 2 « matériau seul », 2 « nulle part » ; par cran, hors passage : 4 au cran 4, 10 au 5, 6 au 7, 1 au 9. *(Le 06/09 : 54 retours, 122 points, 27 recopiées — même compte, huit retours de plus.)*
- [x] **C7L8-14 — le coût en prod, AVANT, la ligne de base** (`api_couts` depuis le 31/08, dépôts avec `routeur_decision_id`, colonne `cout`) : 118 dépôts, 710 appels (6,02/dépôt), 10 689 jetons et **0,0881 $ par dépôt** ; par phase p1 281 · p2 265 · retour 130 · juge 34 ; **2,20 compétences mesurées par dépôt** (1 : 56 · 3 : 43 · 4 : 18). *(Le 06/09 : 103 dépôts, 6,16, 0,091 $, 2,23.)*
- [x] **C7L8-15 — l'interrupteur à l'écran** : `PorteChaineCle.tsx` + `actions-chaine-cle.ts` dans Paramètres de Scriptorium (patron `PorteJugeDocuments`), colonne absente ⇒ fermé ; `basculerLaPorteChaineCle` éprouvée par la couture (ouverte, remise).
- [x] **C7L8-16 — la lecture à l'œil d'un retour de cran 5 et de cran 7 sur téléphone** (375 × 812 émulés, `next dev --webpack --port 3200` — Turbopack refuse le `node_modules` symbolique du worktree —, lien magique de l'élève de test) : le retour de A (cran 5, 2 points cités) et celui de D (cran 7 : point 1 cité « tu écris », **point 2 SANS citation — élaguée comme recopiée — se lit comme un point, « POINT 2 » puis le texte, pas comme un trou**) ; `scrollWidth = innerWidth`, aucun débordement horizontal. ⚠️ Vu à l'œil : la prose du point 2 dit « comme ici » en désignant le passage dont la citation a été élaguée — le point se lit, mais son « ici » ne montre plus rien (question au relevé).

### Reste à jouer — décoché, avec sa condition de reprise
- [ ] **C7L8-x1 — le banc `--prod` sur la première semaine après l'ouverture** : « aucune citation copie ET matériau HORS du passage à corriger ». Condition : la branche sur `main`, la migration jouée en prod, `chaine_cle_actif` OUVERT en prod (le geste de Louis), une semaine de retours.
- [ ] **C7L8-x2 — le coût en prod, APRÈS** : la même requête (`api_couts` par `depot_id`, `phase`, `cout`) sur la première semaine après l'ouverture, contre 0,0881 $ et 6,02 appels par dépôt. ⛔ Un constat, jamais une règle (`01-` §11).
- [ ] **C7L8-x3 — ce que le moteur fait de quatre mesures à UN observable** (`jugerLaLettre`, la fenêtre d'évidence, l'escalade) : la lettre E d'une mesure à `garant_present = 0` seul — est-ce la lettre voulue ? Condition : quatre dépôts réels sous la porte, ou une décision de Louis sur la forme (b) (`IDEES`, 07/09).
- [ ] **C7L8-x4 — la borne du cran 2, l'arbitrage de fabrication (piège 3)** : le code garde la borne de la séance des écrans sous `juge_documents_actif` porte fermée, et la REMPLACE par celle du lot porte ouverte. Condition : Louis confirme, ou demande que la borne du cran 2 quitte `juge_documents_actif` (deux lignes dans `retour.ts`).
- [ ] **C7L8-x5 — la prod** : `c7_l8_chaine_cle.sql` en production (le geste de Louis, avec le push de la branche), puis l'ouverture.

## C7-L9 — Sur un cran qui isole, le juge est la mesure (séance du 07/09/2026, branche `feat/ecrans-passation`, migration `c7_l9_juge_mesure.sql` — bac à sable seulement, derrière `juge_mesure_actif` à OFF)

**Le lot** : cinq pièces (Louis, séance d'arbitrage du 07/09) derrière SON interrupteur `juge_mesure_actif`, qui n'a de sens que `juge_documents_actif` ouvert — aux crans 1·2·3·4·5·7·9 SERVIS PAR LE ROUTEUR (origine `routeur`, maison, formatif), sur un exercice qui isole avec une clé : (1) ni P1 ni P2, la mesure est le verdict du cran converti en valeur extrême de la famille de l'observable isolé (`03-` §1), `n/a` ailleurs ; (2) `lettre_equivalente` NULLE, une sonde de montée à un cran qui isole se dit réussie par le verdict ; (3) le taux de réussite pondéré par cran (6·8 : 1 · 7 : 0,8 · 2·5 : 0,6 · 4·9 : 0,5 · 1·3 : 0,2), la fenêtre et les compteurs en mesures, et le signal de trajectoire (majorité stricte des requis, au taux pondéré) déclenche une sonde de montée au 6·8 ; (4) en vf le juge rejoue à l'aveugle, `delta_v1_vf` = verdict vf − verdict v1 ; (5) Calame sans squelette (`07-` § 4 bis). Porte fermée : la chaîne d'hier à l'octet.

### Prouvé en séance (bac à sable ; élève de test `test@test.com` en décor, marque `assigne_at = 2026-09-07T09:09:09.909Z` ; registre AVANT sur disque — lettres, escalade, montée, niveaux de Monitoring — tout reposé par `couture-c7l9.mjs --retire` et vérifié par requête ; les portes `juge_mesure_actif` et `chaine_cle_actif` bougées le temps de l'essai et REMISES comme trouvées)
- [x] **C7L9-1 — la migration** : `c7_l9_juge_mesure.sql` répétée à blanc sur le corps (transaction annulée, drapeaux à `t`, 0 colonne après rollback), puis jouée : `porte_posee = t`, `porte_a_off = t`, `params_intacts = 1` ; vérifié par psql (`boolean NOT NULL` défaut `false` ; ligne 1 : `juge_mesure_actif = f`, `juge_documents_actif = t`, `chaine_cle_actif = f`, `gabarit_actif = t`). Ligne au `SUIVI_SQL.md` écrite AVANT. Prod : en attente du geste de Louis.
- [x] **C7L9-2 — les 47 observables isolés se convertissent, et la règle est prouvée** (`juge-mesure.test.ts`) : sur les 52 observables que le `09-` isole en bac à sable (206 problèmes, 187 routés), 5 sont sans entrée `observables_mesure` — exactement les cinq du `03-` §1 — et 47 se convertissent ; pour chacun, réussi se relit `reussie` et raté `ratee` au seuil déclaré, **sauf `synthese|contresens_partiel`** (comptage rapporté `au_plus` au seuil 2 : le « 1 » du raté reste sous le seuil — `n/a` + alerte, jamais une valeur fausse) ; pour tout seuil balayé de 0 à 1 (par 0,05, > 1 000 cas), une valeur convertie se relit comme son verdict ou vaut `n/a` et le dit ; sur la plage non dégénérée des proportions et comptages rapportés, jamais de `n/a`.
- [x] **C7L9-3 — les valeurs d'absence des binaires** : les cinq « booléens » de la fiche valent `oui` / `non` en texte (mesuré sur les instruments dérivés : `valeur_reussie: 'oui'`) — l'absence est `non`, pas `false` ; `question_presente` → `absent` (le premier terme d'une liste pour le réussi), `enjeu` → `absent`, `notions_en_tension` / `debat_situe` → `absentes`, `question_propre` → `reprise_enonce`, `question_specifique` → `generique`.
- [x] **C7L9-4 — fait quand (1)** (dépôt A, `ex-gab-argument-garant-connecteur-c5`, origine `routeur`, porte OUVERTE, `chaine_cle_actif` OFF) : `api_couts` = **0 ligne p1/p2, 1 ligne juge (phase NULL), 1 ligne retour** — 2 appels, 0,0221 $ ; **1 mesure** sur `argumentation`, `observables` = `garant_present = 1` et 8 autres à `n/a`, **`lettre_equivalente` NULL**, `instrument_version` = celle de l'instrument dérivé ; **0 squelette** ; le verdict RÉUSSI sur `verdicts_cran.v1` ; le retour chaud écrit (1 point, cité dans la copie). Les 5 compétences de l'instance écartées AVEC MOTIF (C7-L9). Le second essai (même décor, retiré entre les deux) : 0,0223 $, mêmes lignes.
- [x] **C7L9-5 — piège 29, la clé se lit INDÉPENDAMMENT de `chaine_cle_actif`** : l'essai entier tourne C7-L8 à OFF ; `ctx.cle` = « argument.garant.connecteur » → `garant_present`, lecture `ok` ; le régime lit la clé sur `ctx.cle` par le lot, pas par `observableDeLaCleServie`. ⚠️ Corollaire dit au relevé : porte C7-L8 fermée, l'élagage d'une citation « copie ET matériau » (pièce (4) de C7-L8) ne s'applique pas — c'est SON lot.
- [x] **C7L9-6 — fait quand (2)** : la fenêtre d'évidence (4 mesures) porte la convertie en dernier, `statutDeLaMesure` la lit `reussie` sans cas particulier ; `tauxDeReussite` avec le poids du cran 5 : réussies 0,6, dénominateur 0,6, taux 1. Par test (`routeur/observables.test.ts`) : dix mesures réussies au cran 1 donnent une fenêtre de QUATRE, un taux de 1 et un dénominateur de 0,8 — la fenêtre reste en mesures ; deux 6·8 réussis + deux crans 1 ratés : acquis pondéré (2 / 2,4), non acquis à poids égal.
- [x] **C7L9-7 — porte FERMÉE, la chaîne d'hier** (dépôt B, `ex-gab-argument-garant-absent-c5`) : 9 appels (p1 3 · p2 4 · juge 1 · retour 1), 3 mesures avec lettre (argumentation C, expression A, structure D), 3 squelettes, 0,0964 $ ; aucune alerte du lot. **Le coût par dépôt : 9 → 2 appels, 0,0964 → 0,0221 $.** Et **piège 4** : un dépôt `prof` au cran 7, porte OUVERTE, régime inactif sans un mot.
- [x] **C7L9-8 — piège 11, le cran 1** (dépôt C, `ex-gab-argument-garant-absent-c1a`, statut `assigne`, crédence sur les 2 cas — tous les jetons sur le bon candidat) : `cloturerLeCranGuide` → `clos`, `mettreLaMesureEnFile` → job `mesure_v1`, `traiterDepot` **accepte le dépôt sans production** : 1 mesure convertie `lien_explicite = 1`, lettre nulle, **0 ligne `api_couts`**, aucun retour, aucun squelette, le dépôt reste `clos`. Le chemin depuis l'écran (`app/deroule/actions.ts`, `clore` → `mettreEnFileSiLeJugeEstLaMesure`) est écrit, pas smoké à l'écran (C7L9-x4).
- [x] **C7L9-9 — fait quand (4), la vf au cran 2** (dépôt D, `ex-gab-argument-garant-circulaire-c2`) : v1 → verdict RATÉ, mesure `lien_explicite = 0` (lettre nulle) ; le retour chaud sur un verdict raté **sans réussite, admis** (§ 4 bis, règle 2 — au premier essai il avait été refusé pour une citation RR3, rejoué par la file) ; vf → **second verdict sur `verdicts_cran.vf`** (RATÉ), **`delta_v1_vf = 0`** sur la mesure de la v1, **AUCUNE seconde mesure**, retour final écrit ; sur les deux versions, 0 ligne p1/p2 (juge 2, retour 2).
- [x] **C7L9-10 — le rejeu du retour sous le régime** (`rejouerLeRetour(A, { sansEcriture })`) : aucun squelette à relire, le verdict stocké, 1 point rejoué. Le banc (`rejouerUneCompetence`) rejoue le JUGE, pas P1/P2 — par le code, non éprouvé en bac à sable.
- [x] **C7L9-11 — fait quand (5) sur le décor** (`banc-citations-origine.mjs --depuis 2026-09-07`, bac à sable) : 2 retours chauds, 4 points, 0 sans citation, 3 « copie seule », 1 « copie ET matériau DANS le passage » (le dépôt B, porte fermée), **0 HORS du passage** ; le retour de A cite la copie et ne nomme qu'une dimension (« le lien entre la preuve et la conclusion »).
- [x] **C7L9-12 — Calame sans squelette, porte fermée à l'octet, par les tests** (`retour-juge-mesure.test.ts`) : `jugeEstLaMesure` absent ou faux ⇒ message et système identiques à hier (v1 et vf) ; ouvert ⇒ pas de « SQUELETTE ET VERDICTS », le § 4 bis en clair (les cinq lignes), le verdict et la dimension ; sans verdict, Calame le sait ; `controlerRetour` refuse un retour sans réussite comme hier et l'admet SEULEMENT sur `sansReussiteAdmise` (verdict raté), RR4 intact.
- [x] **C7L9-13 — le signal de trajectoire, et la pose** (`decision.test.ts`) : majorité STRICTE des requis acquise → `trajectoire_montee`, ratée → `trajectoire_descente`, 1 sur 2 ou 2 sur 4 → rien, sans taux → ni l'un ni l'autre, sans lettre → rien ; la pose : sur un 6/8 déjà posé (cible) → la sonde s'y pose ; sinon un 6/8 servable du vivier, ajouté sous la règle `trajectoire` s'il tient dans le budget ; sinon `sans_substrat` / `hors_budget` journalisés avec les nombres ; les sondes de trajectoire comptent dans `PLAFOND_SONDES_PAR_CYCLE` ; **la porte du registre laisse passer un 6/8 EN SONDE quand la trajectoire s'est levée sur sa compétence** (`vivier.ts`) — pas un autre cran, pas une autre compétence ; une seule marque `sonde_montee` par exercice (la sonde de trajectoire entre avant celle du registre).
- [x] **C7L9-14 — le signal en bac à sable, sur une pose RÉELLE** (élève de test, lettre C en Argumentation, quatre mesures de décor `instrument_version = decor-c7l9` acquérant les requis, `poserLesSemainesDuRouteur` cycle 2026-09-14) : le signal se lève — « 6 requis acquis, 2 ratés, sur 8 (majorité stricte : plus de 4) » en Argumentation, « 6 sur 7 » en Expression — et se journalise `sans_substrat` sur `alternatives_ecartees.trajectoire` de la première décision. ⚠️ **Aucun 6/8 n'est servable en bac à sable** : le gabarit 1.5 n'a pas de cran 6 ni 8 ; les 18 exercices 6/8 de la banque 1.4 portant l'Argumentation sont écartés `cours_par_notions_non_lu` (342 écarts du vivier). Le « fait quand » (3) tel qu'écrit (« `routeur_decisions` porte une sonde ») n'est donc pas atteignable ici — C7L9-x3.
- [x] **C7L9-15 — la médiane ignore une mesure sans lettre** (`etat.test.ts`) : deux B et deux nulles → B ; rien que des nulles → aucune lettre.
- [x] **C7L9-16 — la sonde de montée à un cran qui isole se dit réussie par le verdict** (`etat-serveur.ts`, `issuesDesSondesSansLettre`) : sans lettre, l'issue du dépôt par `issueDuDepot` (le registre, un seul domicile) ; sans issue, la sonde ne compte ni réussie ni ratée. Par le code et `tsc`, non éprouvé en bac à sable (aucune sonde sans lettre n'y a été posée).
- [x] **C7L9-17 — `Mesure.cran` par jointure, aucune colonne** : `depot:exercices_depots(exercice:exercices(cran))` dans `donnees.ts` et `competences-classe.ts` (tolérant : objet ou tableau, absent ⇒ `null` ⇒ poids 1) ; le taux pondéré n'entre que la porte OUVERTE dans ses trois lecteurs (`etat-serveur`, `competences-classe`, `attention-serveur`) et dans `cycle-serveur` ; fermée, `etatDesObservables` sans poids — hier.
- [x] **C7L9-18 — la variante du 4(b) au registre** : `lireLesDepotsPourLeRegistre` lit désormais `exercices.variante` (`a` / `b`, 38 et 38 en bac à sable) — sans elle `issueDuDepot` attendait un juge au 4(b) ; et un filtre `depotId` pour lire UN dépôt. Couture avec C7-L7, dite au relevé.
- [x] **C7L9-19 — l'interrupteur à l'écran** : `PorteJugeMesure.tsx` + `actions-juge-mesure.ts` dans Paramètres de Scriptorium, à côté de la porte de C7-L8, avec la phrase « n'a d'effet que si le juge est ouvert » et l'état du juge affiché ; colonne absente ⇒ fermé. `tsc` propre ; non capturé à l'écran (C7L9-x4).
- [x] **C7L9-20 — la suite entière** : **2422 → 2465 tests**, **2 rouges hors lot** : `verifie-import-gabarit.test.ts` (connu, piège 29 de C7-L8) et `instruments.test.ts` « les dérivés sont IDENTIQUES » — `MANIFESTE.ts` diverge du `07-` passé en v2.81 (la nuit du 07/09, conception non commitée) ; le geste `derive-instruments.py --ecris` est celui de la fabrication, le lot ne le relance pas. `tsc` propre ; eslint : 0 erreur (un avertissement antérieur au lot sur `chaine.ts`).

### Reste à jouer — décoché, avec sa condition de reprise
- [ ] **C7L9-x1 — fait quand (6), le coût en prod AVANT / APRÈS** : `api_couts` par dépôt sur les crans qui isolent, la première semaine après l'ouverture, contre 0,081 $ (07/09 : p1 0,0225 · p2 0,0321 · retour 0,0242 · juge 0,0021) — attendu ≈ 0,03 $ (le décor dit 0,022 $). Condition : la branche sur `main`, `c7_l9_juge_mesure.sql` en prod, `juge_documents_actif` PUIS `juge_mesure_actif` ouverts en prod (le geste de Louis), une semaine de dépôts. ⛔ Un constat, jamais une règle.
- [x] **C7L9-x2 — la table du `03-` §1 sur un comptage dont le seuil vaut 1 ou plus** : tranché avec Louis le 08/09 — le raté d'un comptage vaut le seuil doublé, 1 au minimum (`valeurDuVerdict`) ; `contresens_partiel` rend 4, les 47 observables se convertissent sans `n/a`. ✅ Le `03-` §1 est amendé (v2.3, 08/09, sur l'accord de Louis ; dépôt de conception, non commité).
- [ ] **C7L9-x3 — fait quand (3), la sonde de trajectoire POSÉE dans `routeur_decisions`** : condition : un exercice 6/8 SERVABLE sur la compétence dans le vivier — les 6/8 du gabarit (C7-L4, « les crans 2, 6, 8 ») ou la banque 1.4 lue par notions (C4-L12, troisième voie). Alors `couture-c7l9.mjs --essai --seulement-trajectoire --eleve-trajectoire test@test.com` doit rendre `exercice_ajoute` ou `sonde_sur_exercice_pose`.
- [ ] **C7L9-x4 — le smoke à l'écran** : la clôture d'un cran 1 depuis l'écran élève met le dépôt en file (le chemin `clore` → `mettreEnFileSiLeJugeEstLaMesure`), et la porte dans Paramètres sur trois tailles. Condition : `next dev --webpack` sur le worktree, lien magique de l'élève de test, porte ouverte puis remise.
- [ ] **C7L9-x5 — la sonde de montée réussie par le verdict** (`etat-serveur.ts`) : une sonde de montée posée sur un cran qui isole, mesurée porte ouverte, puis `ecrireLEtatApresMesure` — M-d la compte réussie ou ratée par l'issue. Condition : un décor avec `sondes_retenues` `sonde_montee: true` sur un exercice 1.5 (la pose de C7-L5 ou de C7-L7).
- [ ] **C7L9-x6 — la prod** : `c7_l9_juge_mesure.sql` en production (le geste de Louis, avec le push de la branche), puis l'ouverture — APRÈS `juge_documents_actif` (piège 30 : 34 appels du juge en prod le 03/09, 0 `verdicts_cran` — à lire avant d'ouvrir ; `ecrireLeVerdict` écrit « séparément et tolérant » : une colonne absente ou une erreur d'`update` part en alerte, le retour est servi, et AUCUNE mesure convertie ne s'écrit sans verdict).

---

## C10-L1 — La semaine comptée se ferme (séance du 07/09/2026, `main`, **AUCUNE migration, AUCUN interrupteur**)

> ✅ **POUSSÉ ET DÉPLOYÉ le 07/09 à 15:18 UTC** *(commit de fusion `d1d4269`, sur le « on peut y aller » de Louis)*.
> ⭐ **Et il ne s'est RIEN fermé au déploiement, c'est mesuré** : le seul cycle compté en production était `2026-08-24`, qui ne porte **aucun** dépôt du routeur *(les 480 sont tous sur `2026-08-31`)*. **La fermeture tombe à 18:00 UTC**, quand le déclencheur pose les lignes du `2026-08-31`.

**Le lot** : au moment où l'assiduité d'une semaine est jouée pour un élève — c'est-à-dire quand la ligne `assiduite_hebdo (eleve_id, cycle_lundi)` **existe** —, les exercices que le **routeur** lui a assignés sur ce cycle cessent d'être jouables. **Le marqueur est un FAIT, pas un statut** : aucune colonne, aucun cron, aucun statut écrit, et la fermeture se **dérive à chaque lecture** *(elle est donc rétroactive, et idempotente par construction)*. ⛔ **Aucun interrupteur — décision de Louis du 05/09** : le retour en arrière est un redéploiement, pas une réparation de données.

### Prouvé en séance — `couture-c10l1.mjs --essai` (bac à sable, **39 contrôles tenus, 0 en échec**, base rendue et vérifiée par requête) et `smoke-c10l1-eleve.mjs` (session élève réelle, lien magique, 3 largeurs)

- [x] **C10L1-1 — ① la couture de la LIGNE, par le VRAI écrivain** : `poserLaSemaineDAssiduite(admin, fuseau, '2026-09-07', '2026-08-31')` — le point d'entrée du cron — pose 17 lignes ; `cyclesComptesDeLEleve` les relit et rend le cycle `2026-08-31`. **Avant la ligne, aucun des 8 dépôts semés n'est fermé ; après, tous ceux du routeur le sont.**
- [x] **C10L1-2 — ⭐ ② LE CONTRÔLE QUI VAUT TOUS LES AUTRES : le dépôt du DIMANCHE 20 h 30** (`assigne_at = 2026-09-07T00:30:00Z`, soit dimanche 6/09 20 h 30 à Toronto). Ma dérivation `toISODate(lundiDuCycle(instant, fuseau))` rend `2026-08-31`, **et `comptesDeLaSemaine` le range dans la même semaine** (`horsSemaine = 0`). Lu en UTC il aurait basculé d'une semaine — à l'heure exacte à laquelle les élèves déposent. Le dépôt est bien fermé.
- [x] **C10L1-3 — ③ la voie du PROFESSEUR reste ouverte** : un dépôt sans `routeur_decision_id`, **même élève, même cycle, ligne posée** → `fermee = false`, ton `a_faire`. Et les deux discriminants concordent : `origine='routeur'` sans décision = **0**, `origine='prof'` avec décision = **0**, dans les deux bases *(la clé étrangère est `ON DELETE SET NULL` : le contrôle se rejoue à chaque passage)*.
- [x] **C10L1-4 — la semaine EN COURS reste ouverte, sans garde à écrire** : la ligne de la semaine courante n'existe jamais (le déclencheur ne compte que l'écoulée) — la clause tombe du prédicat. Vérifié sur un dépôt du cycle `2026-09-07`.
- [x] **C10L1-5 — ⛔⛔ LA RÉDUCTION : matériau, candidats, distracteurs et zones ABSENTS de la charge, pas masqués.** `chargerLeDeroule` sert une vue passée par `vueFermee` — **liste blanche de 13 champs sur 52**. Les 15 champs interdits mesurés vides sur la charge réelle (1 592 octets) ; le test `fermeture.test.ts` farcit **chaque** champ d'un secret reconnaissable et vérifie qu'**aucun** ne sort. ⭐ Un 53ᵉ champ ne compilera pas tant que son auteur n'aura pas dit s'il survit.
- [x] **C10L1-6 — mais la consigne, la RÉPONSE et le RETOUR survivent** : brouillon rendu tel quel (`v1RemiseLe = null` ⇒ l'écran dit « ton brouillon », pas « ta réponse ») ; aucun texte effacé.
- [x] **C10L1-7 — ⛔⛔ `regime` SURVIT à la réduction** — sans lui `actionValiderLaLecture` cesse de clore le dépôt, la ligne reste `a_lire` pour toujours et **le bilan ne s'ouvre jamais**.
- [~] **C10L1-8 — le refus au PORTIER, une fois, pour dix-huit actions** : la garde entre dans le `if (ecriture)` de `portier(depotId, ecriture)` (`app/deroule/actions.ts`), à côté de `messageSiBloque` et `messageSiRetoursNonLus`. **Les six appels `portier(…, false)` restent permis** — ce sont les gestes qui portent sur le RETOUR, pas sur l'exercice.
- [x] **C10L1-9 — ④ la LECTURE est la porte de sortie, et elle reste ouverte** : `validerLaLecture` acceptée sur un exercice fermé, `lu_at` posé, dépôt → `clos`, **et le bilan s'ouvre**.
- [x] **C10L1-10 — ⭐⭐⭐ L'ARBITRAGE DU LOT, éprouvé sur un témoin dont la semaine n'a QUE du routeur** : un exercice jamais fait (fermé) + un retour publié NON LU → **bilan RETENU** ; lecture validée → **bilan OUVERT**. *La mission (« le bilan s'ouvre même sur des exercices NON FAITS ») et `C6-L2` (« la semaine ne se referme jamais sur un retour que l'élève n'a pas ouvert ») tiennent toutes deux : un retour publié non lu n'est pas un exercice non fait.* ⚠️ **Si Louis veut l'autre lecture, elle est à lui.**
- [x] **C10L1-11 — ⛔⛔ LA FRISE NE COMPTE PAS UN FERMÉ COMME UN FAIT** : sans la clause, elle serait passée de « 1 sur 6 » à « 6 sur 6 » le lundi à 18 h **sans que l'élève ait rien fait** — « un décompte réel » (`06-` §5). Mesuré : 1/6, puis 1/2 sur le témoin.
- [x] **C10L1-12 — le bilan DIT combien n'ont pas été faits, et ne les confond pas avec des copies à corriger** : `nonFaits` et `copiesNonMesurees` sont deux nombres distincts *(« une de tes copies n'a pas encore été corrigée » serait faux d'un exercice jamais rendu)*.
- [x] **C10L1-13 — un BONUS se ferme comme un imposé**, par le même test et sans branche ; il ne retient pas le bilan et ne l'ouvre pas non plus.
- [x] **C10L1-14 — un `v1_remis` fermé reste `attente`** (« rendu, retour en préparation » est vrai) ; **un `retour_publie` non lu fermé reste `a_lire`** — l'obligation de lecture passe DEVANT la fermeture. *« Fermé sans retour » est à `C9`, et ce lot ne le fabrique pas.*
- [x] **C10L1-15 — les deux vides ne se confondent pas** : une lecture en **erreur** (`cycles = null`) ne ferme rien *(fail-open — fermer sur une panne priverait un élève d'un travail qui compte encore)* ; un ensemble **vide** est une réponse, pas une panne.
- [x] **C10L1-16 — ⭐⭐ LE SMOKE ÉLÈVE, ET C'EST LA PREUVE QUE LA COUTURE NE POUVAIT PAS DONNER.** `assiduite_hebdo` a la RLS active et **une seule policy, `prof`** — aucune policy élève : lue depuis une session élève elle rend **zéro ligne, SANS erreur**, et rien ne se fermerait jamais, en silence, avec `tsc` et les tests verts. **Depuis une vraie session (lien magique), l'exercice fermé affiche bien le message de Louis** : la lecture passe en service-role sur tout le chemin élève.
- [x] **C10L1-17 — les trois largeurs, mesurées** (`smoke-c10l1-eleve.mjs`, CDP `setDeviceMetricsOverride`) : 1280 · 768 · 375 px sur la vue fermée d'un `assigne`, d'un `ouvert` avec brouillon, d'un `retour_publie` non lu, d'un `vf_remis`, la liste Codex, « Ma semaine » de la semaine précédente et son bilan. **Aucun débordement horizontal** (`document.scrollWidth == window.innerWidth` partout).
- [x] **C10L1-18 — la base rendue à son état d'entrée, VÉRIFIÉE par requête** : `--retire` balaie par la MARQUE et retire l'assiduité **PAR IDENTITÉ (élève × cycle)**, jamais par table. ⛔ *`essai-cron-hebdo.ts --retire` et `routeur-c4l12.mjs --retire`, eux, vident `assiduite_hebdo` en entier — dans un bac à sable partagé ils emportent le décor d'une autre séance.* Après retrait : 0 instance marquée, 0 ligne semée, cardinal d'entrée retrouvé.
- [x] **C10L1-19 — `tsc` propre et `npm test` vert** : 2 499 tests, 0 échec *(2 469 avant le lot ; 30 de plus, dont l'épreuve « aucun secret ne fuit »)*.

### Reste à faire — décoché, avec sa condition de reprise

- [ ] **C10L1-x1 — LA PREMIÈRE FERMETURE RÉELLE EN PRODUCTION, à 18:00 UTC le 07/09.** ⭐ **Le lot est déployé depuis 15:18 UTC et n'a rien fermé** — remesuré juste avant de pousser : cycles comptés en prod = `["2026-08-24"]` seulement, et **0 dépôt du routeur** n'y est rattaché. **La fermeture est donc entièrement portée par le déclencheur de 18:00 UTC** *(puis 18:20)*, qui posera les lignes du `2026-08-31`. **Attendu à cet instant, mesuré à 15:15 UTC** : **480 dépôts** se ferment — **197 jamais rendus** *(170 `assigne` + 27 `ouvert`)*, **81 en attente d'un retour**, 161 déjà `clos`, 38 `retire` *(invisibles : la liste et le chargeur les écartent déjà)*. **Condition de reprise** : après 18:06 UTC, vérifier en prod que `assiduite_hebdo` porte bien les lignes du `2026-08-31`, puis qu'un dépôt `assigne` du routeur se lit « fermé » à l'écran d'un élève. ⚠️ **Et le 14/09, le cycle `2026-09-07` se fermera à son tour** — la fermeture étant une dérivation, elle est rétroactive et n'a rien à rattraper.
- [ ] **C10L1-x2 — la fermeture d'un élève servi au SECOND passage du déclencheur** (`20 18 * * 1`). Le déclencheur a déjà été **coupé en vol le 31/08 — 59 élèves servis sur 62, sans un mot** : sous ce lot, les oubliés garderaient leur semaine ouverte pendant que leurs camarades la perdent — **la fermeture serait partielle À L'INTÉRIEUR D'UNE MÊME CLASSE, et invisible**. *(La file bornée a été poussée le 02/09 ; le second passage est là pour cela. Le défaut est à `C4-L13` : ce lot le nomme, il ne le répare pas.)* **Condition de reprise** : un lundi après déploiement, comparer les lignes posées à 18:00 et à 18:20.
- [ ] **C10L1-x3 — un exercice du routeur assigné sur une SEMAINE DE VACANCES ne se fermera JAMAIS.** Trou **permanent**, pas transitoire : les semaines de vacances n'ont jamais de ligne *(`07-` §1.5, « par omission »)*. ⛔ **Ne pas le réparer en posant une ligne** — la semaine entrerait au dénominateur, faux pour tout élève, chaque année. **Déposé dans la boîte de `C10-L2`.** Même forme : **un élève inscrit après le comptage d'une semaine** n'a pas de ligne pour elle.
- [ ] **C10L1-x4 — la SEMAINE DE MÉTHODE coupée en deux.** Le `01-` v5.15 fait poser au routeur une **séquence de crans** sur un ou deux devoirs (E-D : `1·3 ‖ 4·2`) ; *« les crans de la séquence se servent sur le même devoir, la même semaine »* — donc une séquence tient dans un cycle et la fermeture ne la coupe jamais **entre** deux semaines. ⚠️ **Mais une semaine fermée AU MILIEU d'une séquence emporte les crans restants** : l'élève qui en était au 3 ne verra jamais le 2 ni le 4, et rien ne les rattrape. ⛔ **C'est la règle, et « laisser finir une séquence commencée » serait un SECOND prédicat de fermeture, donc un second domicile.** *Conséquence que le chapitre `C10` n'a pas pu prévoir : il est du 05/09, la séquence sur deux devoirs du 07/09.* **À montrer à Louis quand `gabarit_actif` servira des séquences en production.**
...
- [x] **C10L2-32 — ⛔⛔ LE DISCRIMINANT D'UNE LIGNE D'OVERRIDE EST SA **RÈGLE**, PAS UN EXERCICE ABSENT — faille trouvée par un audit indépendant le 08/09, et réparée.** La première version du lot filtrait sur la nullité de `exercice_id`. **Mesuré en base** : `routeur_decisions_exercice_id_fkey` est **`ON DELETE SET NULL`** — une **vraie** décision de routeur dont l'exercice est supprimé porte donc `exercice_id` NULL, et devenait **indiscernable d'un override**. ⛔ Conséquences que personne n'avait vues : `exercicesParCycle` aurait **sous-compté** un cycle réellement servi *(K de R5 baisse — la régression que ce lot répare, retournée)*, et la garde d'idempotence aurait déclaré l'élève **NON servi**, si bien que **le cron lui aurait REPOSÉ une semaine déjà posée**. ⭐ Réparé par `aServiUneSemaine` *(`utils/routeur/profil.ts`, un seul domicile, avec `REGLE_OVERRIDE_PROF`)* : le test porte sur `regle_declenchee`, et il refuse **par la positive** — une règle inconnue ou nulle penche du côté « servi », le côté prudent. Éprouvé par exécution *(couture, 64/64)* et par quatre tests purs. ⚠️ *Ce que la mesure d'origine avait vu et dont je n'avais pas tiré la conséquence : j'avais lu cette clé étrangère en base le 07/09 sans en déduire qu'elle brisait mon discriminant.*
...
- [x] **C10L2-x1 — LA CLÔTURE RÉELLE EN PRODUCTION : FAITE PAR LOUIS le 07/09 entre 18:15 et 18:27 UTC, sur les quatre instances.** Mesuré en base juste après :
  · **15 `abandonne`**, exactement les 15 cibles — `b5f7a719` 10, `d8de575c` 2, `364359ce` 2, `524f5484` 1 ;
  · **les 69 `retour_publie` et le `v1_remis` intacts** — « les copies remises ne bougent pas », vérifié sur la vraie base ;
  · ⭐⭐ **le journal a reçu ses 15 lignes — LA PREMIÈRE ÉCRITURE RÉELLE D'`override_prof` EN CINQ MOIS DE PRODUCTION.** Geste `cloture_passation` × 15, toutes sur le cycle `2026-08-24`, **toutes des lundis** (isodow = 1 — la dérivation par `lundiDuCycle` tient sur les vraies dates), `exercice_id` NULL sur toutes, et ⛔ **aucun `routeur_decision_id` posé sur les dépôts** : les lignes restent orphelines, donc ces dépôts ne tombent pas aussi sous `C10-L1`.
  ⚠️ *La branche `assigne` n'a toujours pas été rencontrée : les 15 étaient tous `ouvert` (voir `C10L2-x2`).*
...
- [x] **C10L2-30 — ⭐⭐ LE PARTAGE DES DEUX LOTS, VÉRIFIÉ EN PRODUCTION LE SOIR MÊME — et il ne laisse AUCUN orphelin.** Mesuré à 18:27 UTC, après le déclencheur et après la clôture manuelle :
  · **`C10-L1`, automatique** — cycle `2026-08-31`, ligne d'assiduité posée à 18:00 → **277 exercices du routeur fermés par DÉRIVATION, 0 encore ouvert. Aucun geste, aucun statut écrit.**
  · **cycle courant `2026-09-07`** — posé le soir même par le cron *(211 décisions, 57 élèves)*, pas de ligne d'assiduité → **211 ouverts, et ils DOIVENT l'être.**
  · **`C10-L2`, manuel** — les 85 dépôts de classe, qui portent `routeur_decision_id` **NULL sur 85/85** : sans décision, pas de marqueur, donc pas de fermeture dérivée. **C'est exactement pourquoi le bouton existe.**
  · ⛔ **`dépôts sans décision de routeur` = 85 = exactement les 85 de classe → ZÉRO dépôt hors des deux lots.** La couverture est complète.
