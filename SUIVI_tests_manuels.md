...
- [x] **C10L1-8 — le refus au PORTIER, une fois, pour dix-huit actions** : la garde entre dans le `if (ecriture)` de `portier(depotId, ecriture)` (`app/deroule/actions.ts`), à côté de `messageSiBloque` et `messageSiRetoursNonLus`. **Les six appels `portier(…, false)` restent permis** — ce sont les gestes qui portent sur le RETOUR, pas sur l'exercice.
...
- [ ] **C10L1-x1 — LA PREMIÈRE FERMETURE RÉELLE EN PRODUCTION.** ⚠️ **Elle n'a PAS eu lieu le 07/09 : Louis a choisi de construire à fond et de pousser plus tard** *(question posée à 13:55 UTC, avant les 18:00 UTC du déclencheur)*. **Condition de reprise** : le lot poussé et déployé. ⛔ **Ce jour-là, la fermeture est IMMÉDIATE et RÉTROACTIVE** — les lignes du cycle `2026-08-31` sont posées depuis le 07/09 18:00 UTC, celles du `2026-09-07` le seront le 14/09 : **au déploiement, les DEUX cycles se ferment d'un coup.** Mesuré en prod le 07/09 à 13:51 UTC : **480 dépôts du routeur** sur `2026-08-31`, dont **198 jamais rendus** (170 `assigne` + 28 `ouvert`) et **83 en attente de retour**, pour **61 élèves**. **À remesurer avant de pousser.**
