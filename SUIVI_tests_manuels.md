...
- **Production (23/09, sur l'ordre de Louis, absent de son poste)** : `quazian_antichambre.sql` puis `quazian_tuteur.sql` joués AVANT le push (répétition annulée puis passage réel, constats au `SUIVI_SQL.md`), portes relues à OFF. **Commit local fait par l'assistant ; le PUSH a ÉCHOUÉ** : aucun identifiant GitHub dans cette séance (HTTPS sans jeton, pas de `gh`, pas de clé SSH, trousseau vide). **Le code n'est donc PAS déployé** : la production tourne sur l'ancien code, qui ignore les deux migrations (inertes, portes OFF).
- **Reste à Louis** : **pousser `main`** (GitHub Desktop ou terminal), hors passation en cours ; **créer le prochain quiz APRÈS le déploiement** (les questions générées avant gardent la bonne réponse en tête) ; ouvrir les deux portes (Quazian → Paramètres → Passation). ⚠️ Ne pas pousser pendant une passation : les points 1-4, la fuite et la pause changent la passation sans porte.
