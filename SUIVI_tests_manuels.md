...
- `scriptorium_presentation.sql` — UNE colonne `scriptorium_contenus.presentation_ref text null` (chemin `<prof>/<contenu>/presentation-<horodatage>.html` dans le bucket privé `scriptorium`). **Jouée en bac à sable le 14/09** (12 contenus intacts, 0 deck) ; **la prod est le geste de Louis**. Rollback fourni. Lecture tolérante (`presentationsDesContenus`) : le code peut partir avant le SQL — et si la colonne manque au dépôt, le fichier est retiré et le message le dit.
