...
- [x] **C7L7-11 — ⑤ le pull, la reprise à l'identique** (en mémoire, après la semaine 2 : 3 exercices, 24 min, quota 30) : le pull dit `objet_un_par_cycle` sur `argument` et `exemple`, et « rien ne tient » — le 3 d'`objection` porte le même observable que son 1 (Q1, résidu). ⚠️ `servirUnExerciceDePlus` n'a pas été appelé hors Next (`next/navigation`).
...
- [x] **C7L7-14 — les 154 tests de C7-L6 restent verts** ; 187 moteur + registre ; 492 avec le routeur (PB2 sur l'observable, le trio entier, les prérequis de la bande) ; `tsc` propre.
...
- [ ] **C7L7-20 — la séquence de méthode sous PB2-observable** : les crans 1·3·2 d'une même clé portent le même observable ; la séquence ne s'enchaîne que si un autre objet alterne. Condition : Louis dit si la méthode est l'exception à PB2 (une ligne dans `poserLaSemaine`).
