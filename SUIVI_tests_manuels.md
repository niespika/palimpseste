...
- [x] **C7L7-4 — ④ semaine 1 en base** (`poserLesSemainesDuRouteur`, cycle 2026-09-14, segment 3) : **5 décisions, 5 dépôts** ; objets `argument` (crans 1·3·4) et `exemple` (1·3), toutes `objet.etat = methode`, `semaine_1: true`, `methode.rang` et séquence sur chaque ligne, écart au plancher vide — **fait quand (1a)**. ⚠️ Au premier essai (avant « un objet par compétence ») : 1 seule décision — voir Q1 au relevé.
- [x] **C7L7-5 — ④ semaine 2 en base** (cycle 2026-09-21) : `argument` OUVERT reçoit **un** exercice, cran 1 (« à servir 1 »), sur un devoir non servi en semaine 1, observable `lien_explicite` **non acquis** (17 mesures) — la moitié du fait quand (1b) ; `exemple` écarté `objet_sans_cran_ouvert` (Q2/Q3 au relevé) ; **aucun objet neuf** (Q1). Constaté par requête sur `routeur_decisions`.
...
- [x] **C7L7-11 — ⑤ le pull, la reprise à l'identique** (en mémoire) : le premier pull sert « exemple/1 » (Structure, non acquis, 3 mesures) ; le second dit `objet_un_par_cycle` sur `exemple` et « rien ne tient ». ⚠️ `servirUnExerciceDePlus` n'a pas été appelé hors Next (`next/navigation`).
...
- [x] **C7L7-14 — les 154 tests de C7-L6 restent verts** ; 183 moteur + registre ; 487 avec le routeur ; `tsc` propre.
...
- [ ] **C7L7-17 — fait quand (1b) entier** : un exercice sur CHACUN des deux objets ouverts et un troisième objet en méthode la semaine 2. Condition : Q1 (PB2 contre la séquence de méthode), Q2 (la bande dure de C contre la séquence C-B — 50 élèves sur 63 en prod) et Q3 (un objet ouvert n'est servi que si sa compétence est dans la liste) tranchées par Louis.
