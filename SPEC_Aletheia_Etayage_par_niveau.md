...
| Citations du livre faites par l'IA, retrouvées verbatim dans le texte stocké | 117 / 242 | une citation spontanée sur deux est une paraphrase |
...
|---|---|---|---|---|
| `montre` | E, D | **dix lignes** de notre version, la pivot au milieu jamais en tête, mention « dans notre version » | répond en une ou deux phrases | aucune par le code ; le retour VF juge la réponse |
...
| `demi_section` | A | la moitié de la section contenant la pivot, bornée au paragraphe | idem | idem |
...
et une distance d'édition ≤ 2 sont considérés égaux. Le recto reste le mot tel que l'élève
l'a écrit.
...
| **E3** ⏳ code 03/09 (E3a), SQL bac à sable ☑ 03/09, prod ☐ ; recette écrans à faire (E3b) | Gabarits : `gabarits.ts` (4 gabarits en questions, cycle fixe, blocs de prompt), `gabarit-serveur.ts`, colonnes `gabarit_lecture` / `cycle_tournante` / `champ_fixe` / `champ_fixe_vf` / `tournante_cle` / `blocs_gabarits` (`aletheia_etayage_l3.sql`), placeholders `{bloc_gabarit}` dans les 5 prompts, formulaires et page élève, sélecteur dans l'éditeur du livre, surcharge par séance dans la fiche ; identité porte fermée prouvée par `scripts/recette/aletheia-prompts-identite.mjs` | — | reste : un livre par gabarit passe une séance complète en preview, sur les trois tailles d'écran, retours lus par Louis ; l'éditeur prof des blocs de prompt (`blocs_gabarits`) n'a pas encore d'écran (lu par le code, éditable en base) |
