...
**C4-L9 — ⚠️ BOÎTE OUVERTE, quatre items.** *Le lot est né en séance de conception le 22/08 ; **son prompt n'est pas fabriqué**, et il ne se fabrique pas avant que cette boîte soit vide.*

1. ✅ **C4L4-8 — « par quelle voie naît une passation diagnostique ? »** Les deux types seedés sont **vides** *(0 cran, 0 mode, `nature = 'complet'`)*, et **la doctrine ne connaît que les treize objets** *(`RELEVE_C4_L4_2026-08-22.md` §7 ter)*. **Répondu en séance de conception le 22/08, et entré au `07-` §2** : la conception d'un examen diagnostique **ne passe pas par la doctrine objet × mode × cran** — ce n'est pas un objet à un cran, c'est une **copie entière** —, et **c'est la ligne du plan d'évaluation qui appelle la conception**, jamais l'inverse. **L'écran ne demande donc aucune date.**
2. ⚠️ **La clé qui relie la ligne de plan à l'instance n'existe pas, et le sens est fixé.** `scriptorium_exercices_planifies` porte `quiz_id` et `codex_session_id` ; **rien vers `exercices`**. Et **les deux tables ont chacune un cycle `a_concevoir → concu` qui ignore l'autre** — celui de la ligne de plan, et celui de l'instance : *vérifié le 22/08, `app/prof/conception/` ne lit jamais `scriptorium_exercices_planifies`*. **La clé se pose sur la ligne de plan**, à côté des deux autres.
3. ⚠️ **Le patron du signal « à concevoir » existe déjà, DEUX FOIS — ne le réécris pas.** `app/prof/codex/synthese-a-preparer.ts` et `app/prof/quazian/quizz/plan-quazian.ts` servent tous deux l'encart depuis les lignes de plan *à concevoir* des plans validés courants, avec leur date ; la colonne `statut` *(`a_concevoir` · `concu` · `annule`)*, l'index `on (statut, semaine_lundi) where statut = 'a_concevoir'` et les trois pastilles de la grille *(« à concevoir » · « en retard » · « conçu »)* existent. **Le troisième cas se sert du même chemin.**

4. ⚠️ **Le renommage des deux codes de type part avec toi, et il n'a PAS été fait en conception.** `diagnostic_essai` → **`examen_diagnostique_essai`** et `diagnostic_explication_texte` → **`examen_diagnostique_explication_texte`** *(tranché le 22/08 : cinq codes partageaient le préfixe `diagnostic_` — trois pour le **geste** `diagnostiquer`, `diagnostic_guide` · `diagnostic_nomme` · `diagnostic_fin`, et deux pour l'**ancre**)*. **Pourquoi il t'attend** : `c4_l1_seed.sql` est **déjà joué**, et éditer une migration jouée la ferait mentir sur ce qui a tourné — pendant qu'une base prod neuve recevrait des codes qu'aucune migration n'a produits. **Tu poses de toute façon `genres_admis` et `exclusions_parcours` sur ces deux lignes** *(item 1 de ta mission)* : fais le renommage dans la même migration. **Quatre fichiers suivent** — `c4_l1_seed.sql` et son rollback *(pour la base neuve)*, le commentaire de `utils/passation/metacognition.ts`, la fixture de `utils/fabrique/doctrine-lecture.test.ts`. **Et deux libellés**, qui rendent aujourd'hui « Écriture diagnostique (classe) » et doivent rendre **« Examen diagnostique — écriture (classe) »** : `libelleType()` et `TYPES_AJOUT`, dans `GrillePlan.tsx` **et** dans `GrilleModele.tsx`. ⛔ **Ne renomme RIEN dans `scriptorium_exercices_planifies`** — la colonne `diagnostique`, `fenetre_diagnostique` et `origine = 'diagnostic'` **restent** : ≈ 50 sites, une migration sur une table vivante, et **aucune ambiguïté dans cette table**, qui ne parle jamais de crans. *Renoncement chiffré et assumé.*
