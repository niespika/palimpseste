    ...
    <div className="mb-8 space-y-3">
      {classes.map((c) => {
        const sousTitre = [c.niveau ? NIVEAU_LABEL[c.niveau] ?? c.niveau : null, c.filiere, c.annee_scolaire]
          .filter(Boolean).join(' · ')
        return (
          <div key={c.id} className="bg-amber-50 border border-amber-200 rounded-xl px-5 py-4 flex flex-wrap items-center justify-between gap-3">
            <div>
              <p className="text-sm text-amber-900">
                <span className="font-medium">Fin d'année</span> — la classe « {c.nom} » arrive-t-elle à terme ?
              </p>
              {sousTitre && <p className="text-xs text-amber-700/70 mt-0.5">{sousTitre}</p>}
            </div>
            <div className="flex items-center gap-2 flex-shrink-0">
              <button
                onClick={() => continuer(c.id)}
                disabled={pending === c.id}
                className="text-xs px-3 py-1.5 rounded-lg bg-white border border-amber-300 text-amber-800 hover:bg-amber-100 disabled:opacity-50"
              >
