    ...
    <div className="space-y-8">
      ...
      {groupe === 'retours' && (
        <div className="space-y-8">
          ...
          <BlocPrompt
            label="Retour final (VF) — reconstruction + architecture (amont + semaine N + titres aval)" value={p2} onChange={setP2} defaut={defauts.feedback2} rows={22}
            warn={<>⚠️ Le contexte n&apos;est plus le livre entier : <strong>amont</strong> (déjà lu) + <strong>texte de la semaine N</strong> + <strong>titres seuls</strong> de l&apos;aval. Garde la non-divulgation de l&apos;aval et les variables <code>{'{amont_structure}'}</code>, <code>{'{semaine_courante_texte}'}</code>, <code>{'{aval_titres}'}</code>. <strong>Un ancien prompt contenant <code>{'{livre_entier}'}</code> est ignoré</strong> (reviens au défaut, puis ré-adapte).</>}
            hint={<>Variables : <code>{'{amont_structure}'}</code>, <code>{'{semaine_courante_texte}'}</code>, <code>{'{aval_titres}'}</code>, <code>{'{these_initiale}'}</code>, <code>{'{arguments_initiale}'}</code>, <code>{'{accord_initial}'}</code>, <code>{'{these_vf}'}</code>, <code>{'{arguments_vf}'}</code>, <code>{'{accord_vf}'}</code>, <code>{'{syntheses_precedentes}'}</code>, <code>{'{architectures_precedentes}'}</code>, <code>{'{trajectoire_diagnostic}'}</code>, <code>{'{semaine_courante_N}'}</code>, <code>{'{total_semaines}'}</code>. Sortie JSON <code>{'{ synthese_modele, ajouts_verifies, nuances_et_erreurs, architecture_amont, architecture_aval_jalons }'}</code>.</>}
          />
