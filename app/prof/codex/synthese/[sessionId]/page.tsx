...
export default async function SynthesePage({
  ...
  const uniteLabel = libelleSession(session.scriptorium_unites, null)
    || (await titresCoursParSession(supabase, [sessionId])).get(sessionId) || ''
  ...
  // Roster = élèves inscrits dans la classe de la synthèse (ou tous ceux ayant
  // accès au module Codex si la synthèse n'est rattachée à aucune classe).
  const { data: moduleData } = await supabase.from('modules').select('id').eq('slug', 'codex').single()
  const eleveIds = session.classe_id
    ? await eleveIdsInscritsClasse(supabase, session.classe_id as string)
    ...
    <div>
      ...
      {session.statut === 'brouillon' ? (
        ...
        <TableauSynthese
          ...
          phaseFinAt={session.phase_courante_fin_at}
          eleves={eleves}
        />
      )}
    </div>
