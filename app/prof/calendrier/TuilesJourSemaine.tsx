...
export default async function TuilesJourSemaine() {
  ...
  const ligne = (e: CalendarEvent, i: number, avecDate = false) => (
    <div key={i} className="flex items-center gap-2">
      ...
      />
      <span className="text-sm text-encre-douce truncate">
        {avecDate && <span className="text-xs text-muet mr-1">{new Date(e.date + 'T00:00:00Z').toLocaleDateString('fr-FR', { weekday: 'short', day: 'numeric', timeZone: 'UTC' })}</span>}
        {e.label}
        {e.classe_nom && <span className="text-muet"> · {e.classe_nom}</span>}
      </span>
    </div>
  )
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
      ...
      <div className="bg-surface border border-bordure rounded-xl p-5">
        <p className="text-xs font-medium text-muet uppercase tracking-wide">Aujourd&apos;hui</p>
        <p className="text-sm text-muet capitalize mb-3">{fmtJourLong(today)}</p>
        {evJour.length === 0 ? (
          <p className="text-sm text-muet">Rien de prévu.</p>
        ) : (
      ...
      <Link
        ...
        className="bg-surface border border-bordure rounded-xl p-5 block hover:border-muet hover:shadow-sm transition-all"
      >
        <div className="flex items-baseline justify-between">
          <p className="text-xs font-medium text-muet uppercase tracking-wide">Cette semaine</p>
          <span className="text-xs text-muet">vue détaillée →</span>
        </div>
        {evSemaine.length === 0 ? (
          <p className="text-sm text-muet mt-3">Rien de prévu cette semaine.</p>
        ) : (
          <div className="space-y-1.5 mt-3">
            {evSemaine.slice(0, 5).map((e, i) => ligne(e, i, true))}
            {evSemaine.length > 5 && <p className="text-xs text-muet">+ {evSemaine.length - 5} autre{evSemaine.length - 5 > 1 ? 's' : ''}…</p>}
          </div>
        )}
      </Link>
    </div>
