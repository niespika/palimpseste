...
// Une fois le PDF lu, l'encart d'upload s'efface : la barre fine du navigateur prend le
// relais (méta + « Remplacer le PDF »).
export default function DecoupePdf({ nb, titre, auteur, onReady }: {
  nb: number
  titre?: string
  auteur?: string | null
  onReady: (pret: boolean) => void
}) {
  ...
  const [signets, setSignets] = useState<Signet[] | null>(null)
  ...
  async function deposer(file: File) {
    ...
    setImportId(null); setAnalyse(null); setPages(null); setSignets(null)
    setEtat('upload')
  ...
  // « Remplacer le PDF » : purge l'import courant et revient à l'étape d'upload.
  function remplacer() {
    onReady(false)
    if (importIdRef.current) { void supprimerImportPdf(importIdRef.current); importIdRef.current = null }
    setImportId(null); setAnalyse(null); setPages(null); setSignets(null)
    setErreur(null); setEtat('vide')
  }

  const pret = etat === 'pret' && !!pages && !!analyse && !analyse.tropLong
    ...
    <div className="space-y-3">
      ...
      {pret ? (
        <>
          ...
          <NavigateurDecoupe
            pages={pages!} nb={nb} emettreChamps onEtat={handleEtat}
            meta={{ titre, auteur, texteSelectionnable: true }}
            signets={signets} onRemplacer={remplacer}
          />
        </>
      ) : (
        <div className="border border-bordure rounded-lg p-3 space-y-2">
          ...
          {analyse && !analyse.tropLong && <p className="text-sm text-encre-douce">PDF de <span className="font-medium">{analyse.totalPages} pages</span> détecté.</p>}
          {analyse?.tropLong && (
