...
export default function BoutonActiverClasse({ classe }: { classe: string }) {
  const [chargement, setChargement] = useState(false)
  const [message, setMessage] = useState<string | null>(null)

  async function handleActiver(actif: boolean) {
    setChargement(true)
    const res = await activerEssaiPourClasse(classe, actif)
    setChargement(false)
  ...
  return (
    <div className="flex items-center gap-1.5">
      <span className="text-sm font-medium text-stone-700">{classe}</span>
      <button
