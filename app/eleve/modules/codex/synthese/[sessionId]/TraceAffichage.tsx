...
export function TraceAffichage({ trace }: { trace: TraceCodex }) {
  ...
  return (
    <div className="space-y-6">
      {/* Ce que l'élève n'a pas su dire */}
      {trace.erreurs_corrections.length > 0 && (
        <section>
          <h3 className="text-sm font-medium text-stone-700 mb-2">Ce que tu n&apos;as pas su dire</h3>
      ...
      {trace.synthese_completee && (
        <section>
          <h3 className="text-sm font-medium text-stone-700 mb-2">Ta synthèse, complétée</h3>
      ...
      {trace.erreurs_corrections.length === 0 && !trace.synthese_completee && (
        <p className="text-center text-stone-400 text-sm py-8">Retour validé, sans correction à signaler.</p>
      )}
    </div>
  )
}
